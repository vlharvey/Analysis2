...
;
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yt_waccm_usigma_annualcycle.ps'
       device,/color
...
;
    ...
    for i=0,nday-1L do begin
        ...
        for j=0,nr-1L do begin
            ...
            if index(0) ne -1L then begin
               result=moment(ubar(j,index))
               ubar_annual_yt(i,j)=sqrt(result(1))
            endif
        endfor
    endfor
...
;   xyouts,xmx-0.08,(ymx+ymn)/2.,zlab+' km',/normal,color=0,charthick=2,charsize=2
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim yt_waccm_usigma_annualcycle.ps -rotate -90 yt_waccm_usigma_annualcycle.jpg'
    endif
end

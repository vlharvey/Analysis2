...
saveit:
save,file='zt_geos5_tbarth.sav',utz,ttz,alat,th,nr,nth,kday,sdates
plotit:
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_geos5_ubarth_'+slat+'.ps'
   device,/color
...
contour,utzlat,1.+findgen(kday),th,/noeras,xrange=[1.,kday],yrange=[300.,4000.],$
      charsize=1.5,color=0,ytitle='Theta (K)',title='GEOS-5 Ubar at '+slat+' Latitude',/fill,c_color=col1,$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
index=where(level gt 0)
contour,smooth(utzlat,7),1.+findgen(kday),th,levels=[0],color=0,/follow,/overplot,c_labels=1+0*index,thick=2
contour,smooth(utzlat,7),1.+findgen(kday),th,levels=[100],color=0,/follow,/overplot,c_labels=1+0*index,thick=2
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_geos5_ubarth_'+slat+'.ps -rotate -90 zt_geos5_ubarth_'+slat+'.jpg'
endif

;
; timeseries of chemical distinction d and vortex centroid latitude
;
@stddat
@kgmt
@ckday
...
;read,'Enter desired theta ',rth
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='timeseries_d+y0_'+sth+'.ps'
             device,/color
...
area1_all=centroid_latitude1_all
...
if iyear eq 0L then begin
   if ssw(iyear) eq 0 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[40,90],color=0,charsize=2,charthick=2,ytitle='Vortex Centroid Latitude',xrange=[startday-1,max(fdoy)-30],xticks=n_elements(smonth)-1,xtickname=smonth,thick=5,/noeras
      for i=0L,nn-1L do begin
          if fdoy(i) ge startday and fdoy(i) lt max(fdoy)-30 then begin
          plots,fdoy(i),arrmean(i)-arrsig(i)
          plots,fdoy(i),arrmean(i)+arrsig(i),/continue,color=150,thick=8
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim timeseries_d+y0_'+sth+'.ps -rotate -90 timeseries_d+y0_'+sth+'.jpg'
endif

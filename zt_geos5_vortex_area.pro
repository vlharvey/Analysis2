...
plotit:
restore,'vortex_area_2003_2004_geos5.sav'
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_geos5_vortex_area_'+y1+'-'+y2+'.ps'
   device,/color
...
plot,[1,kday,kday,1,1],[400.,400.,3000.,3000.,400.],min_value=0.,$
      ...
      ytitle='Theta (K)',title='GEOS-5 Vortex Area (Strongest Jet) '+y1+'-'+y2,xtickname=[' ',' '],xticks=1
...
plot,[1,kday,kday,1,1],[400.,400.,3000.,3000.,400.],min_value=0.,$
      xrange=[1,kday],yrange=[400.,3000.],/nodata,charsize=1.5,color=0,$
      ytitle='Theta (K)',title='GEOS-5 Vortex Area (Poleward-most Jet) '+y1+'-'+y2,xtickname=[' ',' '],xticks=1
kindex=where(strmid(sfile,3,2) eq '15',nxtick)
xmon=long(strmid(sfile(kindex),0,2))
for i=0,nxtick-1 do begin
    xlab=smon(xmon(i)-1)
...
area_zt=smooth(area_zt_nc5,5,/edge_truncate)
harea_zt=smooth(harea_zt,5,/edge_truncate)
contour,area_zt,1.+findgen(kday),th,levels=level,/fill,/cell_fill,/overplot,c_color=col1
contour,area_zt,1.+findgen(kday),th,levels=level,c_color=0,/follow,/overplot
;contour,harea_zt,1.+findgen(kday),th,levels=level2,c_color=mcolor,/follow,/overplot,thick=4
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='% of the N. Hemisphere',charsize=1.5
ybox=[0,10,10,0,0]
x1=imin

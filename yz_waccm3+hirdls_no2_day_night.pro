...
;
    ...
    for j=0L,nr-1 do begin
        no2yth_mean_waccm(j,k)=total(no2(j,xindex,k))/float(nxpts)
        o3yth_mean_waccm(j,k)=total(o3(j,xindex,k))/float(nxpts)
        tempyth_mean_waccm(j,k)=total(tmp2(j,xindex,k))/float(nxpts)
    endfor
    endfor
;
; restore monthly mean HIRDLS data (DAYTIME)
;
    restore,hdir+mon(imonth)+'_avg_yth_'+ver+'_day.sav'	;nr,nth,alat,th,pyth_mean,no2yth_mean,o3yth_mean,tempyth_mean
    pyth_mean_day=pyth_mean
    no2yth_mean_day=no2yth_mean
    o3yth_mean_day=o3yth_mean
    tempyth_mean_day=tempyth_mean
    restore,hdir+mon(imonth)+'_avg_yth_'+ver+'_night.sav' ;nr,nth,alat,th,pyth_mean,no2yth_mean,o3yth_mean,tempyth_mean
    pyth_mean_night=pyth_mean
    no2yth_mean_night=no2yth_mean
    o3yth_mean_night=o3yth_mean
    tempyth_mean_night=tempyth_mean
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_waccm3+hirdls_'+mon(imonth)+'_no2_day_night.ps'
       device,/color
...
;
    ...
    contour,no2yth_mean*1.e9,alat,th,levels=no2level,c_color=col1,/noeras,/cell_fill,$
            title='HIRDLS Daytime NO!l2!n',color=0,yrange=[500.,2000.],min_value=0.,xticks=6,$
...
;
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_waccm3+hirdls_'+mon(imonth)+'_no2_day_night.ps -rotate -90 yz_waccm3+hirdls_'+mon(imonth)+'_no2_day_night.jpg'
;      spawn,'/usr/bin/rm yz_waccm3+hirdls_'+mon(imonth)+'_no2_day_night.ps'
    endif
    skipmonth:
endfor
end

...
;
spawn,'ls '+dir+'*_2d_nh.sav',ifiles
...
;
        if icount eq 0L then begin
           np=n_elements(pmls)
           zm_markco_mls_daily_nh=fltarr(nr,np,nday)
           area_markco_mls_prof_nh=fltarr(np,nday)
...
;
        ...
        zm_markco_mls_daily_nh(*,*,kcount:kcount+n_elements(SDATE_TIME)-1L)=zm_markco_mls_daily_nh(*,*,kcount:kcount+n_elements(SDATE_TIME)-1L)+zmmark
;
...
;
        for ii=0L,n_elements(SDATE_TIME)-1L do begin
            ...
            for kk=0L,np-1L do begin
                ...
                index=where(mark2d gt 0. and y2d gt 0.)
                if index(0) ne -1L then area_markco_mls_prof_nh(kk,kcount+ii)=100.*total(area(index))/hem_area
            endfor	; loop over pressure levels
;print,SDATE_TIME(ii),' ',area_markco_mls_prof_nh(*,ii)
...
;
; save daily zonal means and daily profiles of vortex area (NH)
;
save,file='mls_COmark_daily_zm_nh.sav',nr,np,nday,lat,pmls,zprof,zm_markco_mls_daily_nh,area_markco_mls_prof_nh,sdate_all

;
;Purpose: create a plot of the 40 year mean annual cycle of polar cap average temperatur, No ES events
;
;
;
;
...
x = where(esday eq 1., nevents)
for iday = 0L, 29L do begin
	if iday eq 0L then meantempseries = fltarr(30L, n_elements(altitude))*!values.f_nan
	y = where(esday eq iday+1L)
	for kk = 0L, n_elements(altitude) - 1L do meantempseries[iday,kk] = mean(temp[y,*,kk],/nan)
endfor
...
if nx gt 0L then fake[x] = min(level) +.1
  ...
  contour, fake,days, z ,  levels=level, /cell_fill, c_color = col1,$
           xrange = [1,30],yrange=[20,100],ytitle='Altitude (km)', ystyle = 1,title = 'NH Mean Annual Cycle - No ES',$
...
loadct, 39
  ...
  if setplot eq 'ps' then begin
	...
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/WACCM_mean_timeseries_composite_ES.png'

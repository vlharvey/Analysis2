;
; read existing MERRA to WACCM files for T, U, V. Substitute Ozone with Water and add DP
...
;  contour,qbar,latitude_rad,levels_rad,/overplot,levels=1.+findgen(19),thick=3
   ...
   qvbar_new=fltarr(nr_waccm,nl_assim)
   for j=0,nr_waccm-1L do $
       for k=0,nl_assim-1L do tbar_new(j,k)=mean(tnew(*,j,k))
   for j=0,nr_waccm-1L do $
       for k=0,nl_assim-1L do qvbar_new(j,k)=mean(qvnew(*,j,k))
   contour,tbar_new,latitude_waccm,levels_assim,/ylog,yrange=[1000.,0.01],levels=150+10.*findgen(30),thick=3,title=sdate,c_color=col1,/fill
;  contour,psnew,longitude_waccm,latitude_waccm,/overplot,levels=500+10.*findgen(50),color=250
   contour,qvbar_new*1.e6,latitude_waccm,levels_assim,/overplot,levels=0.25*findgen(40),thick=3,c_linestyle=5,color=250

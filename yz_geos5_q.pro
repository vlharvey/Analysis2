;
; GEOS-5 plot zonal mean diabatic heating rate
;
@stddat
@kgmt
@ckday
...
; --- Test for end condition and close windows.
      ...
      rd_geos5_nc3_meto,dir+date+'_AVG.V01.nc3',nc,nr,nth,alon,alat,th,$
                  pv2,p2,msf2,u2,v2,q2,qdf2,markold2,sf2,vp2,iflag
      if iflag eq 1 then goto,skipit

q2=q2/86400.
      dum1=findfile(dir+date+'_AVG.V01.nc4')
      if dum1(0) ne '' then ncid=ncdf_open(dir+date+'_AVG.V01.nc4')
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_geos5_q_'+date+'.ps'
   device,/color
...
plot,[min(alat),max(alat),max(alat),min(alat),min(alat)],[500.,500.,5000.,5000.,500.],min_value=0.,$
      ...
      ytitle='Theta (K)',title='GEOS-5 Diabatic Heating Rate '+date,xticks=6
nlvls=41
col1=1+indgen(nlvls)*icolmax/nlvls
level=-10.+0.5*findgen(nlvls)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_geos5_q_'+date+'.ps -rotate -90 yz_geos5_q_'+date+'.jpg'
endif

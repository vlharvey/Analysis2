...
; GEOS-5 version
;
; plot daily Tbar from theta data
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_geos5_tbarth+mark_'+sdate+'.ps'
   device,/color
...
contour,tzm,alat,th,/noeras,xrange=[-90.,90.],yrange=[min(th),1000.],charsize=1.5,color=0,$
...
contour,uzm,alat,th,/noeras,xrange=[-90.,90.],yrange=[min(th),1000.],charsize=1.5,color=0,$
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_geos5_tbarth+mark_'+sdate+'.ps -rotate -90 yz_geos5_tbarth+mark_'+sdate+'.jpg'
endif

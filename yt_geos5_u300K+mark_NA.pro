;
; save only N. American sector. Not zonal mean
;
...
save,file='yt_geos5_ztuvqpv_'+sdate0+'-'+sdate1+'_NA.sav',zbar_yt,tbar_yt,ubar_yt,$
     vbar_yt,qbar_yt,pvbar_yt,markbar_yt,pbar_yt,sdate_all,alat,alat,th,nfile

plotit:
;restore,'yt_geos5_ztuvqpv_20080901-20081209_NA.sav
nth=n_elements(th)
sdate0=sdate_all(0)
sdate1=sdate_all(n_elements(sdate_all)-1)
...
;   level=-25.+2.5*findgen(21)
    ...
    contour,tbar,findgen(nfile),alat,charsize=1.5,/noeras,yrange=[30.,90.],$
    ...
    contour,tbar,findgen(nfile),alat,charsize=1.5,/noeras,/overplot,levels=level,/follow,color=0
...
;   contour,ubar,findgen(nfile),alat,charsize=1.5,/noeras,/overplot,levels=-1.*[120,100,80,60,40],color=mcolor,c_linestyle=5
    markbar=smooth(transpose(reform(markbar_yt(*,18,*))),5)	; 1000 K
...
loadct,0
    contour,markbar,findgen(nfile),alat,levels=[1],color=mcolor*.5,/follow,/noeras,/overplot,min_value=0.,thick=10,c_labels=[0]
loadct,39
    result=moment(zbar)
    zlab=string(long(result(0)))
;   xyouts,xmx-0.08,(ymx+ymn)/2.,zlab+' km',/normal,color=0,charthick=2,charsize=2
    for ii=0L,nxticks-1L do xyouts,xindex(ii),28.,smon(long(xlabs(ii))-1),/data,color=0
    index=where(smn eq '07' and sdy eq '01',nyear)
    for ii=0L,nyear-1L do xyouts,index(ii),0.,syr(index(ii)),/data,color=0,charsize=2
    xyouts,nfile-60.,15.,syr(nfile-1),/data,color=0,charsize=2
    imin=min(level)
    imax=max(level)

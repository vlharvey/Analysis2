...
;NH_UBAR60       FLOAT     = Array[6720, 22]
;SH_DTDY         FLOAT     = Array[6720, 22]
;SH_UBAR60       FLOAT     = Array[6720, 22]
;TH              FLOAT     = Array[22]
;YYYYMMDD        LONG      = Array[6720]
;
restore,file='MetO_dTdy_Ubar_SSW_Climo.sav
nl=n_elements(th)
...
; 
    ...
    nh_major=NH_UBAR60(index,*)
    sh_minor=SH_DTDY(index, *)
    sh_major=SH_UBAR60(index,*)
;
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_dtdy_ubar_meto_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
xyouts,.2,.775,'MetO '+yearlab,/normal,charsize=2.5,charthick=2,color=0
...
if setplot eq 'ps' then begin
device,/close
spawn,'convert -trim zt_dtdy_ubar_meto_'+yearlab+'.ps -rotate -90 zt_dtdy_ubar_meto_'+yearlab+'.jpg'
endif
end

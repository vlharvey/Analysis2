...
yorig=[0.85+0.05,0.7+0.04,0.55+0.03,0.4+0.02,0.25+0.01,0.1]-0.06
xlen=0.5
ylen=0.15
cbaryoff=0.075
cbarydel=0.04
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='zt_ace_co_spring_5pan.ps'
   device,/color
...
;
    ...
    if syear(iyear) eq '2004' then begin
       oco_fill(0:41,*)=0.      ; do not extrapolate data voids 
    ...
    if syear(iyear) eq '2008' then begin
       if min(fdoy) gt 3. then oco_fill(0:long(min(fdoy))-1,*)=0.
    endif
    if syear(iyear) eq '2009' then begin
;      if max(fdoy) lt 90. then oco_fill(long(max(fdoy)):long(nday)-1L,*)=0.
    endif
    oco=oco_fill
;
; smooth
...
col1=1L+indgen(nlvls)*mcolor/float(nlvls)
    print,syear(iyear),'  ',min(fdoy),max(fdoy)
    kday=365.
...
plot,[0,0],[min(level),max(level)],xrange=[0,10],color=0,$
     ...
     yrange=[min(level),max(level)],charsize=1.5,title='CO (ppmv)',charthick=2
xbox=[0,10,10,0,0]
y1=min(level)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_ace_co_spring_5pan.ps -rotate -90 zt_ace_co_spring_5pan.jpg'
endif
end

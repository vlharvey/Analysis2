;
; NH version
; plot ACE ozone in longitude altitude section
; add polar projection at a user prompted theta
; VLH 2/6/05
;
; swaths of MLS added	VLH 5/19/06
;
; find coincidences between ACE and MLS inside anticyclones
; VLH 10/28/06
;
@aura2date
@rd_ukmo_nc3
...
ifile='                             '
lstmn=1 & lstdy=1 & lstyr=2006 & lstday=0
...
;mlat=o3.latitude
;mlon=o3.longitude
;index=where(mlon lt 0. and mlon ne -999.990)
...
;
      ...
      if norbith gt 1L then begin
         index=where(yhal ge 0. and yhal le 90.,norbith)
         ...
         if index(0) ne -1 then begin
            ...
            o3halsr=o3hal(index,*)
            thhalsr=thhal(index,*)
            index=where(yhalsr ge 0. and yhalsr le 90.)
         ...
         if index(0) ne -1 then begin
            ...
            o3halss=o3hal(index,*)
            thhalss=thhal(index,*)
            index=where(yhalss ge 0. and yhalss le 90.)
      ...
      if norbits3 gt 1L then begin
         index=where(ysage3 ge 0. and ysage3 le 90.,norbits3)
         if index(0) ne -1 then begin
            ysage3=ysage3(index)
            xsage3=xsage3(index)
...
;           index=where(ysage3ss ge 0. and ysage3ss le 90.)
...
;        endif
      ...
      if norbits2 gt 1L then begin
         index=where(ysage2 ge 0. and ysage2 le 90.,norbits2)
         if index(0) ne -1 then begin
            ysage2=ysage2(index)
            xsage2=xsage2(index)
         ...
         if index(0) ne -1 then begin
            ...
            index=where(ysage2sr ge 0. and ysage2sr le 90.)
         ...
         if index(0) ne -1 then begin
            ...
            index=where(ysage2ss ge 0. and ysage2ss le 90.)
      ...
      if norbitp gt 1L then begin
         index=where(ypoam ge 0. and ypoam le 90.,norbitp)
         if index(0) ne -1 then begin
            ypoam=ypoam(index)
            xpoam=xpoam(index)
         ...
         if index(0) ne -1 then begin
            ...
            index=where(ypoamsr ge 0. and ypoamsr le 90.)
         ...
         if index(0) ne -1 then begin
            ...
            index=where(ypoamss ge 0. and ypoamss le 90.)
      ...
      if norbiti gt 1L then begin
         index=where(yilas ge 0. and yilas le 90.,norbiti)
         if index(0) ne -1 then begin
            yilas=yilas(index)
            xilas=xilas(index)
         ...
         if index(0) ne -1 then begin
            ...
            index=where(yilassr ge 0. and yilassr le 90.)
         ...
         if index(0) ne -1 then begin
            ...
            index=where(yilasss ge 0. and yilasss le 90.)
      ...
      if norbita gt 1L then begin
         index=where(yace ge 0. and yace le 90.,norbita)
         if index(0) ne -1 then begin
            yace=yace(index)
            xace=xace(index)
         ...
         index=where(modea eq 'r' or modea eq 's')
         if index(0) ne -1 then begin
            yacesr=yace(index)
            xacesr=xace(index)
            o3acesr=o3ace(index,*)
            thacesr=thace(index,*)
            index=where(yacesr ge 0. and yacesr le 90.)
         ...
         jumpacesr:
;        index=where(modea eq 's' or modea eq 'r')
;        if index(0) ne -1 then begin
;           yacess=yace(index)
;           xacess=xace(index)
;           o3acess=o3ace(index,*)
;           thacess=thace(index,*)
;           index=where(yacess ge 0. and yacess le 90.)
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,bits=8,$
                 filename='XZ_Ozone/xz_o3_ace+mls+polar_nh_'+lfile+'_'+stheta+'K_coins.ps'
         device,/color
...
;         if index(0) ne -1 then oplot,xz2d(index),yz2d(index),psym=3,color=mcolor
          ...
          !type=2^2+2^3+2^5
          plot,[0,0],[imin,imax],xrange=[0,10],yrange=[imin,imax],color=0,charsize=1.5,charthick=2
          xbox=[0,10,10,0,0]
          y1=imin
      ...
      mark(nc,*)=mark(0,*)
      !type=2^2+2^3
      xyouts,.35,.95,syr+smn+sdy+'  '+stheta+' K',/normal,color=0,charsize=3,charthick=2
      set_viewport,.05,.45,.525,.925
      ...
      xmdata=mlon2(index) & ymdata=mlat2(index)
      o3mdata=mo3(index)
      ...
      omin=2.
      omax=10.	;max(o3mdata)
      for i=0L,n_elements(o3mdata)-1L do begin
          if o3mask(i) ne -99. then $
             oplot,[xmdata(i),xmdata(i)],[ymdata(i),ymdata(i)],psym=8,$
                   color=((o3mdata(i)-omin)/(omax-omin))*mcolor,symsize=1.5
      endfor
;
...
;
      ...
      if norbits3 gt 1L then begin
         ...
         index=where(abs(thsage3-rtheta) le 25. and o3sage3 gt 0. and ysage3d gt 0.,s3count)
      ...
      if norbitp gt 1L then begin
         ...
         index=where(abs(thpoam-rtheta) le 25. and o3poam gt 0. and ypoam2d gt 0.,pcount)
      ...
      if norbita gt 1L then begin
;
; find ACE/MLS coincidences inside anticyclones
;
a=findgen(6)*(2*!pi/5.)
usersym,2.*cos(a),2.*sin(a),/fill
xmcoin=[-99.] & ymcoin=[-99.]
xacoin=[-99.] & yacoin=[-99.]
o3acoin=fltarr(nleva)-99.
thacoin=fltarr(nleva)-99.
for ii=0L,norbita-1L do begin
    o3aprof=reform(o3ace(ii,*))
    if max(o3aprof) eq -9.90000e+07 then goto,jumpaprof
    xa=xace(ii) & ya=yace(ii)
    oplot,[xa,xa],[ya,ya],psym=8,symsize=2,color=10
print,'ACE profile ',ii,xa,ya
index=where(abs(xmdata-xa) le 5. and abs(ymdata-ya) le 5.)
if index(0) ne -1L then begin
if min(o3mark(index)) lt 10. then begin		; set to 0. to only look in anticyclones
   oplot,xmdata(index),ymdata(index),psym=8,color=0
;
; retain all coincidences inside anticyclones
;
   if xmcoin(0) ne -99. then begin
      xmcoin=[xmcoin,xmdata(index)]
      ymcoin=[ymcoin,ymdata(index)]
   endif
   if xmcoin(0) eq -99. then begin
      xmcoin=xmdata(index)
      ymcoin=ymdata(index)
   endif
;
; ACE coincident profiles inside anticyclones
;
   if xacoin(0) ne -99. then begin
      xacoin=[xacoin,xa]
      yacoin=[yacoin,ya]
      o3acoin=[o3acoin,o3ace(ii,*)]
      thacoin=[thacoin,thace(ii,*)]
   endif
   if xacoin(0) eq -99. then begin
      xacoin=xa
      yacoin=ya
      o3acoin=o3ace(ii,*)
      thacoin=thace(ii,*)
...
jumpaprof:
endfor	; loop over ACE profiles looking for MLS coincidences
;
; find ozone profiles corresponding to coincident x and y
;
index=where(xacoin ne -99.,nacoin)
index=where(xmcoin ne -99.,nmcoin)
if nmcoin gt 0L then begin
o3mcoin=fltarr(nmcoin,mlev)
thmcoin=fltarr(nmcoin,mlev)
for ii=0L,nmcoin-1L do begin
    xa=xmcoin(ii) & ya=ymcoin(ii)
    index=where(mlon eq xa and mlat eq ya)
if index(0) eq -1L then stop
if n_elements(index) gt 1L then stop
o3mcoin(ii,*)=mo3(*,index(0))
thmcoin(ii,*)=mtheta2(*,index(0))
endfor	; loop over MLS coincidences 
endif
;
; plot ACE at 1000 K
;
         ...
         index=where(abs(thace-rtheta) le 25. and o3ace gt 0. and yace2d gt 0.,hcount)
         if hcount gt 0L then begin
            xday=xace2d(index)
            yday=yace2d(index)
            thday=thace(index)
            ...
            for i=0,hcount-1 do $
                oplot,[xday(i),xday(i)],[yday(i),yday(i)],$
                      psym=8,color=mcolor*(o3day(i)-o3min)/(o3max-o3min),symsize=2
            a=findgen(6)*(2*!pi/5.)
            usersym,2.*cos(a),2.*sin(a)
            oplot,xday,yday,psym=8,color=lc,symsize=2
         endif
      endif
;
; extract MLS swath
...
;            oplot,mlon(kindex),mlat(kindex),psym=8,symsize=1.25,color=(float(i+1)/float(npt))*mcolor
             ...
             xyouts,xtmp(index(0)),5.,stime,/data,charsize=1.5,alignment=0.5,color=0,charthick=2
          endif
      endfor
...
;
; overplot swath location on polar plot
;
...
thswath=transpose(mtheta2(*,kindex))
...
omax=10.
    ...
    contour,o3swath,xswath,thswath,levels=level,/cell_fill,title='MLS Swath at '+stime+' UT',$
            ...
            yrange=[500.,1600.],charsize=1.5,color=0,charthick=2
...
for i=0L,mprof-1L,20 do begin
    plots,i,500
    plots,i,450,/data,/continue,color=0,thick=2
    xyouts,i,400.,xlabels(i),/data,charsize=1.25,alignment=0.5,color=0,charthick=2
    xyouts,i,300.,ylabels(i),/data,charsize=1.25,alignment=0.5,color=0,charthick=2
endfor
plots,mprof-1,500
plots,mprof-1,450,/data,/continue,color=0,thick=2
xyouts,mprof-1,400.,xlabels(mprof-1),/data,charsize=1.25,alignment=0.5,color=0,charthick=2
xyouts,mprof-1,300.,ylabels(mprof-1),/data,charsize=1.25,alignment=0.5,color=0,charthick=2
;
; draw arrows indicating ACE latitude
;
yaceavg=total(yace)/float(n_elements(yace))
for i=0L,mprof-2L do begin
    if latswath(i) lt yaceavg and latswath(i+1) gt yaceavg then begin
       plots,i,450
       plots,i,600,/data,/continue,color=icolmax*.9,thick=10
       plots,i-5,550
       plots,i,600,/data,/continue,color=icolmax*.9,thick=10
       plots,i+5,550
       plots,i,600,/data,/continue,color=icolmax*.9,thick=10
    endif
    if latswath(i) gt yaceavg and latswath(i+1) lt yaceavg then begin
       plots,i,450
       plots,i,600,/data,/continue,color=icolmax*.9,thick=10
       plots,i-5,550
       plots,i,600,/data,/continue,color=icolmax*.9,thick=10
...
;
; plot coincident profiles inside anticyclones
;
;o3acoin, nacoin, nleva, th
;o3mcoin, nmcoin, mlev, thmcoin
;
!type=12
index=where(o3acoin eq -9.90000e+07)
if index(0) ne -1L then o3acoin(index)=0./0.
set_viewport,.1,.45,.15,.5
plot,findgen(10),th,/noeras,xrange=[2.,8.],yrange=[500.,1600.],/nodata,color=0,charsize=1.5,$
     xtitle='Ozone (ppmv)',charthick=2,ytitle='Theta (K)'
loadct,0
if nmcoin gt 0L then begin
o3mavg=fltarr(mlev)
thmavg=fltarr(mlev)
for ii=0L,nmcoin-1L do begin
    oplot,o3mcoin(ii,*),thmcoin(ii,*),color=150,thick=2
    o3mprof=reform(o3mcoin(ii,*))
    thmprof=reform(thmcoin(ii,*))
    o3mavg=o3mavg+o3mprof
    thmavg=thmavg+thmprof
endfor
o3mavg=o3mavg/float(nmcoin)
;o3mavg=smooth(o3mavg,3)
thmavg=thmavg/float(nmcoin)
oplot,o3mavg,thmavg,color=0,thick=10
endif
loadct,38
if nacoin gt 0L then begin
o3aavg=fltarr(nleva)
thaavg=fltarr(nleva)
for ii=0L,nacoin-1L do begin
    oplot,o3acoin(ii,*),thacoin(ii,*),color=icolmax*.9,thick=3
    o3aprof=reform(o3acoin(ii,*))
    thaprof=reform(thacoin(ii,*))
    o3aavg=o3aavg+o3aprof
    thaavg=thaavg+thaprof
endfor
o3aavg=o3aavg/float(nacoin)
;o3aavg=smooth(o3aavg,3)
thaavg=thaavg/float(nacoin)
oplot,o3aavg,thaavg,color=icolmax*.9,thick=15
;
; interpolate MLS average profiles to ACE theta levels
o3mavgint=fltarr(nleva)
thmavgint=fltarr(nleva)
for ii=0L,nleva-1L do begin
    thlev=thaavg(ii)
    for kk=0L,mlev-2L do begin
        if thmavg(kk) lt thlev and thmavg(kk+1L) ge thlev then begin
           scale=(thmavg(kk+1L)-thlev)/(thmavg(kk+1L)-thmavg(kk))
           o3mavgint(ii)=o3mavg(kk+1L)-scale*(o3mavg(kk+1L)-o3mavg(kk))
           thmavgint(kk)=thmavg(kk+1L)-scale*(thmavg(kk+1L)-thmavg(kk))
           goto,jumplev
        endif
    endfor
    jumplev:
endfor
o3diff=100.*(o3aavg-o3mavgint)/ ((o3mavgint+o3aavg)/2.)
so3diff=strcompress(string(format='(f5.1)',o3diff),/remove_all)
for ii=0L,nleva-1L,2 do begin
    if so3diff(ii) ne 'NaN' and thaavg(ii) ge 500. and thaavg(ii) le 1550. then $
       xyouts,2.1,thaavg(ii),so3diff(ii),/data,charsize=1.75,charthick=2,color=0
endfor
xyouts,1.5,1610.,'100*(A-M)/AVG',/data,charsize=1.5,charthick=2,color=0
endif
xyouts,4.5,1610.,'MLS',charthick=2,charsize=2,color=0,/data
if nmcoin gt 0L then begin
   smcoin=strcompress(string(nmcoin),/remove_all)
   xyouts,5.75,1610.,'('+smcoin+')',charthick=2,charsize=2,color=0,/data
endif
xyouts,6.75,1610.,'ACE',charthick=2,charsize=2,color=icolmax*.9,/data
if nacoin gt 0L then begin
   sacoin=strcompress(string(nacoin),/remove_all)
   xyouts,8.,1610.,'('+sacoin+')',charthick=2,charsize=2,color=icolmax*.9,/data
endif
;   imin=omin
;   imax=omax
;   xmnb=0.45+.03
...
;   endfor
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim XZ_Ozone/xz_o3_ace+mls+polar_nh_'+lfile+'_'+stheta+'K_coins.ps -rotate -90 '+$
               ' XZ_Ozone/xz_o3_ace+mls+polar_nh_'+lfile+'_'+stheta+'K_coins.jpg'
         spawn,'/usr/bin/rm XZ_Ozone/xz_o3_ace+mls+polar_nh_'+lfile+'_'+stheta+'K_coins.ps'
      endif
      icount=icount+1L
      goto,jump
end

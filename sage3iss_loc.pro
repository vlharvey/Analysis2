...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='sage3iss_loc.ps'
   !p.thick=2
   !p.charthick=5
   !y.thick=2
   !x.thick=2
...
dirs3i='/atmos/aura3/data/SAGE_3ISS_data/Datfiles_SOSST/'
...
; SAGE III/ISS
;
    ex=findfile(dirs3i+'cat_sage3iss_v5.00.'+syear(iyear))
    if ex(0) ne '' then begin
       restore,dirs3i+'cat_sage3iss_v5.00.'+syear(iyear)
       oplot,fdoy,latitude,psym=8,color=col1(iyear)
    endif
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim sage3iss_loc.ps -rotate -90 sage3iss_loc.jpg'
endif
end

;
; plot timeseries of MERRA polar water at a given altitude
;
@stddat
@kgmt
@ckday
...
;polarh2o=smooth(polarh2o,7,/edge_truncate)
...
imin=6
imax=8.
ubarlev=reform(polarh2o(*,ialt))*1.e6
plot,dayno,ubarlev,color=col1(iyear),ytitle='Mean MLS H2O in MERRA Anticyclone',yrange=[imin,imax],charsize=1.5,thick=1,/nodata	;,xrange=[60.,180]
...
for iyear=0L,nyear-1L do begin
    ...
    if syear(iyear) eq '2005' or syear(iyear) eq '2007' or syear(iyear) eq '2011' then oplot,dayno,smooth(ubarlev,3,/Nan,/edge_truncate),color=col1(iyear),thick=4,linestyle=5
    if syear(iyear) ne '2005' and syear(iyear) ne '2007' and syear(iyear) ne '2011' then oplot,dayno,smooth(ubarlev,3,/Nan,/edge_truncate),color=col1(iyear),thick=4
endfor
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim timeseries_merra_polarh2o_'+salt+'.ps -rotate -90 '+$
                            'timeseries_merra_polarh2o_'+salt+'.jpg'
     endif

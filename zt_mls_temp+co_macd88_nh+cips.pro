...
; save temp, co, etc
;
save,file='Save_files/zt_mls_temp+co_'+yearlab+'_nh.sav',mlspolarco_zt,mlspolartp_zt,kday,altitude,sdate_all
quick:
restore,'Save_files/zt_mls_temp+co_'+yearlab+'_nh.sav'
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co_'+yearlab+'_nh+cips.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS Average Temperature > 70!uo!n N (K)',charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co_'+yearlab+'_nh+cips.ps -rotate -90 zt_mls_temp+co_'+yearlab+'_nh+cips.jpg'
    endif
end

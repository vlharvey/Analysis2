;
; save yearly ubar and vortex and anticyclone area
; MERRA version
;
@stddat
@kgmt
...
dirm='/atmos/aura6/data/MLS_data/Datfiles_Grid/MLS_grid_theta_'		; will need to archive anticyclone averaged water as N2O is not saved on theta
...
; --- Test for end condition and close windows.
    ...
    nth2=35
    h2o=fltarr(nr,nc,nth2)
    dum=findfile(dirm+sdate+'.nc3')
    if dum(0) ne '' then begin
       ncid=ncdf_open(dirm+sdate+'.nc3')
       ncdf_varget,ncid,12,h2o
       ncdf_close,ncid
       h2o=reform(h2o(*,*,5:nth2-1))
    endif
...
;      read,'Enter desired latitude ',rlat
       ...
       area1h=fltarr(kday,nth)
       polarh2o=fltarr(kday,nth)
       polarpv=fltarr(kday,nth)
...
;
      for k = 0L, nth - 1L do begin
          ...
          mark1new=transpose(marknew2(*,*,k))
          h2olev=transpose(h2o(*,*,k))
          pvlev=transpose(pv2(*,*,k))
          index=where(lat gt 0. and mark1 eq 1.0,nn)
          if index(0) ne -1L then area1(icount,k)=100.*total(area(index))/hem_area
          index=where(lat gt 0. and mark1new lt 0.0,nn)
          if index(0) ne -1L then area1h(icount,k)=100.*total(area(index))/hem_area
          if index(0) ne -1L then polarh2o(icount,k)=mean(h2olev(index))
          if index(0) ne -1L then polarpv(icount,k)=mean(pvlev(index))
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   set_plot,'ps'
   device,/color,/landscape,bits=8,filename='zt_merra_ubar+area_'+syear(iyear)+'_'+slat+'.ps'
...
contour,ubar,dayno,pressure,/ylog,levels=level,/cell_fill,c_color=col1,color=0,title=syear(iyear),$
            ytitle='Pressure (hPa)',yrange=[100.,0.1],charsize=1.5
...
loadct,0
contour,smooth(area1h,3),dayno,pressure,/ylog,levels=5+5.*findgen(7),/overplot,/follow,c_color=1+indgen(7)*mcolor/7.,thick=5
loadct,39
...
;!type=2^2+2^3+2^6
;plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MERRA Ubar at '+slat+' N (m/s)',/noeras
;ybox=[0,10,10,0,0]
;x1=imin
...
;x1=x1+dx
;endfor
save,file='merra_ubar+area_'+syear(iyear)+'_'+slat+'.sav',ubar,dayno,pressure,altitude,area1,area1h,polarh2o,polarpv

; Close PostScript file and return control to X-windows
     if setplot ne 'ps' then stop
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim zt_merra_ubar+area_'+syear(iyear)+'_'+slat+'.ps -rotate -90 '+$
                            'zt_merra_ubar+area_'+syear(iyear)+'_'+slat+'.jpg'
     endif

...
for j=28L,nlat-1L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       device,font_size=9
       device,/landscape,bits=8,filename='scatter_mls+geos5_tbar_'+slat+'.ps'
       device,/color
...
contour,tzlat,1.+findgen(kday),altitude_mls,/noeras,xrange=[1.,kday],yrange=[20.,68.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='MLS',/fill,c_color=col1,$
...
contour,tzlat,1.+findgen(kday),altitude_mls,levels=[250.],color=0,/follow,/overplot,thick=3
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
ymxb=ymnb+cbarydel
...
loadct,39
;
; scatter plot of MLS T vs Tdiff
;
...
plot,tzlat1(index),pdiff(index),xrange=[200.,270.],yrange=[-15.,5.],psym=8,$
     charsize=1.5,xtitle='MLS Temperature',color=0,ytitle='T Difference',$
     title='20-35 km'
...
set_viewport,xmn,xmx,ymn,ymx
plot,tzlat1(index),pdiff(index),xrange=[200.,270.],yrange=[-15.,5.],psym=8,$
     charsize=1.5,xtitle='MLS Temperature',color=0,$
     title='35-45 km'
;oplot,tzlatg1(index),pdiff(index),psym=8,color=90
for iday=0L,kday-1 do begin
    tprof=reform(tzlat1(iday,*))
...
jump:
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim scatter_mls+geos5_tbar_'+slat+'.ps -rotate -90 scatter_mls+geos_tbar_'+slat+'.jpg'
    endif

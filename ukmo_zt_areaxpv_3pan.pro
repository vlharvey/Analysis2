...
xyouts,0.35,.95,'MetO Analyses '+y1+'-'+y2,/normal,charsize=2
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
ymx=yorig(0)+ylen
...
plot,[1,kday,kday,1,1],[500.,500.,2000.,2000.,500.],min_value=0.,$
      xrange=[1,kday],yrange=[500.,2000.],/nodata,charsize=1,$
      ytitle='Theta (K)',title='Vortex Area (millions of sq. km)',xtickname=[' ',' '],xticks=1
...
for i=0,nxtick-1 do begin
    ...
    xyouts,kindex(i)+1,325.,xlab,/data,alignment=0.5,charsize=2
endfor
nlvls=21
col1=1+indgen(nlvls)*icolmax/nlvls
index=where(area_zt eq 0.)
if index(0) ne -1 then area_zt(index)=-9999.
index=where(area_zt gt 0.)
imin=min(area_zt(index))
imax=max(area_zt(index))
level=imin+((imax-imin)/(nlvls-1))*findgen(nlvls)
contour,area_zt,1.+findgen(kday),th,levels=level,/fill,$
        /cell_fill,/overplot,c_color=col1,min_value=0.
contour,area_zt,1.+findgen(kday),th,levels=level,c_color=0,$
...
plot,[1,kday,kday,1,1],[500.,500.,2000.,2000.,500.],min_value=0.,$
      xrange=[1,kday],yrange=[500.,2000.],/nodata,charsize=1,$
      ytitle='Theta (K)',title='Mean MPV (PVU)',xtickname=[' ',' '],xticks=1
kindex=where(strmid(sfile,3,2) eq '15',nxtick)
xmon=long(strmid(sfile(kindex),0,2))
for i=0,nxtick-1 do begin
    xlab=smon(xmon(i)-1)
...
index=where(mpv_zt eq 0. or mpv_zt gt 1.e8)
if index(0) ne -1 then mpv_zt(index)=-9999.
index=where(mpv_zt ne -9999.)
imin=min(mpv_zt(index))
imax=max(mpv_zt(index))
level=imin+((imax-imin)/(nlvls-1))*findgen(nlvls)
contour,mpv_zt,1.+findgen(kday),th,levels=level,/fill,$
        /cell_fill,/overplot,c_color=col1,min_value=0.
contour,mpv_zt,1.+findgen(kday),th,levels=level,c_color=0,$
        /follow,/overplot,min_value=0.
...
plot,[1,kday,kday,1,1],[500.,500.,2000.,2000.,500.],min_value=0.,$
      xrange=[1,kday],yrange=[500.,2000.],/nodata,charsize=1,$
      ytitle='Theta (K)',title='Vortex Area x Mean MPV',xtickname=[' ',' '],xticks=1
kindex=where(strmid(sfile,3,2) eq '15',nxtick)
xmon=long(strmid(sfile(kindex),0,2))
for i=0,nxtick-1 do begin
    xlab=smon(xmon(i)-1)
...
if index(0) ne -1 then prod(index)=-9999.
index=where(prod ne -9999.)
imin=min(prod(index))
imax=max(prod(index))
level=imin+((imax-imin)/(nlvls-1))*findgen(nlvls)

...
;
      for kk=0,nth2-1L do begin
          ...
          print,slev
          kindex=where(mlat2 ge 0. and abs(mtheta-rlev) le 50. and mh2o ne -99.,mprof)
...
save,file='yt_merra_h2o+mark_'+yearlab+'.sav',alat,kday,sdate_all,yth2o,ytmark,ytspeed,slev,th,zindex,nth2

quick:
restore,'yt_merra_h2o+mark_'+yearlab+'.sav'
syear=strmid(sdate_all,0,4)
smon=strmid(sdate_all,4,2)
sday=strmid(sdate_all,6,2)
xindex=where(sday eq '01' or sday eq '15',nxticks)
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='yt_merra_h2o+mark_'+yearlab+'_'+slev+'.ps'
         device,/color
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim yt_merra_h2o+mark_'+yearlab+'_'+slev+'.ps -rotate -90 '+$
         'yt_merra_h2o+mark_'+yearlab+'_'+slev+'.jpg'
endif

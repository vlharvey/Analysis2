...
;
index=where(tbar_zt_saber le 0.)
...
altitude_geos5=altitude
TBAR_ZT_geos5=TBAR_ZT
SDATE_ALL_geos5=SDATE_ALL
index=where(tbar_zt_geos5 le 0.)
if index(0) ne -1L then begin
...
for j=29L,34L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       device,font_size=9
       device,/landscape,bits=8,filename='zt_mls+saber_tbar_'+slat+'.ps'
       device,/color
...
contour,tzlat,1.+findgen(kday),altitude_saber,/noeras,xrange=[1.,kday],yrange=[20.,60.],$
      charsize=1.5,color=0,title='SABER',/fill,c_color=col1,$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-99.
index=where(level mod 10 eq 0.)
contour,tzlat,1.+findgen(kday),altitude_saber,levels=level(index),color=0,/follow,/overplot,c_labels=0*index,min_value=-99.
imin=min(level)
imax=max(level)
...
contour,tzlatg,1.+findgen(kday),altitude_geos5,/noeras,xrange=[1.,kday],yrange=[20.,60.],$
      charsize=1.5,color=0,title='MLS',/fill,c_color=col1,ytitle='Altitude (km)',$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-99.
index=where(level mod 10 eq 0.)
contour,tzlatg,1.+findgen(kday),altitude_geos5,levels=level(index),color=0,/follow,/overplot,c_labels=0*index,min_value=-99.
imin=min(level)
imax=max(level)
...
tvlct,c1,c2,c3
col2=1+indgen(11)
zindex=where(altitude_saber le max(altitude_geos5))
tzlat=reform(tzlat(*,zindex))
pdiff=-99.+0.*tzlatg
...
contour,pdiff,1.+findgen(kday),altitude,xrange=[1.,kday],yrange=[20.,60.],$
        ...
        title='MLS - SABER',c_color=col2,color=0,min_value=-99.,$
...
index=where(tzlat lt 0.)
if index(0) ne -1L then tzlat(index)=-9999.
;contour,tzlat,1.+findgen(kday),altitude,/overplot,levels=[230.],color=0,thick=6,min_value=-9999.
good=where(reform(tzlat(*,40)) gt 0.)
contour,tzlat(good,*),1.+good,altitude,/overplot,levels=[230.],color=0,thick=6,min_value=-9999.
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
ymxb=ymnb+cbarydel
...
jump:
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls+saber_tbar_'+slat+'.ps -rotate -90 zt_mls+saber_tbar_'+slat+'.jpg'
    endif

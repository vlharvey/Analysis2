...
;
lstmn=9L & lstdy=18L & lstyr=4L
ledmn=9L & leddy=1L & ledyr=7L
lstday=0L & ledday=0L 
...
;
      spawn,'ls '+dir+'MLS-Aura_L2GP-BrO_v02-2*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',brofiles
      spawn,'ls '+dir+'MLS-Aura_L2GP-CO_v02-2*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',cofiles
      spawn,'ls '+dir+'MLS-Aura_L2GP-ClO_v02-2*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',clofiles
      ...
      spawn,'ls '+dir+'MLS-Aura_L2GP-HCl_v02-2*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',hclfiles
      spawn,'ls '+dir+'MLS-Aura_L2GP-HNO3_v02-2*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',hno3files
      spawn,'ls '+dir+'MLS-Aura_L2GP-N2O_v02-2*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',n2ofiles
...
;
      ...
      if h2obad(0) ne -1L then h2omask(h2obad,*)=-99.
      hno3bad=where(hno3quality lt 0.4)			; do not use if quality < 0.4
      if hno3bad(0) ne -1L then hno3mask(hno3bad,*)=-99.
...
;     if h2obad(0) ne -1L then h2omask(h2obad,*)=-99.
      hno3bad=where(hno3convergence gt 1.8)                 ; do not use if convergence > 1.8
      if hno3bad(0) ne -1L then hno3mask(hno3bad,*)=-99.
...
;
      ...
      if index(0) ne -1L then begin
         ...
         ltime=ltime(index)
         bromls=bromls(index,*)
         broprecision=broprecision(index,*)
         bromask=bromask(index,*)
         o3mls=o3mls(index,*)
         o3precision=o3precision(index,*)
         o3mask=o3mask(index,*)
         gpmls=gpmls(index,*)
         gpprecision=gpprecision(index,*)
         gpmask=gpmask(index,*)
         comls=comls(index,*)
         coprecision=coprecision(index,*)
         comask=comask(index,*)
         hclmls=hclmls(index,*)
         hclprecision=hclprecision(index,*)
         hclmask=hclmask(index,*)
         clomls=clomls(index,*)
         cloprecision=cloprecision(index,*)
         clomask=clomask(index,*)
         h2omls=h2omls(index,*)
         h2oprecision=h2oprecision(index,*)
         h2omask=h2omask(index,*)
         hno3mls=hno3mls(index,*)
         hno3precision=hno3precision(index,*)
         hno3mask=hno3mask(index,*)
         n2omls=n2omls(index,*)
         n2oprecision=n2oprecision(index,*)
         n2omask=n2omask(index,*)
         tpmls=tpmls(index,*)
         tpprecision=tpprecision(index,*)
         tpmask=tpmask(index,*)
      endif
;
; convert errors to percent
;
      broerr=-99.+0.*broprecision
      index=where(bromls ne -99. and broprecision ne -99.)
      if index(0) ne -1L then broerr(index)=100.*abs(broprecision(index))/abs(bromls(index))
      o3err=-99.+0.*o3precision
      index=where(o3mls ne -99. and o3precision ne -99.)
      if index(0) ne -1L then o3err(index)=100.*abs(o3precision(index))/abs(o3mls(index))
      gperr=-99.+0.*gpprecision
      index=where(gpmls ne -99. and gpprecision ne -99.)
      if index(0) ne -1L then gperr(index)=100.*abs(gpprecision(index))/abs(gpmls(index))
      coerr=-99.+0.*coprecision
      index=where(comls ne -99. and coprecision ne -99.)
      if index(0) ne -1L then coerr(index)=100.*abs(coprecision(index))/abs(comls(index))
      hclerr=-99.+0.*hclprecision
      index=where(hclmls ne -99. and hclprecision ne -99.)
      if index(0) ne -1L then hclerr(index)=100.*abs(hclprecision(index))/abs(hclmls(index))
      cloerr=-99.+0.*cloprecision
      index=where(clomls ne -99. and cloprecision ne -99.)
      if index(0) ne -1L then cloerr(index)=100.*abs(cloprecision(index))/abs(clomls(index))
      h2oerr=-99.+0.*h2oprecision
      index=where(h2omls ne -99. and h2oprecision ne -99.)
      if index(0) ne -1L then h2oerr(index)=100.*abs(h2oprecision(index))/abs(h2omls(index))
      hno3err=-99.+0.*hno3precision
      index=where(hno3mls ne -99. and hno3precision ne -99.)
      if index(0) ne -1L then hno3err(index)=100.*abs(hno3precision(index))/abs(hno3mls(index))
      n2oerr=-99.+0.*n2oprecision
      index=where(n2omls ne -99. and n2oprecision ne -99.)
      if index(0) ne -1L then n2oerr(index)=100.*abs(n2oprecision(index))/abs(n2omls(index))
      tperr=-99.+0.*tpprecision
      index=where(tpmls ne -99. and tpprecision ne -99.)
      if index(0) ne -1L then tperr(index)=100.*abs(tpprecision(index))/abs(tpmls(index))
;
; save daily file containing everything
;
      comment=strarr(3)
...
;
      rawfile=odir+'raw_isobaric_mls_'+sver+'_'+sdate+'.sav'
      save,file=rawfile,yyyymmdd,uttime,longitude,latitude,pmls,pmls2,fdoy,lday,ltime,$
           bromls,broerr,bromask,o3mls,o3err,o3mask,gpmls,gperr,gpmask,comls,coerr,comask,$
           hclmls,hclerr,hclmask,clomls,cloerr,clomask,h2omls,h2oerr,h2omask,hno3mls,hno3err,$
           hno3mask,n2omls,n2oerr,n2omask,tpmls,tperr,tpmask,comment
goto,jump
end

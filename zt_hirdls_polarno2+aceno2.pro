...
;
      if kcount eq 0L then begin
         hirpolarno2_zt=fltarr(kday,nlv)
...
;
      ...
      hirpolarno2_zt(icount,*)=polarno2
;
; check 
;
erase
...
plot,polarno2,altitude,color=0,/noerase,title=sdate+' NO!l2!n',xrange=[1.e-10,1.e-7],/xlog,thick=3
skip:
;
...
plotit:
index=where(hirpolarno2_zt ge 1.e-7 or hirpolarno2_zt eq 0.)
if index(0) ne -1L then hirpolarno2_zt(index)=0./0.
hirpolarno2_zt=hirpolarno2_zt*1.e9
hirpolarno2_zt=smooth(hirpolarno2_zt,7,/NaN,/edge_truncate)
index=where(finite(hirpolarno2_zt) ne 1)
hirpolarno2_zt(index)=0.
index=where(hirpolarno2_zt gt 1000.)
if index(0) ne -1L then hirpolarno2_zt(index)=0.
...
save,file='zt_hirdls_polarno2+aceno2_2007.sav',hirpolarno2_zt,$
     acepolarno2_zt,kday,altitude,sdate_all
quick:
restore,'zt_hirdls_polarno2+aceno2_2007.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_hirdls_polarno2+aceno2_'+syear(0)+'.ps'
   device,/color
...
contour,hirpolarno2_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[15.,57.],$
...
contour,hirpolarno2_zt,1.+findgen(kday),altitude,levels=level,color=mcolor,/follow,/overplot,c_labels=1+fltarr(nlvls)
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_hirdls_polarno2+aceno2_'+syear(0)+'.ps -rotate -90 zt_hirdls_polarno2+aceno2_'+syear(0)+'.jpg'
    endif
end

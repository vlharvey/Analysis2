;
; Southern hemisphere
; plot latitude of vortex max wind speed as a function of altitude and time
;
@stddat
@kgmt
@ckday
...
plotit:
    ...
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='ukmo_lat_wmax_'+y1+'_sh.ps'
       device,/color
    ...
    nlvls=10
    level=reverse(-10.*findgen(nlvls))
    col1=reverse(1+indgen(nlvls)*icolmax/nlvls)
    contour,latzt,1.+findgen(ndays),th,levels=level,/fill,$
            /cell_fill,/overplot,c_color=col1
    contour,latzt,1.+findgen(ndays),th,levels=level,c_color=0,$
            /follow,/overplot
    imin=min(level)
    imax=max(level)
    ...
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],$
         xtitle='Latitude of Maximum Vortex Wind Speed (m/s)',charsize=2
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim ukmo_lat_wmax_'+y1+'_sh.ps -rotate -90 ukmo_lat_wmax_'+y1+'_sh.jpg'
    endif

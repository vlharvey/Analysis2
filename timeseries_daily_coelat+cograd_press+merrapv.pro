...
restore,ifiles(0)       ;'daily_mls_coelatedge+merra_pvelatedge_'+syear(iyear)+smon(imon)+'_2.15443hPa.sav'     ;,elatedge_time,pvelatedge_time,nashedge_time,sdate_time,spress,sth
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_daily_coelat+cograd_press_merrapv_'+spress+'.ps'
   !p.charsize=1.2
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
xlabs=xlabs0+xlabs1
plot,findgen(n_elements(sdate_time_all)),elatedge_time_all,color=0,/noeras,charsize=1.5,ytitle='Elat Vortex Edge',yrange=[20,90],title='DJF '+spress,xtickname=xlabs,xtickv=xindex,xticks=nxticks-1,psym=8,symsize=2
loadct,0
oplot,findgen(n_elements(sdate_time_all)),LOWLAT_ELATEDGE_TIME_ALL,color=150,psym=8
loadct,39
oplot,findgen(n_elements(sdate_time_all)),pvelatedge_time_all,color=mcolor*.9,psym=8,symsize=1.25
oplot,findgen(n_elements(sdate_time_all)),nashedge_time_all,color=mcolor*.3,psym=8,symsize=1
xyouts,xmn,0.54,'CO Elat',/normal,color=0,charsize=2,charthick=2
xyouts,xmn,0.5,'PV Elat',/normal,color=mcolor*.9,charsize=2,charthick=2
xyouts,xmn,0.46,'Nash',/normal,color=mcolor*.3,charsize=2,charthick=2
;
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim timeseries_daily_coelat+cograd_press_merrapv_'+spress+'.ps -rotate -90 '+$
                       'timeseries_daily_coelat+cograd_press_merrapv_'+spress+'.jpg'
;  spawn,'rm -f timeseries_daily_coelat+cograd_press_merrapv_'+spress+'.ps'
endif
end

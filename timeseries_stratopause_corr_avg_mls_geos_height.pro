...
;
geost=reform(geos5heights(iday,*,*))
mlst=reform(mlsheights(iday,*,*))
sabert=reform(saberheights(iday,*,*))
...
; save file
;
restore,'timeseries_stratopause_corr_avg_mls_geos_height.sav'
;save,file='timeseries_stratopause_corr_avg_mls_geos_height.sav',nday,dates,nc,nr,doy,$
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_stratopause_corr_avg_mls_geos_height.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
!type=2^2+2^3
xyouts,.13,.95,'Stratopause Z: Avg Corr 2004-2009',/normal,color=0,charsize=1.5
imin=-1.
imax=1.
...
oplot,1.+findgen(mday),geos_mls_anticorr_nh_avg,psym=8,color=0.9*mcolor,symsize=0.75
oplot,1.+findgen(mday),geos_mls_ambcorr_nh_avg,psym=8,color=0,symsize=0.3
xyouts,130.,-0.5,'Vortex',/data,color=mcolor*.2
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim timeseries_stratopause_corr_avg_mls_geos_height.ps -rotate -90 timeseries_stratopause_corr_avg_mls_geos_height.jpg'
endif
end

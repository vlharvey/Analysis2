;
; plot time-altitude section of vortex carbon monixide
;
@fillit
@smoothit
...
;oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_co_spring_'+syear(iyear)+'.ps'
       device,/color
...
;kday=long(max(fdoy))	; uncomment to "zoom" in on partial year
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,c_color=col1,/cell_fill,levels=level,$
         ...
         title=syear(iyear)+' ACE Arctic Carbon Monoxide',min_value=0.
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_co_spring_'+syear(iyear)+'.ps -rotate -90 zt_ace_co_spring_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

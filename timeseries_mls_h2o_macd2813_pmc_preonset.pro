...
; save temp
;
save,file='timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_preonset.sav',mlspolarh2o_xt,kday,sdate_all,mlspolarh2o_xt_min,mlspolarh2o_xt_max,doy_all
quick:
yearlab='2007-2018'
slat=slt
salt='83'
salt2='30'
restore,'timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_preonset.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_preonset.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
plot,doy_all(*,0),smooth(mlspolarh2o_xt(*,0),3,/nan,/edge_truncate),/noeras,xrange=[min(doy_all(*,0)),max(doy_all(*,0))],yrange=[0.,6.],ytitle='MLS H2O (ppmv)',$
...
for nn=0,nyear-1L do begin
    ...
    oplot,[start_doy(nn),start_doy(nn)],[0,6],color=mcolor*(nn/float(nyear)),thick=5,linestyle=5
endfor
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_preonset.ps -rotate -90 timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_preonset.png'
endif

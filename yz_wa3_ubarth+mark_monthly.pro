...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='yz_wa3_ubarth+mark_'+month(lstmn-1)+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
save,file='yz_wa3_ubarth+mark_'+month(lstmn-1)+'.sav',alat,th,tzm,uzm,markzm
;
...
contour,uzm,alat,th,/noeras,xrange=[-90.,90.],yrange=[400.,3000.],color=0,$
      ytitle='Potential Temperature (K)',xticks=6,/fill,c_color=col1,levels=level,min_value=-9999.,$
      xtitle='Latitude',title='WACCM-3 '+month(lstmn-1)
...
contour,markzm,alat,th,levels=[0.1,0.5,0.9],color=0,/follow,/overplot,thick=15,c_labels=1+0*findgen(3)
imin=min(level)
imax=max(level)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='Ubar (m/s)'
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_wa3_ubarth+mark_'+month(lstmn-1)+'.ps -rotate -90 yz_wa3_ubarth+mark_'+month(lstmn-1)+'.jpg'
endif

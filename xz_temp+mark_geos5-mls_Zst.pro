...
;         for j=0L,nlat-1L do begin
      ...
      if setplot eq 'ps' then begin
         ...
         device,/landscape,bits=8,$
                 filename='xz_temp+mark_geos5-mls_'+slat+'_'+sdate+'.ps'
         device,/color
...
contour,geost1,lonbin,altitude,levels=level,c_color=col1,thick=1,/cell_fill,/noeras,yrange=[20,60],$
        xticks=3,xtickname=['0','90','180','270'],xtickv=[0.,90.,180.,270.],$
...
contour,geosmark1,lonbin,altitude,/overplot,levels=[-0.1],/follow,color=mcolor,c_labels=0*level,thick=5
loadct,0
oplot,mlslon,mheight,color=150,psym=8,symsize=2
loadct,39
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
...
loadct,0
oplot,geos5lon,gheight,psym=8,color=100
oplot,mlslon,mheight,psym=8,color=150,symsize=2
restore,'c11_rb.tbl'
tvlct,c1,c2,c3
col2=1+indgen(11)
imin=min(level)

...
; change instances of instrument number 5 to instrument number 39 reserved for SNOE
;
...
ACE_FTS_period = [19980310,20031230] 		; SNOE data range

ace_obs_path='/atmos/aura6/data/SNOE_data/Datfiles_Sathist/'
...
;open NetCDF file for sequential writing
  ncid = ncdf_create(output_path+'sathist_39_SNOE_v2_'+start_date+'-'+end_date+'_c'+today()+'.nc',/clobber)
  ncdf_attput, ncid, 'Description', 'Satellite profile positions and times from '+start_date+' through '+end_date, /global
  ncdf_attput, ncid, 'Author', 'V. Lynn Harvey, file was created on '+today()+' using satellite_locations_jmp_snoe_c20130529_update.pro', /global
  ncdf_attput, ncid, 'Comment', '(39) SNOE geolocations in snoe_mission_profile_locs.sav from Scott Bailey were used, '+str_ace_fts_period+'; ', /global
  ncdf_attput, ncid, 'prof_num', '(39) SNOE: prof_num ranges from 1 to nprof each day ', /global
  ncdf_attput, ncid, 'orbit_num', '(39) SNOE: Orbit Number is the original SNOE_ID number and uniquely identifies profiles ', /global
  ...
  printf,2,'date  nprofs_orig nprofs_incl SNOE'
;
...
;   seconds_since_19930101=86400*(current_julday-julday(1,1,1993)) ;for MLS
    ...
    ;read entire SNOE dataset
    file=file_search(ace_obs_path+'SNOE_v2_sathist_1998-2003.sav',count=nfiles)
...
ace_orig_orbit_num=ORBIT_NUMBER
    ...
    if ace_nprofs ne 0 then begin
      instrument=[instrument,replicate(39,ace_nprofs)]			; 39=SNOE
...
if max(daytime) gt 86399 then stop
    ...
    ace_incl_nprofs = total(instrument eq 39,/int)

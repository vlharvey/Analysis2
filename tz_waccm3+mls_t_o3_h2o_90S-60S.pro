;
; plot altitude-time section of SH polar (90S-60S) T, O3, and water vapor
;
loadct,39
mcolor=byte(!p.color)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='zt_waccm3+mls_90S-60S_t_o3_h2o.ps'
       device,/color
...
;
    loadct,39
    erase
    xyouts,.45,.95,'90S-60S',/normal,charsize=2,charthick=2,color=0
    !type=2^2+2^3
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
...
;
    ...
    contour,h2ozt_waccm,1.+findgen(nmonth),th,levels=h2olevel,c_color=col1,/noeras,/cell_fill,$
            title='WACCM H!l2!nO',color=0,yrange=[400.,2000.]
...
;
    ...
    contour,h2odiff,1.+findgen(nmonth),th,levels=dlevel,c_color=col2,/noeras,/cell_fill,$
            title='(WACCM-MLS)/MLS',color=0,yrange=[400.,2000.]
    index=where(dlevel gt 0.)
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim zt_waccm3+mls_90S-60S_t_o3_h2o.ps -rotate -90 zt_waccm3+mls_90S-60S_t_o3_h2o.jpg'
;      spawn,'/usr/bin/rm zt_waccm3+mls_90S-60S_t_o3_h2o.ps'
    endif

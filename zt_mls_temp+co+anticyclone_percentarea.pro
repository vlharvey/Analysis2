...
; save temp, co, etc
;
save,file='zt_mls_temp+co+anticylone_percentarea_'+yearlab+'.sav',mlspolarco_zt,mlspolartp_zt,kday,$
     varea_nh_zt_alt,aarea_nh_zt_alt,altitude,sdate_all
quick:
restore,'zt_mls_temp+co+anticylone_percentarea_'+yearlab+'.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co+anticylone_percentarea_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
loadct,0
contour,aarea_nh_zt_alt,1.+findgen(kday),altitude,levels=[10],color=mcolor*.7,/follow,/overplot,thick=15
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co+anticylone_percentarea_'+yearlab+'.ps -rotate -90 zt_mls_temp+co+anticylone_percentarea_'+yearlab+'.jpg'
    endif
end

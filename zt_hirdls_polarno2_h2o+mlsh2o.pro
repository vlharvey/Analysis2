...
; Superimpose MLS H2O contours
; Superimpoase HIRDLS H2O contours
;
@stddat
@kgmt
@ckday
...
;
      ...
      for ii=0L,n_elements(id)-1L do begin
          if latitude(ii) ge 60. then begin
             ...
             good=where(n2o_prof ne -99. and n2o_prof lt 1.e-5,ngood)
             if good(0) ne -1L then begin
...
plot,polarn2o,altitude,color=0,/noerase,title='H!l2!nO',xrange=[5.e-7,1.e-5],/xlog
;
...
save,file='zt_hirdls_polarno2_h2o+mlsh2o_2006.sav',polart_zt,polarno2_zt,polarn2o_zt,$
     mlspolarn2o_zt,kday,altitude,sdate_all
quick:
restore,'zt_hirdls_polarno2_h2o+mlsh2o_2006.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_hirdls_polarno2_h2o+mlsh2o_'+syear(0)+'.ps'
   device,/color
   device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
endif
;
; plot Arctic mean NO2, H2O, MLS H2O
;
erase
xmn=xorig(0)
xmx=xorig(0)+xlen
...
contour,polarno2_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[25.,57.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='HIRDLS Avg NO!l2!n and H!l2!nO and MLS H!l2!nO > 60 N',/fill,c_color=col1,$
...
contour,mlspolarn2o_zt,1.+findgen(kday),altitude,levels=1+findgen(5),color=0,/follow,/overplot,$
        min_value=0.,c_labels=[1,1,1,1,1],thick=5
contour,polarn2o_zt,1.+findgen(kday),altitude,levels=1+findgen(5),color=mcolor,/follow,/overplot,$
        min_value=0.,c_labels=[1,1,1,1,1],thick=5
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_hirdls_polarno2_h2o+mlsh2o_'+syear(0)+'.ps -rotate -90 zt_hirdls_polarno2_h2o+mlsh2o_'+syear(0)+'.jpg'
    endif
end

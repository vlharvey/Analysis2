;
; average at terminator, zangle between 85 and 95 degrees
; no2
...
;
      ...
      for ii=0L,n_elements(id)-1L do begin
          if latitude(ii) ge 60. and zangle(ii) le 95. and zangle(ii) ge 85. then begin
...
xyouts,1.2e-10,57.,'85 > zangle > 95',color=0,charthick=3,/data,charsize=2
oplot,polarno2_day,altitude,color=0,thick=3
skip:
;
...
save,file='zt_hirdls_polarno2+aceno2_2007_term.sav',hirpolarno2_zt_day,$
     acepolarno2_zt,kday,altitude,sdate_all
quick:
restore,'zt_hirdls_polarno2+aceno2_2007_term.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_hirdls_polarno2+aceno2_'+syear(0)+'_term.ps'
   device,/color
...
contour,hirpolarno2_zt_day,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[15.,57.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='Terminator HIRDLS (color); ACE (white)',/fill,c_color=col1,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_hirdls_polarno2+aceno2_'+syear(0)+'_term.ps -rotate -90 zt_hirdls_polarno2+aceno2_'+syear(0)+'_term.jpg'
    endif
end

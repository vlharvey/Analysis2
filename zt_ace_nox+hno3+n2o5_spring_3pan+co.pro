...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='zt_ace_nox+hno3+n2o5_spring_3pan+co.ps'
   device,/color
...
;
    ...
    restore,dira+'n2o5_ace_v2.2.'+syear(iyear)
    n2o5mix=mix & n2o5mask=mask
    restore,dira+'hno3_ace_v2.2.'+syear(iyear)
    hno3mix=mix & hno3mask=mask

;help,no2mix,dmpdate
;
; NOx is NO + NO2 + HNO3 + 2*N2O5 and take only good values
;
    noxmix=0.*nomix
    index=where(nomask ne -99. and no2mask ne -99. and hno3mask ne -99. and n2o5mask ne -99.)		; all good
    if index(0) ne -1L then noxmix(index)=nomix(index)+no2mix(index)+hno3mix(index)+2.*n2o5mix(index)
print,'NO NO2 HNO3 N2O5',n_elements(index)
    index=where(noxmix eq 0. and nomask ne -99. and no2mask ne -99. and hno3mask ne -99.)		; good NO,NO2,HNO3
    if index(0) ne -1L then noxmix(index)=nomix(index)+no2mix(index)+hno3mix(index)
print,'NO NO2 HNO3 ',n_elements(index)
    index=where(noxmix eq 0. and nomask ne -99. and no2mask ne -99. and n2o5mask ne -99.)		; good NO,NO2,N2O5
    if index(0) ne -1L then noxmix(index)=nomix(index)+no2mix(index)+2.*n2o5mix(index)
print,'NO NO2 N2O5 ',n_elements(index)
    index=where(noxmix eq 0. and nomask ne -99. and no2mask ne -99.)					; good NO,NO2
    noxmix(index)=nomix(index)+no2mix(index)
print,'NO NO2 ',n_elements(index)
    index=where(noxmix eq 0. and nomask ne -99. and no2mask eq -99.)					; good NO
    noxmix(index)=nomix(index)
print,'NO ',n_elements(index)
    index=where(noxmix eq 0. and nomask eq -99. and no2mask ne -99.)					; good NO2
    noxmix(index)=no2mix(index)
print,'NO2 ',n_elements(index)
;
...
kday=91
    ...
    contour,oco,1.+findgen(nday),altitude,/noeras,color=mcolor,/follow,levels=colevel,/overplot,$
            c_labels=1L+0*colevel,min_value=0.,thick=5,c_charsize=2
...
oplot,fdoy(index),latitude(index),psym=1,color=0
loadct,38
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_ace_nox+hno3+n2o5_spring_3pan+co.ps -rotate -90 zt_ace_nox+hno3+n2o5_spring_3pan+co.jpg'
endif
end

...
;  !p.font=0
   !p.charthick=3
   device,font_size=8
   device,/landscape,bits=8,filename='zt_ace_nox_nh_spring_6pan+co.ps'
   device,/color
...
if index(0) ne -1L then oco(index)=0./0.
    contour,onox,1.+findgen(nday),altitude_save,/noeras,c_color=col1,/cell_fill,levels=level,$
         yrange=[30.,90.],xrange=[1.,kday],xticks=3L,xtickname=xlab,$
         charsize=2,ytitle='Altitude (km)',color=0,min_value=0.,charthick=2,yminor=1
    ...
    xyouts,5.,35.,syear(iyear),/data,color=0,charsize=4,charthick=8
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_ace_nox_nh_spring_6pan+co.ps -rotate -90 zt_ace_nox_nh_spring_6pan+co.jpg'
endif
end

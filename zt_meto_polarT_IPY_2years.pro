...
nxdim=1000
nydim=750
xorig=0.025+[0.05,0.275,0.5,0.725,0.05,0.275,0.5,0.725]
yorig=[0.55,0.55,0.55,0.55,0.15,0.15,0.15,0.15]
xlen=0.2
ylen=0.3
cbaryoff=0.06
cbarydel=0.01
...
ledmn=4L & leddy=30L & ledyr=12L
lstday=0L & ledday=0L
;read,' Enter starting date (month, day, year) ',lstmn,lstdy,lstyr
;read,' Enter ending date   (month, day, year) ',ledmn,leddy,ledyr
lstmn=9L & lstdy=1L & lstyr=12L
ledmn=4L & leddy=30L & ledyr=13L
...
;
; restore 2007-2008
; sdate_all,th,temp_pf_zt,temp_al_zt,temp_eu_zt,$
;     temp_so_zt,u_pf_zt,u_al_zt,u_eu_zt,u_so_zt
;
restore,'ZT_2007-2008.sav'
nth=n_elements(th)
sdate_all_2007=sdate_all
temp_pf_zt_2007=temp_pf_zt
temp_al_zt_2007=temp_al_zt
temp_eu_zt_2007=temp_eu_zt
temp_so_zt_2007=temp_so_zt
u_pf_zt_2007=u_pf_zt
u_al_zt_2007=u_al_zt
u_eu_zt_2007=u_eu_zt
u_so_zt_2007=u_so_zt
;goto,plotit
;
; --- Loop here --------
jump: iday = iday + 1
...
;
; interpolate MetO temperature to Poker Flat (65N, 147W)
;
...
plotit:
;save,file='ZT_2007-2008.sav',sdate_all,th,temp_pf_zt,temp_al_zt,temp_eu_zt,$
;     temp_so_zt,u_pf_zt,u_al_zt,u_eu_zt,u_so_zt
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='IPY/ZT.ps'
   device,/color
...
;
for ipt=0L,7L do begin
if ipt eq 0L then begin
   temp_zt=temp_pf_zt_2007
   kday=n_elements(sdate_all_2007)
   kday0=n_elements(sdate_all_2007)
   sdate0=sdate_all_2007(0)
   sdate1=sdate_all_2007(kday-1)
endif
if ipt eq 1L then temp_zt=temp_al_zt_2007
if ipt eq 2L then temp_zt=temp_eu_zt_2007
if ipt eq 3L then temp_zt=temp_so_zt_2007
if ipt eq 0L then u_zt=u_pf_zt_2007
if ipt eq 1L then u_zt=u_al_zt_2007
if ipt eq 2L then u_zt=u_eu_zt_2007
if ipt eq 3L then u_zt=u_so_zt_2007
if ipt eq 4L then begin
   temp_zt=temp_pf_zt
   kday=n_elements(sdate_all)
   sdate0=sdate_all(0)
   sdate1=sdate_all(kday-1)
endif
if ipt eq 5L then temp_zt=temp_al_zt
if ipt eq 6L then temp_zt=temp_eu_zt
if ipt eq 7L then temp_zt=temp_so_zt
if ipt eq 4L then u_zt=u_pf_zt
if ipt eq 5L then u_zt=u_al_zt
if ipt eq 6L then u_zt=u_eu_zt
if ipt eq 7L then u_zt=u_so_zt
...
;
    ...
    syr=strmid(sdate_all_2007,0,4)
    smn=strmid(sdate_all_2007,4,2)
    sdy=strmid(sdate_all_2007,6,2)
    xindex=where(sdy eq '15',nxticks)
    xlabs=smn(xindex)
    ylab=' '
    yticklab=[' ',' ',' ',' ',' ']
    if ipt eq 0L or ipt eq 4L then begin
       ylab='Theta (K)'
       yticklab=['300','500','1000','1500','2000']
       ytickplace=float(yticklab)
    endif
    nyticks=n_elements(yticklab)
    title1=slab(ipt)+' '+'('+slon(ipt)+','+slat(ipt)+')'
    if ipt ge 4 then title1=' '
    contour,temp_zt,findgen(kday),th,charsize=1.5,/noeras,yrange=[300.,max(th)],charthick=2,$
            ytitle=ylab,yticks=nyticks-1,ytickv=ytickplace,ytickname=yticklab,$
            levels=level,c_color=col1,/cell_fill,color=0,title=title1,$
            xrange=[0.,kday-1],xticks=nxticks-1,xtickname=' '+strarr(nxticks+1)
    contour,temp_zt,findgen(kday),th,levels=[190.],color=mcolor,/follow,/noeras,/overplot,min_value=0.
...
if index(0) ne -1L then oplot,x2d(index),y2d(index),psym=8,color=mcolor,symsize=0.25
    ...
    for ii=0L,nxticks-1L do begin
        plots,xindex(ii),300.
        plots,xindex(ii),200.,/continue,color=0,thick=2,/data
    endfor

    endfor
    xyouts,.5,.9,'2007-2008',/normal,color=0,charsize=3,charthick=2,alignment=0.5
    xyouts,.5,.46,'2008-2009',/normal,color=0,charsize=3,charthick=2,alignment=0.5

    imin=min(level)
    imax=max(level)
    ymnb=yorig(4)-cbaryoff
    ymxb=ymnb+cbarydel
    set_viewport,xorig(4),xorig(7)+xlen,ymnb,ymxb
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MetO Temperature (K)',charsize=2,charthick=2
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim IPY/ZT.ps -rotate -90 IPY/ZT.png'
       spawn,'/usr/bin/rm IPY/ZT.ps'
    endif
end

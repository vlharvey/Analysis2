FUNCTION YYYYMMDD_TO_DFS_SH,DATE

;CALL THIS WITH:  RESULT=YYYYMMDD_TO_DFS_SH(DT)
...
;CALCULATES DAYS FROM 21 DECEMBER.
;IF DATES IN MULTIPLE YEARS ARE INCLUDED,
;   IT ASSUMES 21 DECEMBER OF *EACH* YEAR SEPARATELY.
;   DAYS FROM 22 DEC TO 21 JUNE ARE POSITIVE DFS. DAYS FROM
;   22 JUNE TO 20 DEC ARE NEGATIVE DFS.
   ...
   if npos gt 0 then begin
      for i=0,npos-1 do begin
         index=pos(i)
         dfs(index) = jday(index)-julday(12,21,yyyy(index))
      endfor
   endif
   ...
   if npos gt 0 then begin
      for i=0,npos-1 do begin
         index=pos(i)
         dfs(index)=jday(index)-julday(12,21,yyyy(index)-1)
      endfor
   endif

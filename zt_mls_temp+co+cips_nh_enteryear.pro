;
; NH summer PMCs, SH winter temperature
...
; save temp, co, etc
;
save,file='zt_mls_temp+co_'+yearlab+'.sav',mlspolarco_zt,mlspolartp_zt,kday,altitude,sdate_all
quick:
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co+cips_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
tlevel=165.+5.*findgen(23)
nlvls=n_elements(tlevel)
col1=1+indgen(nlvls)*icolmax/nlvls
contour,mlspolartp_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[15.,100.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='MLS Avg Temperature > 60 N',/cell_fill,c_color=col1,$
...
;
; loop over CIPS lat bins
;
restore,'~harvey/CIPS_data/Pre_process/cloud_freq_latbin_all.sav'
nlat=n_elements(latbin)
;for ilat=0L,nlat/2-1L do begin
for ilat=0L,1	do begin	;nlat/2-1L do begin
    ...
    cloud_freq_avg_nhlat=reform(cloud_freq_avg_all(*,ilat2))
    cloud_freq_avg_shlat=reform(cloud_freq_avg_all(*,ilat))
    syear=strmid(strcompress(sdate_all,/remove_all),0,4)

year1=strcompress(minyear,/remove_all)
year2=strcompress(maxyear,/remove_all)
index1=where(((syear eq year1 and fdoy_all ge 305.) or (syear eq year2 and fdoy_all le 121.)) and cloud_freq_avg_shlat ge 0.1)
;print,latbin(ilat),n_elements(index1)
if n_elements(index1) ge 5L then begin
;print,sdate_all(index1)
   cfreqnew=smooth(cloud_freq_avg_shlat(index1),3)
   cfreqnew=cloud_freq_avg_shlat(index1)
   fdoysave=fdoy_all(index1)
   fdoynew=fdoy_all(index1)
   index=where(fdoysave gt 130.)
   fdoynew(index)=fdoynew(index)-305.
   index=where(fdoysave lt 130.)
   fdoynew(index)=fdoynew(index)+61.
;  oplot,fdoynew,cfreqnew,color=mcolor,psym=8
   oplot,fdoynew,cfreqnew,color=mcolor,psym=0,thick=10
endif
...
xyouts,xmn+0.02,ymn+0.02,yearlab,/normal,color=0,charsize=3
;
; print end date
;
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co+cips_'+yearlab+'.ps -rotate -90 zt_mls_temp+co+cips_'+yearlab+'.jpg'
    endif
end

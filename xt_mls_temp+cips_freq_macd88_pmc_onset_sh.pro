;
; Hovmoller of MLS temperature 1 Nov to 1 Jan
...
start_date=[-20,-21,-32,-5,-25,-30,-34,-29,-8,-34,-32]
...
for ilat=-75,-75 do begin

syr=string(format='(i2.2)',iyear-2000)
slt=strcompress(abs(ilat),/remove_all)
;restore,'/Volumes/Data/CIPS_data/Pre_process/xt_cips_3c_SH'+syr+'_DSC_'+slt+'Lat_2G_'+ver+'_freq.sav
;decfreq=XTFREQ
;restore,'/Volumes/Data/CIPS_data/Pre_process/xt_cips_3c_SH'+syr+'_ASC_'+slt+'Lat_2G_'+ver+'_freq.sav
;ascfreq=XTFREQ
;cipsfreq=0.*decfreq
...
; save temp
;
save,file='xt_mls_temp_'+yearlab+'_'+slat+'_pmc_onset.sav',mlspolartp_xt,kday,longrid,sdate_all
quick:
yearlab=string(format='(i4)',iyear)+'-'+strcompress(iyear+1,/remove_all)
;restore,'xt_mls_temp_'+yearlab+'_'+slat+'_pmc_onset.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_mls_temp+cips_freq_'+yearlab+'_'+slat+'_pmc_onset.ps'
   !p.charsize=1.55
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
axis,yaxis=1,xaxis=1,xrange=[0.,360.],yrange=[max(dfs_all),min(dfs_all)],/save,charsize=2,color=0,charthick=2
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_mls_temp+cips_freq_'+yearlab+'_'+slat+'_pmc_onset.ps -rotate -90 xt_mls_temp+cips_freq_'+yearlab+'_'+slat+'_pmc_onset.png'
    endif

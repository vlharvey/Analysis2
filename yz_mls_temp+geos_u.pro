;
; daily zonal mean MLS temperature and GEOS zonal mean zonal wind
;
@stddat
@kgmt
@ckday
...
ledmn=1L & leddy=15L & ledyr=2013L
lstday=0L & ledday=0L
;
; get date range
;
...
;
; interpolate GEOS-5 to height surfaces
;
tbarz=fltarr(nr,nlv)
ubarz=fltarr(nr,nlv)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_mls_temp+geos_u_'+sdate+'.ps'
   device,/color
...
contour,ubarz,alat,altitude,levels=10*findgen(20),color=0,/follow,/overplot,min_value=-9999.,c_labels=0*level(index),thick=10
contour,ubarz,alat,altitude,levels=-100+10*findgen(10),color=mcolor,c_linestyle=5,/follow,/overplot,min_value=-9999.,c_labels=0*level(index),thick=10
imin=min(level)
imax=max(level)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_mls_temp+geos_u_'+sdate+'.ps -rotate -90 yz_mls_temp+geos_u_'+sdate+'.jpg'
   spawn,'rm -f yz_mls_temp+geos_u_'+sdate+'.ps'
endif
goto,jump
end

...
for iES = 0L, n_elements(dayzeros) - 1L do begin
	...
	if iday ge 0L and iday le 9L then begin

		if iday eq 0L then plot,/ylog,phase60[inday,*], pressure_grid,position=[.001,.001,.999,.999],xrange = [0,360],yrange = [100,.001], psym = 2,symsize = 1.2,$
		title = 'ES days 0-10 event ' + strtrim(strcompress(string(format='(I2.2)',ies + 1L)),2), color = 0,xstyle = 1, ystyle = 1
	
		if iday gt 0L then oplot,phase60[inday,*], pressure_grid, psym = 2,symsize = 1.2, color = (iday)/10. *255.
	
		oplot,  lon,stratopause, linestyle = 1, thick = 2,  color = (iday)/10.*255.
	endif
  ...
  if setplot eq 'ps' then begin
	...
	spawn,$
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/60N_phase_plot_days_0-10_event_' + strtrim(strcompress(string(format='(I2.2)',ies + 1L)),2)+ '.png'

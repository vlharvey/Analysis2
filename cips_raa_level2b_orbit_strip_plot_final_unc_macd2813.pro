...
; trick the code
;
RAYLEIGH_ALBEDO_ANOMALY=RAYLEIGH_ALBEDO_ANOMALY_unc
	...
	RAA_Plot_filename = outpath+'RAA_Level2b_v01.1_rev04_Orbit-'+strOrbits[iorbit]+'_'+UT_DATE_ORBIT_START+'_contour_unc.png'
	RAA_Plot_filename_60 = outpath+'small_RAA_Level2b_v01.1_rev04_Orbit-'+strOrbits[iorbit]+'_'+UT_DATE_ORBIT_START+'_contour_unc.png'
	RAA_Plot_filename_thumb = outpath+'RAA_Level2b_Orbit-'+strOrbits[iorbit]+'_'+UT_DATE_ORBIT_START+'.thumb_unc.png'
...
if nh_pmc eq 1 then begin
	...
	levels1 = (findgen(62)*(raa_2std/60.))
	endif else 	levels1 = (findgen(62)*(raa_2std/60.))	; (findgen(62)*.1-3.)
imin=0	;min(rayleigh_albedo_anomaly,/Nan)
imax=3.	;max(rayleigh_albedo_anomaly,/Nan)
nlvls=12L
interval=(imax-imin)/float(nlvls)
levels1=imin+interval*findgen(nlvls)
...
imax=3. ;max(rayleigh_albedo_anomaly,/Nan)
nlvls2=12L
interval=(imax-imin)/float(nlvls2)
levels2=imin+interval*findgen(nlvls2)

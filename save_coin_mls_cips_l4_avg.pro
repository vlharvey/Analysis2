...
; 	1) save total number of CIPS points regardless of albedo value
; then, points with 50 < sza < 91 and albedo gt 5 Garys and albedo ne NaN save:
...
;     map_set,-90,0,0,/ortho,/contin,/grid,title='CIPS',color=0,limit=[-50.,0.,-80.,360.],/noerase	; SH
      utmin=0. & utmax=24.
...
;oplot,clon(allindex),clat(allindex),color=mcolor*.7,psym=1
print,'ncoin ncloud frac ',ncoin,ncloud,100.*float(ncloud)/float(ncoin),' dx dt = ',dist(cloudindex(0)),clttime(cloudindex(0))-ltime0
...
;save,file='cips_mls_coins_'+sdate+'_all.sav',comment,ncoins,nclouds,$
...
;
xcoincips_median=-99.fltarr(mprof)		; accommodate points crossing GM
ycoincips_median=-99.fltarr(mprof)
utcoincips_median=-99.fltarr(mprof)
ltcoincips_median=-99.fltarr(mprof)
dxcoincips_median=-99.fltarr(mprof)
dtcoincips_median=-99.fltarr(mprof)
albcoincips_median=-99.fltarr(mprof)
radcoincips_median=-99.fltarr(mprof)
iwccoincips_median=-99.fltarr(mprof)
szacoincips_median=-99.fltarr(mprof)
xcoincips_sigma=-99.fltarr(mprof)
ycoincips_sigma=-99.fltarr(mprof)
utcoincips_sigma=-99.fltarr(mprof)
ltcoincips_sigma=-99.fltarr(mprof)
dxcoincips_sigma=-99.fltarr(mprof)
dtcoincips_sigma=-99.fltarr(mprof)
albcoincips_sigma=-99.fltarr(mprof)
radcoincips_sigma=-99.fltarr(mprof)
iwccoincips_sigma=-99.fltarr(mprof)
szacoincips_sigma=-99.fltarr(mprof)
...
comment(1)='Coincidence Criteria = '+sdxc+' km and '+sdtc+' hours'
comment(2)='Water Vapor is in ppmv.'
save,file='cips_mls_coins_'+sdate+'_median.sav',comment,ncoins,nclouds,$
     xcoinmls,ycoinmls,utcoinmls,ltcoinmls,tempcoinmls,h2ocoinmls,ldoy,$
     ...
     iwccoincips_median,szacoincips_median,$
     xcoincips_sigma,ycoincips_sigma,utcoincips_sigma,ltcoincips_sigma,$
     dtcoincips_sigma,dxcoincips_sigma,albcoincips_sigma,radcoincips_sigma,$
     iwccoincips_sigma,szacoincips_sigma,$

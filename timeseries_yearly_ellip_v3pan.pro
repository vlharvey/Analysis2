...
print,'MLS ',ith,th(ith)
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='timeseries_ellip_3pan_'+sth+'.ps'
             device,/color
...
for i=0,nn-1L do begin
    dum=reform(ellip_allyearsmls(i,*))
...
for i=0,nn-1L do begin
    dum=reform(ellip_allyearsmerra(i,*))
    index=where(finite(dum) eq 1 and dum ne 0. and ssw ne 2)
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=smooth(reform(ellip_allyears(*,iyear)),3,/nan)
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[0,1],color=0,charsize=2,charthick=2,xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
...
;oplot,fdoysave,arrmean-arrsig,color=150,thick=5
loadct,39
xyouts,xmn+0.05,ymn+0.03,'SD-WACCM',/normal,charsize=2,charthick=2,color=0
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=smooth(reform(ellip_allyearsmerra(*,iyear)),3,/nan)
    arr1=reform(ellip_allyearsmerra(*,iyear))
index=where(finite(arr1) eq 1)
if index(0) ne -1L then begin
   kindex=where(arr1 eq max(arr1(index)))
endif
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmeanmerra,yrange=[0,1],color=0,charsize=2,charthick=2,ytitle=sth+' Vortex Ellipticity',xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
;          if fdoy(i) ge startday and fdoy(i) lt max(fdoy) then begin
...
;oplot,fdoysavemerra,arrmeanmerra-arrsigmerra,color=150,thick=5
loadct,39
xyouts,xmn+0.05,ymn+0.03,'MERRA',/normal,charsize=2,charthick=2,color=0
;
; MLS
;
xmn=xorig(2)
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=smooth(reform(ellip_allyearsmls(*,iyear)),3,/nan)
    arr1=reform(ellip_allyearsmls(*,iyear))
index=where(finite(arr1) eq 1)
if index(0) ne -1L then begin
   kindex=where(arr1 eq max(arr1(index)))
endif
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmeanmls,yrange=[0,1],color=0,charsize=2,charthick=2,xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
;          if fdoy(i) ge startday and fdoy(i) lt max(fdoy) then begin
...
;oplot,fdoysavemls,arrmeanmls-arrsigmls,color=150,thick=5
loadct,39
xyouts,xmn+0.05,ymn+0.03,'MLS',/normal,charsize=2,charthick=2,color=0

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert timeseries_ellip_3pan_'+sth+'.ps -rotate -90 timeseries_ellip_3pan_'+sth+'.jpg'
endif

...
; superimpose anticyclone area from GEOS-5
; VLH February 5 2010
;
@stddat
@kgmt
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co+anticylone_area_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
aarea_nh_zt_alt=smooth(aarea_nh_zt_alt,7,/nan)
index=where(finite(aarea_nh_zt_alt) eq 1)
if index(0) ne -1L then aarea_nh_zt_alt(index)=aarea_nh_zt_alt(index)/1.e6	; millions of km^2
loadct,0
contour,aarea_nh_zt_alt,1.+findgen(kday),altitude,levels=[10],color=mcolor*.9,/follow,/overplot,thick=15
contour,aarea_nh_zt_alt,1.+findgen(kday),altitude,levels=[20],color=mcolor*.7,/follow,/overplot,thick=15
contour,aarea_nh_zt_alt,1.+findgen(kday),altitude,levels=[30],color=mcolor*.5,/follow,/overplot,thick=15
contour,aarea_nh_zt_alt,1.+findgen(kday),altitude,levels=[40],color=mcolor*.3,/follow,/overplot,thick=15
contour,aarea_nh_zt_alt,1.+findgen(kday),altitude,levels=[50],color=mcolor*.2,/follow,/overplot,thick=15
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co+anticylone_area_'+yearlab+'.ps -rotate -90 zt_mls_temp+co+anticylone_area_'+yearlab+'.jpg'
    endif
end

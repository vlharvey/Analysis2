...
ifiles=file_search(dirw+'MERRA-on-WACCM_theta_*.nc.sav',count=nfile)
;
; loop over files
;
...
;
    dum=findfile(dirw+'MERRA-on-WACCM_theta_'+sdate+'.nc2')
...
;
ofile=dirw+'MERRA-on-WACCM_theta_'+sdate+'.nc2'
print,'writing ',ofile
nocid = ncdf_create(ofile,/CLOBBER)

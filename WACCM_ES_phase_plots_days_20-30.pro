...
for iES = 0L, n_elements(dayzeros) - 1L do begin
	...
	if iday ge 20L and iday le 29L then begin

		if iday eq 20L then plot,/ylog,phase60[inday,*], pressure_grid,position=[.001,.001,.999,.999],xrange = [0,360],yrange = [100,.001], psym = 2,symsize = 1.2,$
		title = 'ES days 20-30 event ' + strtrim(strcompress(string(format='(I2.2)',ies + 1L)),2), color = 0,xstyle = 1, ystyle = 1
	
		if iday gt 20L then oplot,phase60[inday,*], pressure_grid, psym = 2,symsize = 1.2, color = (iday-20)/10.*255.
	
		oplot,  lon,stratopause, linestyle = 1, thick = 2,  color = (iday-20)/10.*255.
	endif
  ...
  if setplot eq 'ps' then begin
	...
	spawn,$
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/60N_phase_plot_days_20-30_event_' + strtrim(strcompress(string(format='(I2.2)',ies + 1L)),2)+ '.png'

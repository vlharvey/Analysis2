...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_speed_'+sdate+'_'+slev0+'_waccm_Traject_4pan_cross_pole.ps'
       device,/color
...
;   drawvectors,nc+1,nr,x,alat,u,v,20,1
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    contour,speedyz,alat,zlev,yrange=[0.,140.],levels=colevel,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Altitude (km)',title='Wind Speed',$
            xtitle=slon1+'       Latitude      '+slon2,xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6
    contour,speedyz,alat,zlev,/overplot,levels=10.+10.*findgen(nlvls),/follow,c_labels=1+0*colevel,/noeras,color=0
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_speed_'+sdate+'_'+slev0+'_waccm_Traject_4pan_cross_pole.ps -rotate -90 yz_speed_'+sdate+'_'+slev0+'_waccm_Traject_4pan_cross_pole.jpg'
       spawn,'rm -f yz_speed_'+sdate+'_'+slev0+'_waccm_Traject_4pan_cross_pole.ps'
    endif
jumplev:

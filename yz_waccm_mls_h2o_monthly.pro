;
; monthly mean zonal mean WACCM "specified met" version and MLS water vapor
;
@stddat
@kgmt
@ckday
...
;
; WACCM zonal mean h2o and geopotential height
;
    t3d=reform(h2o_4d(*,*,*,imonth))*1.e6
...
;
    dum=findfile(mdir+'waccm_mls_v2.2_'+syear+smonth+'_h2o.sav')
...
;
    ...
    if dum(0) ne '' then begin
       restore,mdir+'cat_mls_v2.2_'+yyyymmdd+'.sav'
       restore,mdir+'h2o_mls_v2.2_'+yyyymmdd+'.sav'
    endif
    if dum(0) eq '' then begin
       dum=findfile(mdir2+'cat_mls_v2.2_'+yyyymmdd+'.sav')
       if dum(0) eq '' then goto,jump
       restore,mdir2+'cat_mls_v2.2_'+yyyymmdd+'.sav'
       restore,mdir2+'h2o_mls_v2.2_'+yyyymmdd+'.sav'   
    endif
        ...
        if good(0) ne -1L then begin
           ...
           for j=0L,nr-1L do begin
               if ymean ge wlat(j)-dy/2. and ymean lt wlat(j)+dy/2. then begin
                  tbar(j,good)=tbar(j,good)+mix(ii,good)*1.e6
...
;
; interpolate WACCM to height surfaces
;
tbarz=fltarr(nr,nlv)
...
h2obar=tbar
h2obarz=tbarz
save,file=mdir+'waccm_mls_v2.2_'+syear+smonth+'_h2o.sav',wlat,altitude,h2obar,h2obarz
plotit:
restore,mdir+'waccm_mls_v2.2_'+syear+smonth+'_h2o.sav'
tbar=h2obar
tbarz=h2obarz
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_waccm_mls_h2o_'+syear+smonth+'.ps'
   device,/color
...
;
erase
xyouts,.3,.7,month(long(smonth)-1)+' '+syear+' Water Vapor',charsize=2,charthick=2,/normal,color=0
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
ymx=yorig(0)+ylen
...
contour,tbarz,wlat,altitude,/noeras,xrange=[-90.,90.],yrange=[20.,90.],charsize=1.5,color=0,$
...
contour,tbar,wlat,altitude,/noeras,xrange=[-90.,90.],yrange=[20.,90.],charsize=1.5,color=0,$
...
pdiff(index)=100.*(tbarz(index)-tbar(index))/tbar(index)
level=-100.+20.*findgen(11)
!type=2^2+2^3
xmn=xorig(2)
xmx=xorig(2)+xlen
ymn=yorig(2)
ymx=yorig(2)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,pdiff,wlat,altitude,xrange=[-90.,90.],yrange=[20.,90.],$
        xticks=6,xtitle='Latitude',charsize=1.5,levels=level,/cell_fill,$
        title='(WACCM-MLS)/MLS',c_color=col2,color=0,min_value=-99.,charthick=2
index=where(level gt 0.)
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,$
      xtitle='(%)',color=0,xticks=n_elements(level)/2,charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x2=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_waccm_mls_h2o_'+syear+smonth+'.ps -rotate -90 yz_waccm_mls_h2o_'+syear+smonth+'.jpg'
endif
endfor	; loop over months
end

...
; Superimpose MLS HNO2 contours
; Superimpoase HIRDLS HNO3 contours
;
@stddat
@kgmt
@ckday
...
plot,polarn2o,altitude,color=0,/noerase,title='HNO!l3!n',xrange=[1.e-10,1.e-7],/xlog
;
...
; compute polar HNO3
;
      mlspolarn2o=fltarr(nlv)
      mlsnn2oprof=lonarr(nlv)
      for ii=0L,n_elements(id)-1L do begin
          if latitude(ii) ge 60. then begin
             n2o_prof=reform(mlsn2omix(ii,*))
             good=where(n2o_prof ne -99.,ngood)
             if good(0) ne -1L then begin
                mlspolarn2o(good)=mlspolarn2o(good)+reform(n2o_prof(good))
                mlsnn2oprof(good)=mlsnn2oprof(good)+1L
             endif
          endif
      endfor
      good=where(mlsnn2oprof gt 0L)
      if good(0) ne -1L then mlspolarn2o(good)=mlspolarn2o(good)/float(mlsnn2oprof(good))
      if good(0) ne -1L then oplot,mlspolarn2o(good),altitude(good),psym=2,color=0
      if good(0) ne -1L then print,'HIRDLS ',polarn2o(good)
      if good(0) ne -1L then print,'MLS ',mlspolarn2o(good)
      if good(0) ne -1L then print,'HIRDLS/MLS ',polarn2o(good)/mlspolarn2o(good)
      mlspolarn2o_zt(icount,*)=mlspolarn2o
skipmls:
      icount=icount+1L
goto,jump
...
polarn2o_zt=polarn2o_zt*1.e9
polarn2o_zt=smooth(polarn2o_zt,7,/NaN,/edge_truncate)
index=where(finite(polarn2o_zt) ne 1)
polarn2o_zt(index)=0.

mlspolarn2o_zt=mlspolarn2o_zt*1.e9
save,file='zt_hirdls_polarno2_hno3+mlshno3_2006.sav',polart_zt,polarno2_zt,polarn2o_zt,$
     mlspolarn2o_zt,kday,altitude,sdate_all
quick:
restore,'zt_hirdls_polarno2_hno3+mlshno3_2006.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_hirdls_polarno2_hno3+mlshno3_'+syear(0)+'.ps'
   device,/color
   device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
endif
;
; plot Arctic mean NO2, HNO3
;
erase
xmn=xorig(0)
xmx=xorig(0)+xlen
...
contour,polarno2_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[15.,50.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='HIRDLS Avg NO!l2!n and HNO!l3!n and MLS HNO!l3!n > 60 N',/fill,c_color=col1,$
...
contour,mlspolarn2o_zt,1.+findgen(kday),altitude,levels=[0.5,1,2,4,6,8,10],color=0,/follow,/overplot,$
        min_value=0.,c_labels=[1,1,1,1,1,1,1],thick=5
contour,polarn2o_zt,1.+findgen(kday),altitude,levels=[4,6,8,10],color=mcolor,/follow,/overplot,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_hirdls_polarno2_hno3+mlshno3_'+syear(0)+'.ps -rotate -90 zt_hirdls_polarno2_hno3+mlshno3_'+syear(0)+'.jpg'
    endif
end

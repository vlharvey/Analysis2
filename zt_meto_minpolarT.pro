;
; plot altitude time section of max temperature poleward of 45 N
...
; save file
;
save,file='MetO_polarT.sav',fdoy,yyyymmdd,th,nh_polart,sh_polart
plotit:
restore,'MetO_polarT.sav'
;
; remove leap days and data voids
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_meto_minpolarT.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
imin=180.
imax=300.
contour,nh_polart_zt,findgen(ndoy),th,ytitle='Temperature',yrange=[imin,imax],$
     xtickname=smonth,xticks=n_elements(smonth)-1,charsize=1.5,color=0,$
     title='MetO Max T poleward of 45 N'
for idoy=0L,ndoy-1L do begin
...
oplot,fdoy(index)-181,NH_POLART_LEV(index),color=250,thick=5
xyouts,10.,185.,'2008-2009',color=250,/data,charsize=1.5,charthick=5
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_meto_minpolarT.ps -rotate -90 zt_meto_minpolarT.jpg'
endif
end

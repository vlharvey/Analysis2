...
MLS_period = [20040808,20150419] ;uses original MLS he5 O3 files [last update 20130226]
...
ncid = ncdf_create(output_path+'sathist_01_EOSMLS_v3_'+start_date+'-'+end_date+'_c'+today()+'.nc',/clobber)
  ncdf_attput, ncid, 'Description', 'Satellite profile positions and times from '+start_date+' through '+end_date, /global
  ncdf_attput, ncid, 'Author', 'V. Lynn Harvey, file was created on '+today()+' using satellite_locations_jmp_mls_c20130326_update.pro', /global
  ncdf_attput, ncid, 'Comment', '(1) EOS MLS O3 he5 files version v03-30 were used (leap seconds not corrected!), '+str_mls_period, /global
...
; ncdf_attput, ncid, 'julian', 'long_name', 'julian date' & ncdf_attput, ncid, 'julian', 'units', 'days since '+strcompress(date_reference,/r)
  ...
  for iday=0l,ndays-1l do begin
    ...
    ;read MLS
    file=file_search(mls_obs_path+'cat_mls_v3.3_'+str_year+str_date_mmdd+'.sav',count=nfiles)
    print,file

...
; 
    nh_minor=NH_DTDY_ALL(index, *)
    nh_major=NH_UBAR65_ALL(index,*)
    sh_minor=SH_DTDY_ALL(index, *)
    sh_major=SH_UBAR65_ALL(index,*)
;
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_dtdy_ubar_era40_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
xyouts,.2,.775,'ERA-40 '+yearlab,/normal,charsize=2.5,charthick=2,color=0
contour,minorsswnh,1.+findgen(nmonth),press,/ylog,yrange=[50.,1.],/noeras,levels=level,$
        c_color=col1,/cell_fill,color=0,ytitle='Pressure (hPa)',title='NH Minor SSWs &',$
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,minorsswsh,1.+findgen(nmonth),press,/ylog,yrange=[50.,1.],/noeras,levels=level,$
...
contour,majorsswnh,1.+findgen(nmonth),press,/ylog,yrange=[50.,1.],/noeras,levels=level,$
        c_color=col1,/cell_fill,color=0,ytitle='Pressure (hPa)',title='NH Major SSWs %',$
        xrange=[1.,nmonth],xticks=nmonth-1,xtickname=nhlabs,xtickv=1+findgen(nmonth),$
        charsize=1.25,charthick=2,xticklen=-0.05
contour,majorsswnh,1.+findgen(nmonth),press,/ylog,yrange=[50.,1.],/noeras,levels=level,$
        /follow,color=mcolor,/overplot,c_labels=0*index
...
ymx=yorig(3)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,majorsswsh,1.+findgen(nmonth),press,/ylog,yrange=[50.,1.],/noeras,levels=level,$
...
if setplot eq 'ps' then begin
device,/close
spawn,'convert -trim zt_dtdy_ubar_era40_'+yearlab+'.ps -rotate -90 zt_dtdy_ubar_era40_'+yearlab+'.jpg'
endif
end

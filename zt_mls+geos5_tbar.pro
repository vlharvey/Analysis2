...
for j=30L,nlat-1L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       device,font_size=9
       device,/landscape,bits=8,filename='zt_mls+geos5_tbar_'+slat+'.ps'
       device,/color
...
contour,tzlat,1.+findgen(kday),altitude_mls,/noeras,xrange=[1.,kday],yrange=[20.,68.],$
      ...
      levels=level,min_value=-99.	;,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
...
jump:
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls+geos5_tbar_'+slat+'.ps -rotate -90 zt_mls+geos_tbar_'+slat+'.jpg'
    endif

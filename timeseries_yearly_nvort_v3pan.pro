;
; SD-WACCM
; timeseries of number of vortex lobes
;
@stddat
@kgmt
@ckday
...
print,'MLS ',ith,th(ith)
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='timeseries_nvort_3pan_'+sth+'.ps'
             device,/color
...
for i=0,nn-1L do begin
    dum=reform(nvort_allyearsmls(i,*))
...
for i=0,nn-1L do begin
    dum=reform(nvort_allyearsmerra(i,*))
    index=where(finite(dum) eq 1 and dum ne 0. and ssw ne 2)
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[1,4],color=0,charsize=2,charthick=2,xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
      loadct,39
   endif
day0=startday
...
if ssw(iyear) ne 2 then begin
   oplot,fdoy,arr1,psym=8,color=col1(iyear)	;1.+mcolor*(iyear/float(nyear-1))
   oplot,fdoy,arr1,thick=3,color=col1(iyear)	;1.+mcolor*(iyear/float(nyear-1))
   kcount=kcount+1
endif
endfor
loadct,39
xyouts,xmn+0.05,ymx-0.03,'SD-WACCM',/normal,charsize=2,charthick=2,color=0
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=reform(nvort_allyearsmerra(*,iyear))
index=where(finite(arr1) eq 1)
if index(0) ne -1L then begin
   kindex=where(arr1 eq max(arr1(index)))
endif
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmeanmerra,yrange=[1,4],color=0,charsize=2,charthick=2,ytitle=sth+' # Vortex Lobes',xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
      loadct,39
   endif
day0=startday
endif
...
loadct,39
xyouts,xmn+0.05,ymx-0.03,'MERRA',/normal,charsize=2,charthick=2,color=0
;
; MLS
;
xmn=xorig(2)
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=reform(nvort_allyearsmls(*,iyear))
index=where(finite(arr1) eq 1)
if index(0) ne -1L then begin
   kindex=where(arr1 eq max(arr1(index)))
endif
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmeanmls,yrange=[1,4],color=0,charsize=2,charthick=2,xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
      loadct,39
   endif
day0=startday
endif
...
loadct,39
xyouts,xmn+0.05,ymx-0.03,'MLS',/normal,charsize=2,charthick=2,color=0

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert timeseries_nvort_3pan_'+sth+'.ps -rotate -90 timeseries_nvort_3pan_'+sth+'.jpg'
endif

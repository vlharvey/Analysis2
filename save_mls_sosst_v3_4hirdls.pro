...
;                       clomaskgrid(i,k)=0.              ; unmask if both are good
                     ...
                     if hno3prof(kk+1L) ne -999.99 and hno3prof(kk) ne -999.99 then begin
...
;                       h2omaskgrid(i,k)=0.              ; unmask if both are good
                     ...
                     if o3prof(kk+1L) ne -999.99 and o3prof(kk) ne -999.99 then begin
                     ...
                     if tpprof(kk+1L) ne -999.99 and tpprof(kk) ne -999.99 then begin
...
; check
;
;plot,tpprof,zprof2,yrange=[0.,130.],xrange=[180.,400.],thick=2,title='Temperature'
;oplot,tpgrid(i,*),altitude,psym=2
;x=where(tpmaskgrid(i,*) eq -99.)
;oplot,tpgrid(i,x),altitude(x),psym=8,color=.9*mcolor
;stop
;plot,1.e6*o3prof,zprof2,yrange=[0.,130.],xrange=[0.,20.],thick=2,title='Ozone'
;oplot,1.e6*o3grid(i,*),altitude,psym=2
;x=where(o3maskgrid(i,*) eq -99.)
;oplot,1.e6*o3grid(i,x),altitude(x),psym=8,color=.9*mcolor
;stop
;plot,1.e9*hno3prof,zprof,yrange=[0.,130.],xrange=[0.,20.],thick=2,title='HNO3'
;oplot,1.e9*hno3grid(i,*),altitude,psym=2
;x=where(hno3maskgrid(i,*) eq -99.)
;oplot,1.e9*hno3grid(i,x),altitude(x),psym=8,color=.9*mcolor
;stop
          jumpprof:
      endfor
;

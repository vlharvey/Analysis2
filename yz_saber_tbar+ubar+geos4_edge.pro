...
;
      ...
      for j=0L,nr-1L do begin
          ...
          if index(0) ne -1L then pzm(j,k)=total(p2(j,index,k))/float(npt)
            ...
            for j=0L,nlat-2L do begin
                ...
                if yp le xlat and yp ge xlatp1 then begin
                   ...
                   for k=0L,nlv-2L do begin
                       ...
                       p1=alog(p(k))
                       pp2=alog(p(kp1))
                       if pp ge p1 and pp le pp2 then begin
                          pscale=(pp-p1)/(pp2-p1)
...
jumplev:
    ...
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_saber_'+sdate+'_tbar+ubar+geos4_edge.ps'
       device,/color
...
;   pdiff(index)=100.*(tbar_saberp(index)-tbar_saber(index))/tbar_saber(index)
    ...
    contour,ubar_saberp,alat,press,levels=level(index),color=mcolor,/follow,/noeras,/overplot,min_value=-9999.,$
            c_linestyle=2
index=where(markzm le 0. or pzm le 0.)
if index(0) ne -1L then markzm(index)=9999.
    contour,markzm,y2d,pzm,levels=0.1+0.1*findgen(9),color=0,thick=5,/follow,/noeras,/overplot,max_value=9999.
...
;   pdiff(index)=100.*(ubar_saberp(index)-ubar_saber(index))/ubar_saber(index)
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert yz_saber_'+sdate+'_tbar+ubar+geos4_edge.ps -rotate -90 '+$
             ' yz_saber_'+sdate+'_tbar+ubar+geos4_edge.jpg'
    endif
    jump:

...
;             /bold,/color,bits_per_pixel=8,/times,filename='mls_ubar_'+sdate(iday)+'.ps'
;      !p.charsize=1.25
;      !p.thick=2
;      !p.charthick=5
;      !y.thick=2
...
good=where(finite(u2d))
if max(u2d(good)) eq -9999. then goto,skipday

erase
    contour,u2d,wlat,p,/ylog,xrange=[-90.,90.],yrange=[1000.,0.001],title='Ubar '+sdate(iday),$
            xtitle='Latitude',ytitle='Pressure (hPa)',charsize=2,charthick=2,xticks=6,/nodata,color=0
    contour,u2d,wlat,p,/ylog,/overplot,levels=10+10*findgen(20),color=0,thick=2
    contour,u2d,wlat,p,/ylog,/overplot,levels=-200+10*findgen(20),c_linestyle=5,color=0,thick=2
    contour,u2d,wlat,p,/ylog,/overplot,levels=[0],color=0,thick=6
    oplot,[-65.,-65.],[50.,50.],psym=8,symsize=2,color=0
    oplot,[-60.,-60.],[20.,20.],psym=8,symsize=2,color=0
;
; jet max
;
    index1=where(wlat2d le -40. and p2d le 200. and p2d ge 0.1 and finite(u2d))
    if index1(0) eq -1L then goto,skipday
    wlattemp=reform(wlat2d(index1))
    ptemp=reform(p2d(index1))
    ubartemp=reform(u2d(index1))
    index2=where(ubartemp eq max(ubartemp))

    if ptemp(index2(0)) lt 200. then begin
...
;
    ...
    ubar1d=fltarr(nl)
    for k=0L,nl-1L do ubar1d(k)=mean(ubar(iday,ilat,k))
    index=where(p lt 200. and p ge 0.1,nl2)			; don't capture top of easterly layer - want bottom of it
...
;   if setplot eq 'ps' then begin
;      device, /close
;      spawn,'convert -trim mls_ubar_'+sdate(iday)+'.ps -rotate -90 mls_ubar_'+sdate(iday)+'.png'
;      spawn,'rm -f mls_ubar_'+sdate(iday)+'.ps'
;  endif
   skipday:
endfor
save,file='mls_12Z_SH_Ujet_lat_p_zero.sav',sdate,latjet,pjet,zzero
end

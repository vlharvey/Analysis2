...
;
        ...
        ncfile0=dir+strmid(sdate,2,2)+'_'+smn+sdy+'_NOX.sav'
...
;
      for kk=0,nth2-1L do begin
          ...
          if wtheta(j,km1) gt rlev and wtheta(j,k) le rlev then begin
             ...
             ytnox(icount,j,kk)=nox(j,km1)-zscale*(nox(j,km1)-nox(j,k))
;print,wtheta(j,k),rlev,wtheta(j,km1),zscale
;print,nox(j,k), ytnox(icount,j,kk),nox(j,km1)
;stop
          endif
          endfor
          endfor
...
save,file='yt_waccm_nox+mark_'+yearlab+'.sav',alat,kday,sdate_all,ytnox,ytco,yth2o,ytmark,ytspeed,slev,th,zindex,nth2

quick:
restore,'yt_waccm_nox+mark_'+yearlab+'.sav'
syear=strmid(sdate_all,0,4)
smon=strmid(sdate_all,4,2)
sday=strmid(sdate_all,6,2)
xindex=where(sday eq '01' or sday eq '15',nxticks)
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='yt_waccm_nox+mark_'+yearlab+'_'+slev+'.ps'
         device,/color
...
plotarray=ytnox*1.e9
stitle=yearlab+' NOx, Arctic Vortex, and Wind Speed'
...
;contour,smooth(ytnox(*,*,kk),3),1.+findgen(kday),alat,levels=level,color=0,/noeras,/follow,/overplot,min_value=-9999.
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim yt_waccm_nox+mark_'+yearlab+'_'+slev+'.ps -rotate -90 '+$
         'yt_waccm_nox+mark_'+yearlab+'_'+slev+'.jpg'
endif

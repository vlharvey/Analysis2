...
oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_ch4_spring_'+syear(iyear)+'.ps'
       device,/color
...
;kday=long(max(fdoy))	; uncomment to "zoom" in on partial year
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,c_color=col1,/cell_fill,levels=level,$
         ...
         title=syear(iyear)+' ACE Arctic Methane',min_value=0.
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,color=0,/follow,levels=level,/overplot,$
            c_labels=0*level,min_value=0.
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,color=mcolor,/follow,levels=[0.05,0.1],/overplot,$
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_ch4_spring_'+syear(iyear)+'.ps -rotate -90 zt_ace_ch4_spring_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

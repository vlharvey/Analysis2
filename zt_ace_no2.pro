;
; plot time-altitude section of vortex NO2
;
@fillit
@smoothit
...
;
    noxmix=0.*no2mix
    index=where(no2mask ne -99.)	; good
    noxmix(index)=no2mix(index)
;
; daily average NO2 in the vortex
;
...
onox=onoxsmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_no2_'+syear(iyear)+'.ps'
       device,/color
...
;kday=long(max(fdoy))	; uncomment to "zoom" in on partial year
    contour,onox,1.+findgen(nday),altitude,/noeras,c_color=col1,/cell_fill,levels=level,$
         ...
         title=syear(iyear)+' ACE Arctic NO!l2!n',min_value=0.
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_no2_'+syear(iyear)+'.ps -rotate -90 zt_ace_no2_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

;
; latitude-time series of MLS temperature + H2O
;
@stddat
@kgmt
@ckday
...
;
      spawn,'ls '+dir+'MLS-Aura_L2GP-H2O_v04-*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',h2ofiles
      spawn,'ls '+dir+'MLS-Aura_L2GP-Temperature_v04-*'+string(FORMAT='(i4.4,a1,i3.3)',iyr,'d',iday)+'.he5',tpfiles
...
;     plev=4.64159
      plev=pmls(kk)
      slev=strcompress(string(plev,format='(f7.3)'),/r)
...
temp=mlstp_yt
sdates=sdate_all
save,file='yt_mls_temp_h2o_pmc_'+yearlab+'_'+slev+'.sav',slev,h2o,temp,latbin,sdates
quick:
;restore,'yt_mls_temp_h2o_pmc_'+yearlab+'_'+slev+'.sav'
mlsh2o_yt=h2o
mlstp_yt=temp
sdate_all=sdates
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='yt_mls_temp+h2o_'+yearlab+'_'+slev+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
index=where(finite(mlstp_yt) eq 1)
tmin=min(mlstp_yt(index))
tmax=max(mlstp_yt(index))
nlvls=23
tinc=(tmax-tmin)/float(nlvls)
tlevel=tmin+tinc*findgen(nlvls)
col1=1+indgen(nlvls)*icolmax/nlvls
contour,mlstp_yt,1.+findgen(kday),latbin,/noeras,xrange=[1.,kday],yrange=[-90.,90.],$
      charsize=1.5,color=0,ytitle='Latitude',/cell_fill,c_color=col1,title=yearlab+' MLS '+slev+' hPa',$
...
;
; oplot DFS 34 (1=June 1=DFS -21)
;
dfs=1.+findgen(kday)-21.
index=where(dfs eq 34.)
oplot,index(0)+0*latbin,latbin,thick=3,color=mcolor
index=where(dfs eq 48.)
oplot,index(0)+0*latbin,latbin,thick=3,color=mcolor
index=where(dfs eq 53.)
oplot,index(0)+0*latbin,latbin,thick=3,color=mcolor
imin=min(tlevel)
imax=max(tlevel)
...
set_viewport,xmn,xmx,ymn,ymx
!type=2^2+2^3
index=where(finite(mlsh2o_yt) eq 1)
tmin=min(mlsh2o_yt(index))
tmax=max(mlsh2o_yt(index))
nlvls=23
tinc=(tmax-tmin)/float(nlvls)
tlevel=tmin+tinc*findgen(nlvls)
...
contour,mlsh2o_yt,1.+findgen(kday),latbin,/noeras,xrange=[1.,kday],yrange=[-90.,90.],$
      charsize=1.5,color=0,ytitle='Latitude',/cell_fill,c_color=col1,$
      levels=tlevel,yticks=6,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-99.
...
set_viewport,xmn,xmx,ymnb,ymxb
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='Water Vapor (ppmv)'
ybox=[0,10,10,0,0]
x1=imin
dx=(imax-imin)/float(nlvls)
for jj=0,nlvls-1 do begin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim yt_mls_temp+h2o_'+yearlab+'_'+slev+'.ps -rotate -90 yt_mls_temp+h2o_'+yearlab+'_'+slev+'.jpg'
    endif

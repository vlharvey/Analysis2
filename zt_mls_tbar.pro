;
; Plot time-altitude Tbar for G5 and MLS
;
@stddat
@kgmt
@ckday
...
; TEMPERATURE_MASK
;
    dum=findfile(sdir+'tpd_mls_v2.2_'+sdate+'.sav')
    if dum(0) eq '' then goto,skip
    restore,sdir+'tpd_mls_v2.2_'+sdate+'.sav'
...
;contour,tbar,latbin,altitude,c_color=col1,color=0,/fill,levels=170.+5.*findgen(nlvls),/noerase,title='MLS',yrange=[10.,68.]
...
save,file='zt_mls_tbar_'+yearlab+'.sav',tbar_zt,sdate_all,latbin,altitude

plotit:
restore,'zt_mls_tbar_'+yearlab+'.sav'
sdate0=sdate_all(0)
sdate1=sdate_all(n_elements(sdate_all)-1)
...
for j=nlat-6L,nlat-1L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_tbar_'+slat+'_'+yearlab+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
contour,tzlat,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,100.],$
        color=0,ytitle='Altitude (km)',/fill,c_color=col1,$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
index=where(level gt 0)
contour,tzlat,1.+findgen(kday),altitude,levels=level(index),color=0,/follow,/overplot,c_labels=0*index;,c_linestyle=5
contour,tzlat,1.+findgen(kday),altitude,levels=[170.],color=mcolor,thick=3,/follow,/overplot,c_labels=[1]
contour,tzlat,1.+findgen(kday),altitude,levels=[260.],color=0,thick=3,/follow,/overplot,c_labels=[1]
xyouts,2.,21.,yearlab,/data,charsize=3,charthick=3,color=mcolor
imin=min(level)
imax=max(level)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS '+slat+'N Temperature (K)'
ybox=[0,10,10,0,0]
x1=imin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_tbar_'+slat+'_'+yearlab+'.ps -rotate -90 zt_mls_tbar_'+slat+'_'+yearlab+'.jpg'
    endif

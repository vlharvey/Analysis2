...
;
      if result4(0) eq 0L or result5(0) eq 0L or result10(0) eq 0L then begin	; no OH after 12/12/09
         print,'MLS data missing on '+sdate
         goto,jump
      endif
      print,'MLS data complete on '+sdate
      print,gpfiles,ho2files,tpfiles
;
...
;
      ...
      for k=0L,mlev2-1L do begin
          index1=where(pmls2(k) eq pmls)
          if index1(0) ne -1L then begin
             zflag(k)=1.
;            print,k,pmls2(k),pmls(index1(0))
          endif
      endfor
      kindex=where(zflag eq 1.,mlev)
      longitude=ho2.longitude
      index=where(longitude lt 0. and longitude ne -999.990)
      if index(0) ne -1L then longitude(index)=longitude(index)+360.
      latitude=ho2.latitude
      mtime=float(ho2.time)
;
...
; check
;
;plot,1.e9*ho2prof,zprof,yrange=[0.,130.],xrange=[0.,10.],thick=2
;oplot,1.e9*ho2grid(i,*),altitude,psym=2
;a=findgen(8)*(2*!pi/8.)
;usersym,cos(a),sin(a),/fill
;x=where(ho2maskgrid(i,*) eq -99.)
;oplot,1.e9*ho2grid(i,x),altitude(x),psym=8,color=.9*mcolor
...
;
      ...
      id=syyyymmdd+'.'+sprof
      catfile=odir+'cat_mls_'+sver+'_'+sdate+'_new.sav'

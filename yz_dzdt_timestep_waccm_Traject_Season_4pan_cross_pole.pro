...
;
zlev=fltarr(nl)
for k=0L,nl-1L do zlev(k)=mean(zgrd(*,nr/2:nr-1,k))
;
...
for ilev=0L,12L do begin	;nl-1L do begin
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_dzdt_'+sdate+'_'+slev0+'_waccm_Traject_Season_4pan_cross_pole.ps'
       device,/color
...
imax=max(noyz(index))
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,noyz,alat,zlev,yrange=[0.,max(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Altitude (km)',title='NO',$
...
imax=1.
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,aoa1yz,alat,zlev,yrange=[0.,max(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Altitude (km)',title='AOA1=Latitude',$
            xtitle=slon1+'       Latitude      '+slon2,xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6
index=where(level gt 0)
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_dzdt_'+sdate+'_'+slev0+'_waccm_Traject_Season_4pan_cross_pole.ps -rotate -90 yz_dzdt_'+sdate+'_'+slev0+'_waccm_Traject_Season_4pan_cross_pole.jpg'
       spawn,'rm -f yz_dzdt_'+sdate+'_'+slev0+'_waccm_Traject_Season_4pan_cross_pole.ps'
    endif
jumplev:

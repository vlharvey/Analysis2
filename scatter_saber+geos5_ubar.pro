;
; Plot time-altitude Ubar plus scatter plots of Ubar and Udiff
;
@stddat
@kgmt
@ckday
...
for j=28L,nlat-1L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       device,font_size=9
       device,/landscape,bits=8,filename='scatter_saber+geos5_ubar_'+slat+'.ps'
       device,/color
...
;
erase
xyouts,.25,.8,'Zonal Mean Zonal Wind at '+slat+' Latitude',color=0,charsize=2,/normal
xmn=xorig(1)
xmx=xorig(1)+xlen
ymn=yorig(1)
ymx=yorig(1)+ylen
...
tzlat=transpose(reform(UBARZ_YT_SABER(j,*,*)))
index=where(tzlat eq 0. or finite(tzlat) ne 1)
if index(0) ne -1L then tzlat(index)=-999.
;
; hardwire xticks
;
...
contour,tzlat,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,68.],$
      ...
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-999.
...
tzlatg=transpose(reform(UBARZ_YT_GEOS5(index(0),*,*)))
...
ymx=yorig(3)+ylen
set_viewport,xmn,xmx,ymn,ymx
plot,tzlatg1(index),pdiff(index),xrange=[-100.,100.],yrange=[-25.,25.],psym=8,$
     charsize=1.5,xtitle='SABER Zonal Wind',color=0,ytitle='U Difference',$
     title='20-35 km'
;oplot,tzlatg1(index),pdiff(index),psym=8,color=90
for iday=0L,kday-1 do begin
    tprof=reform(tzlatg1(iday,*))
    tdiffprof=reform(pdiff(iday,*))
    index=where(tprof ne 0. and tdiffprof ne 0.)
    if index(0) ne -1L then oplot,tprof(index),tdiffprof(index),psym=8,color=((iday+1)/(kday+2.))*mcolor
endfor
plots,-100,0
plots,100,0,/continue,color=0
plots,0,-25
plots,0,25,/continue,color=0
...
set_viewport,xmn,xmx,ymn,ymx
plot,tzlatg1(index),pdiff(index),xrange=[-100.,100.],yrange=[-25.,25.],psym=8,$
     charsize=1.5,xtitle='SABER Zonal Wind',color=0,$
     title='35-45 km'
;oplot,tzlatg1(index),pdiff(index),psym=8,color=90
for iday=0L,kday-1 do begin
    tprof=reform(tzlatg1(iday,*))
...
jump:
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim scatter_saber+geos5_ubar_'+slat+'.ps -rotate -90 scatter_saber+geos_ubar_'+slat+'.jpg'
    endif

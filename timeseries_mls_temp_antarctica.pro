...
start_dfs=[-27, -21, -24, -24, -26, -27, -34, -28,-33,-28,-27]			; 2017 DFS of -27 as a placeholder is the mean of the previous years
start_doy=[-27, -21, -24, -24, -26, -27, -34, -28,-33,-28,-27]+172
end_date=[66, 65, 61, 61, 64, 61, 64, 80]
nyear=n_elements(start_year)
for ilat=-75,-60,5 do begin               ;80,5 do begin
...
; save temp
;
save,file='timeseries_mls_temp_'+yearlab+'_'+slat+'_antarctica.sav',mlspolartp_xt,kday,doy_all,sdate_all,mlspolartp_xt_min,mlspolartp_xt_max,mlspolartp_strat
quick:
yearlab='2007-2017'
slat=slt
salt='83'
salt2='30'
restore,'timeseries_mls_temp_'+yearlab+'_'+slat+'_antarctica.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_mls_temp_'+yearlab+'_'+slat+'_antarctica.ps'
   !p.charsize=1.55
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
plot,doy_all(*,0),smooth(mlspolartp_xt(*,0),3,/nan,/edge_truncate),/noeras,xrange=[min(doy_all(*,0)),max(doy_all(*,0))],yrange=[200.,220.],ytitle='MLS Temperature (K)',$
...
for nn=0,nyear-1L do begin
    oplot,doy_all(*,nn),smooth(mlspolartp_xt(*,nn),3,/nan,/edge_truncate),color=mcolor*(nn/float(nyear)),thick=20	;psym=nn
;   oplot,[start_doy(nn),start_doy(nn)],[140,180],color=mcolor*(nn/float(nyear)),thick=5,linestyle=5
endfor
xyouts,xmn+0.01,ymn+0.01,slat+'N/'+salt+'km',/normal,charsize=2,color=0,charthick=2
loadct,39
...
plot,doy_all(*,0),smooth(mlspolartp_strat(*,0),3,/nan,/edge_truncate),/noeras,xrange=[min(doy_all(*,0)),max(doy_all(*,0))],yrange=[195.,215.],ytitle='MLS Temperature (K)',$
      charsize=1.5,color=0,xtitle='DOY',charthick=2,/nodata
for nn=0,nyear-1L do begin
    oplot,doy_all(*,nn),smooth(mlspolartp_strat(*,nn),3,/nan,/edge_truncate),color=mcolor*(nn/float(nyear)),thick=20  ;psym=nn
;    oplot,[start_doy(nn),start_doy(nn)],[220,240],color=mcolor*(nn/float(nyear)),thick=5,linestyle=5
endfor
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,charsize=1.2,charthick=2,xticks=nyear,xtickname=['       '+strcompress(start_year,/r),' ']
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert timeseries_mls_temp_'+yearlab+'_'+slat+'_antarctica.ps -rotate -90 timeseries_mls_temp_'+yearlab+'_'+slat+'_antarctica.png'
endif

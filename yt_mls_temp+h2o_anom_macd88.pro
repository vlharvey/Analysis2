...
restore,'yt_mls_temp_h2o_pmc_2005.sav'
sdates_save=sdates
...
restore,'yt_mls_temp_h2o_pmc_2005-2012.sav'
tempmean=temp
h2omean=h2o
...
smonday=smon+sday
    ...
    index=where(smonday ne '0229',kday)
    temp=temp(index,*)
    h2o=h2o(index,*)
    sdates=sdates(index)

index=where(h2o ne 0.)
mlsh2o_yt=0.*h2o
mlsh2o_yt(index)=h2o(index)-h2omean(index)
mlstp_yt=0.*temp
index=where(temp ne 0.)
mlstp_yt(index)=temp(index)-tempmean(index)

sdate_all=sdates_save
sdate0=sdate_all(0)
sdate1=sdate_all(n_elements(sdate_all)-1)
syear=strmid(sdate_all,0,4)
smon=strmid(sdate_all,4,2)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='yt_mls_temp+h2o_anom_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
tlevel=-20.+2.*findgen(21)
nlvls=n_elements(tlevel)
col1=1+indgen(nlvls)*icolmax/nlvls
contour,mlstp_yt,1.+findgen(kday),latbin,/noeras,xrange=[1.,kday],yrange=[-90.,90.],$
      charsize=1.5,color=0,ytitle='Latitude',/cell_fill,c_color=col1,title='Temperature Anomaly',$
...
contour,mlstp_yt,1.+findgen(kday),latbin,levels=[0.],color=0,/follow,/overplot,thick=7
imin=min(tlevel)
imax=max(tlevel)
...
contour,mlsh2o_yt,1.+findgen(kday),latbin,/noeras,xrange=[1.,kday],yrange=[-90.,90.],$
      charsize=1.5,color=0,ytitle='Latitude',/cell_fill,c_color=col1,title='Water Vapor Anomaly',$
      levels=tlevel,yticks=6,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-99.
contour,mlsh2o_yt,1.+findgen(kday),latbin,levels=tlevel,color=0,/follow,/overplot,c_labels=fltarr(nlvls)
contour,mlsh2o_yt,1.+findgen(kday),latbin,levels=[0.],color=0,/follow,/overplot,thick=7
imin=min(tlevel)
imax=max(tlevel)
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim yt_mls_temp+h2o_anom_'+yearlab+'.ps -rotate -90 yt_mls_temp+h2o_anom_'+yearlab+'.jpg'
    endif
end

;
; Hovmoller of MLS temperature NH
...
start_dfs=[-27, -21, -24, -24, -26, -27, -34, -28,-33,-28,-27,-27]                  ; 2017 DFS of -27 as a placeholder is the mean of the previous years
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_mls_temp+cips_freq_12pan_'+slat+'.ps'
   !p.charsize=1.5
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
index=where(smmdd ge '0715' and smmdd le '0812',kday)
mlspolartp_xt=reform(mlspolartp_xt(*,index))
...
;
if iyear-start_year(0) eq 0L then erase
...
if rlat eq 60. or rlat eq 65. then tlevel=130.+2.5*findgen(17)
if rlat eq 50. or rlat eq 55. then tlevel=135.+2.5*findgen(17)
...
;contour,mlspolartp_xt,longrid,findgen(kday),levels=[165],color=0,/follow,/overplot,c_labels=[1],thick=4
;contour,mlspolartp_xt,longrid,findgen(kday),levels=[170],color=mcolor,/follow,/overplot,c_labels=[1],thick=4
;
; set yrange dynamically to feed off existing sdate_all range!
...
;axis,yaxis=1,xaxis=1,xrange=[0.,360.],yrange=[74.,-51.],/save,charsize=1.5,color=0,charthick=2,ytitle='DFS'    ; May 1 to Sep 1
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_mls_temp+cips_freq_12pan_'+slat+'.ps -rotate -90 xt_mls_temp+cips_freq_12pan_'+slat+'.png'
    endif

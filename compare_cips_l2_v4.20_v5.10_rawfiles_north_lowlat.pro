;
; lowlat version. subset clouds equatorward of 65N
...
;
      ...
      if n_elements(FNAMES20) ne n_elements(FNAMESCAT20) then goto,skipcips
      if n_elements(FNAMES51) ne n_elements(FNAMESCAT51) then goto,skipcips
      norbit=norbit20
;
; loop over orbits
;
...
;
          good=WHERE(finite(uttime) eq 1 and quality_flags eq 0. and latitude le 65.,ngood)	; all data
...
;---DO NOT--- GET RID OF ANY INFINITE DATA YET
;
          good=WHERE(finite(uttime) eq 1 and quality_flags eq 0. and latitude le 65.,ngood)
          IF NGOOD GT 0 THEN BEGIN
;
...
; 
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/helvetica,filename='Figures_Level2_v4.2_v5.1/compare_cips_l4_v4.20_v5.10_rawfiles_'+sdate+'_lowlat.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
;
    ...
    if setplot ne 'ps' then stop
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim Figures_Level2_v4.2_v5.1/compare_cips_l4_v4.20_v5.10_rawfiles_'+sdate+'_lowlat.ps -rotate -90 Figures_Level2_v4.2_v5.1/compare_cips_l4_v4.20_v5.10_rawfiles_'+sdate+'_lowlat.jpg'
         spawn,'rm -f Figures_Level2_v4.2_v5.1/compare_cips_l4_v4.20_v5.10_rawfiles_'+sdate+'_lowlat.ps'
      endif
      skipcips:
      icount=icount+1L
goto,jump
...
plotyear:
save,filename='cips_l2_medians_vs_sza_north_'+syear+'_lowlat.sav',SZALIM_HI,SZALIM_LO,kday,sdate_all,dfs_all,freq20,freq51,szabin,medalb20_all,medalb51_all,sigalb20_all,sigalb51_all,$
...
quick:
;restore,'cips_l2_medians_vs_sza_north_'+syear+'_lowlat.sav
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_north_'+syear+'_lowlat.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
for j=0,nlvls-1 do begin
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_north_'+syear+'_lowlat.ps -rotate -90 time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_north_'+syear+'_lowlat.jpg'
;      spawn,'rm -f time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_north_'+syear+'_lowlat.ps'
    endif

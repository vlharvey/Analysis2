...
ACE_FTS_period = [20070515,20170207] 		; SOFIE data range

ace_obs_path='/atmos/aura3/data/SOFIE_data/Datfiles_Sathist/'
...
ncid = ncdf_create(output_path+'sathist_05_SOFIE_v1.3_'+start_date+'-'+end_date+'_c'+today()+'.nc',/clobber)
  ncdf_attput, ncid, 'Description', 'Satellite profile positions and times from '+start_date+' through '+end_date, /global
  ncdf_attput, ncid, 'Author', 'V. Lynn Harvey, file was created on '+today()+' using satellite_locations_jmp_sofie_c20130529_update.pro', /global
  ncdf_attput, ncid, 'Comment', '(5) SOFIE Level 2 files version 1.3 were used, '+str_ace_fts_period+'; ', /global

  ncdf_attput, ncid, 'prof_num', '(5) SOFIE: prof_num is original Event number and uniquely identifies profiles; ', /global
  ncdf_attput, ncid, 'orbit_num', '(5) SOFIE: Orbit Number; ', /global
...
;sequentially read data and write to NetCDF
  ...
  printf,2,'date  nprofs_orig nprofs_incl SOFIE'
;
...
;
  for iday=0l,ndays-1l do begin
    ...
    ;read entire SOFIE dataset
    file=file_search(ace_obs_path+'cat_sofie_v1.3.2007-2017.sav',count=nfiles)
    if iday eq 0L then begin
;
; COMMENT         STRING    = Array[2]
; DATE            LONG      = Array[29995]
; LATITUDE        FLOAT     = Array[29995]
; LONGITUDE       FLOAT     = Array[29995]
; ORBIT_NUMBER    LONG      = Array[29995]
; PROFILE_NUMBER  LONG      = Array[29995]
; TIME            FLOAT     = Array[29995]
;
       restore,file
;
...
ace_orig_orbit_num=ORBIT_NUMBER
    ...
    if ace_nprofs ne 0 then begin
      instrument=[instrument,replicate(5,ace_nprofs)]			; 5=SOFIE
...
;intercept error codes (-999.99) and values out of range
    ...
    ace_incl_nprofs = total(instrument eq 5,/int)

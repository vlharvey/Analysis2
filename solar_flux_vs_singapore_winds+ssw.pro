...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='solar_flux_vs_singapore_winds+ssw_'+slev+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
for i=0L,nyear-1L do if muavg60n10(i) ne -99. then oplot,[f10_feb(i),f10_feb(i)],$
    [urad(i),urad(i)],psym=8,color=((muavg60N10(i)-imin)/(imax-imin))*mcolor
for i=0L,nyear-1L do if uavg60n10(i) ne -99. then oplot,[f10_feb(i),f10_feb(i)],$
    [urad(i),urad(i)],psym=8,color=((uavg60N10(i)-imin)/(imax-imin))*mcolor
      ...
      plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,$
           xtitle='Zonal Wind Speed (m/s) at 60 N and 10 hPa'
      ybox=[0,10,10,0,0]
      x1=imin
...
if setplot eq 'ps' then begin
device,/close
spawn,'convert -trim solar_flux_vs_singapore_winds+ssw_'+slev+'.ps -rotate -90 solar_flux_vs_singapore_winds+ssw_'+slev+'.jpg'
endif
end

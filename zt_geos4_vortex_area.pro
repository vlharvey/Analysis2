...
;
      for thlev=0,nth-1 do begin
          ...
          if index(0) ne -1 then area_zt(icount,thlev)=100.*total(area(index))/hem_area
...
; save file
;
save,file='vortex_area_'+y1+'_'+y2+'_geos4.sav',area_zt,harea_zt,th,sfile

plotit:
restore,'vortex_area_2006_2006_geos4.sav'
y1='2006'
y2='2006'
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_geos4_vortex_area_'+y1+'-'+y2+'.ps'
   device,/color
   device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,$
          xsize=xsize,ysize=ysize
endif
;
; vortex flux indicator, look at flux=area*avg descent
;
erase
xmn=xorig(0)
xmx=xorig(0)+xlen
...
kdaysave=kday
kday=121
plot,[1,kday,kday,1,1],[500.,500.,5000.,5000.,500.],min_value=0.,$
      xrange=[1,kday],yrange=[500.,5000.],/nodata,charsize=1.5,color=0,$
      ytitle='Theta (K)',title='GEOS-4 Vortex Area '+y1+'-'+y2,xtickname=[' ',' '],xticks=1
kindex=where(strmid(sfile,3,2) eq '15',nxtick)
xmon=long(strmid(sfile(kindex),0,2))
for i=0,nxtick-1 do begin
    xlab=smon(xmon(i)-1)
    plots,kindex(i)+1,400.
    plots,kindex(i)+1,500.,/continue,/data,color=0
    xyouts,kindex(i)+1,250.,xlab,/data,alignment=0.5,charsize=1.5,color=0
endfor
nlvls=26
...
level2=[1.,5.,10.,15.,20.]
kday=kdaysave
area_zt=smooth(area_zt,5,/edge_truncate)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='% of the N. Hemisphere'
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_geos4_vortex_area_'+y1+'-'+y2+'.ps -rotate -90 zt_geos4_vortex_area_'+y1+'-'+y2+'.jpg'
endif

...
;
loadct,39
restore,'zt_height_waccm_Traject_Season_70-90N.sav
device,decompose=0
result=size(zt_co)
ksteps=result(1)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='zt_height_waccm_Traject_Season_4pan_70-90N.ps'
       device,/color
...
zlev=lev
    erase
    xyouts,.25,.95,'WACCM-4 >70N Average',/normal,charsize=2,color=0
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
    ymx=yorig(0)+ylen
    ...
    contour,zt_no,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='NO',/ylog
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,zt_co,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='CO',/ylog
    index=where(level gt 0)
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
;   contour,zt_height,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='AOA=Altitude',/ylog
    contour,zt_gph,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='AOA=Altitude',/ylog
    index=where(level gt 0)
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    level=120.+20.*findgen(21)
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,zt_t,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,xtitle='Days',title='Temperature',/ylog
    index=where(level gt 0)
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,zt_u,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,xtitle='Days',title='Zonal Wind',/ylog
    index=where(level gt 0)
...
;   contour,zt_u,findgen(ksteps),zlev,/overplot,levels=[0],/follow,c_labels=[0],/noeras,color=0,thick=5
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,zt_v,findgen(ksteps),zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,xtitle='Days',title='Meridional Wind',/ylog
    index=where(level gt 0)
...
;   contour,zt_v,findgen(ksteps),zlev,/overplot,levels=[0],/follow,c_labels=[0],/noeras,color=0,thick=5
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim zt_height_waccm_Traject_Season_4pan_70-90N.ps -rotate -90 zt_height_waccm_Traject_Season_4pan_70-90N.jpg'
;      spawn,'rm -f zt_height_waccm_Traject_Season_4pan_70-90N.ps'
    endif
end

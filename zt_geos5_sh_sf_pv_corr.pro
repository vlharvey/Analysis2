...
ledmn=10L & leddy=31L & ledyr=2007L
lstday=0L & ledday=0L
;
; get date range
;
...
; 
          ...
          if index(0) ne -1L then rcorr3(kcount-1,ilev)=correlate(sf(index),pv(index))
          index=where(y2d lt 0. and marknew gt 0.)
          if index(0) ne -1L then rcorr5(kcount-1,ilev)=correlate(sf(index),pv(index))
;print,th(ilev),rcorr3(kcount-1,ilev),rcorr5(kcount-1,ilev)
      endfor
goto,jump
;
; plot
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_geos5_sh_sf_pv_corr_'+syr+'.ps'
   device,/color
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim zt_geos5_sh_sf_pv_corr_'+syr+'.ps -rotate -90 '+$
         'zt_geos5_sh_sf_pv_corr_'+syr+'.jpg'
endif
end

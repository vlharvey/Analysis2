...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   set_plot,'ps'
   device,/color,/landscape,bits=8,filename='zt_merra2_ubar_'+syear(iyear)+'_'+slat+'.ps'
...
contour,ubar,dayno,pressure,/ylog,levels=level,/cell_fill,c_color=col1,color=0,title=syear(iyear),$
            ytitle='Pressure (hPa)',yrange=[100.,0.01],charsize=1.5,charthick=2,xtitle='DOY'
...
save,file='merra2_ubar_'+syear(iyear)+'_'+slat+'.sav',ubar,dayno,pressure,altitude,area1

; Close PostScript file and return control to X-windows
     if setplot ne 'ps' then stop
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim zt_merra2_ubar_'+syear(iyear)+'_'+slat+'.ps -rotate -90 '+$
                            'zt_merra2_ubar_'+syear(iyear)+'_'+slat+'.jpg'
     endif

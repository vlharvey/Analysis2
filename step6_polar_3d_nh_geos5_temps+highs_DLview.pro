...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='polar_3d_nh_geos5_meto_'+date+'_DL.ps'
   device,/color
...
;              index=where(tmp ne -9999. and y2d gt 13.7500 and dum eq -1.0*(ihigh+1))
               index=where(tmp ne -9999. and y2d gt 20. and dum eq -1.0*(ihigh+1))
...
;endif
    ...
    !psym=0
    xyouts,0.25,0.9,'Arctic Vortex in GEOS-5',/normal,charsize=3,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_3d_nh_geos5_meto_'+date+'_DL.ps -rotate -90 polar_3d_nh_geos5_meto_'+date+'_DL.jpg'
    endif
goto, jump
end

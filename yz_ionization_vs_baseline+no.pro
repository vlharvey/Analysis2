...
;
    for ivar=0,result0.nvars-1 do begin
        ...
        if result.name eq 'P0' or result.name eq 'lat' or result.name eq 'lon' or result.name eq 'lev' or result.name eq 'hyai' or $
           ...
           result.name eq 'PS' or result.name eq 'e' or result.name eq 'Z3' or result.name eq 'NO' or result.name eq 'T' then begin
           ncdf_varget,ncid,ncdf_varid(ncid,result.name),data
           ...
           if result.name eq 'e' then exfrcgrd=data
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_ionization_vs_baseline+no_'+sdate+'.ps'
       device,/color
...
if index(0) ne -1L then exfrczm(index)=0./0.
    erase
    xyouts,.3,.95,'Baseline '+sdate,/normal,color=0,charsize=2
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
    ymx=yorig(0)+ylen
...
;   contour,xezm,xlat,xaltitude,levels=level,/overplot,color=0,/noeras
    ...
    contour,exfrczm,lat,zmean,levels=level,/cell_fill,c_color=col1,/noeras,color=0,xtitle='Latitude',ytitle='Global Mean Altitude (km)',$
            yrange=[70.,150.],xrange=[-90.,90.],title='WACCM e',charsize=1.5,xticks=6
...
nomax=3.e8
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_ionization_vs_baseline+no_'+sdate+'.ps -rotate -90 yz_ionization_vs_baseline+no_'+sdate+'.jpg'
       spawn,'rm -f yz_ionization_vs_baseline+no_'+sdate+'.ps'
    endif

;
; altitude-time series of MLS N2O
;
@stddat
@kgmt
@ckday
...
save,file='zt_mls_n2o.sav',mlspolarco_zt,kday,altitude,sdate_all
quick:
restore,'zt_mls_n2o.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_mls_n2o_'+syear(0)+'.ps'
   device,/color
...
contour,mlspolarco_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,80.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='MLS Avg N2O > 70 N',/cell_fill,c_color=col1,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_n2o_'+syear(0)+'.ps -rotate -90 zt_mls_n2o_'+syear(0)+'.jpg'
    endif
end

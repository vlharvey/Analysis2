...
; TEMPERATURE_MASK
;
    dum=findfile(sdir+'dmps_hirdls_v2.04.19.geos5.'+sdate+'.sav')
    if dum(0) eq '' then goto,skip
    restore,sdir+'dmps_hirdls_v2.04.19.geos5.'+sdate+'.sav'
    restore,sdir+'cat_hirdls_v2.04.19_'+sdate+'.sav'	; latitude
    print,sdir+'cat_hirdls_v2.04.19_'+sdate+'.sav'
...
;        title='GEOS-5 at HIRDLS '+sdate,yrange=[350.,3000.],xtitle='Latitude',ytitle='Theta (K)'
...
save,file='zt_geos5_at_hirdls_tbar.sav',tbar_zt,sdate_all,latbin,altitude

plotit:
restore,'zt_geos5_at_hirdls_tbar.sav
sdate0=sdate_all(0)
sdate1=sdate_all(n_elements(sdate_all)-1)
...
for j=nlat-6L,nlat-1L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='zt_geos5_at_hirdls_tbar_'+slat+'.ps'
       device,/color
...
contour,tzlat,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,60.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='GEOS-5 at HIRDLS Tbar at '+slat+' Latitude',/fill,c_color=col1,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_geos5_at_hirdls_tbar_'+slat+'.ps -rotate -90 zt_geos5_at_hirdls_tbar_'+slat+'.jpg'
    endif

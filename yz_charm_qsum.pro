...
;             if result.name ne 'T' and result.name ne 'U' and result.name ne 'V' then $
              ...
              if result.name eq 'OMEGA' then wgrd=86400.*(data/100.)	; Pa/s to hPa/day
              ...
              if result.name eq 'QSUM' then iongrd=data*86400.		; s-1 to day-1
...
;
          ...
          for k=0L,nl-1L do begin
              for j=0L,nr-1L do begin
                  ...
                  wbar(j,k)=mean(wgrd(*,j,k))
                  noybar(j,k)=mean(noygrd(*,j,k))
                  ionbar(j,k)=mean(iongrd(*,j,k))
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_charm_qsum_'+sdate+'.ps'
       device,/color
...
lev=zmean
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,ionbar,alat,lev,levels=level,/cell_fill,c_color=col1,/noeras,color=0,$
            yrange=[min(lev),max(lev)],xrange=[-90.,90.],title='Ion bar',charsize=1.5,xticks=6
;   contour,ionbar,alat,lev,levels=level,/overplot,color=0,/noeras
    imin=min(level)
    imax=max(level)
    ymnb=ymn-cbaryoff
    ymxb=ymnb+cbarydel
    set_viewport,xmn,xmx,ymnb,ymxb
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle='(/day)',charsize=1,color=0
    ybox=[0,10,10,0,0]
    x1=imin
    dx=(imax-imin)/float(nlvls)
    for j=0,nlvls-1 do begin
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_charm_qsum_'+sdate+'.ps -rotate -90 yz_charm_qsum_'+sdate+'.jpg'
       spawn,'rm -f yz_charm_qsum_'+sdate+'.ps'
    endif

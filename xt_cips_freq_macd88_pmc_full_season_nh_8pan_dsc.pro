;
; Hovmoller of CIPS frequency NH DSC
;
@stddat
@kgmt
@ckday
...
slat2=strcompress(long(rlat),/remove_all)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_cips_freq_8pan_'+slat2+'_pmc_full_season_dsc.ps'
   !p.charsize=1.5
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='CIPS DSC Frequency V4.20 Rev05 '+slat2+'N/'+salt+'km',$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_cips_freq_8pan_'+slat2+'_pmc_full_season_dsc.ps -rotate -90 xt_cips_freq_8pan_'+slat2+'_pmc_full_season_dsc.png'
    endif
end

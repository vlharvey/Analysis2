;
; compare MLS and MERRA Arctic vortex at a given level
;
@stddat
@kgmt
@ckday
...
!noeras=1
dirm='/Volumes/atmos/aura6/data/MLS_data/Datfiles_Grid/MLS_grid_theta_'
...
; --- Test for end condition and close windows.
      ...
      rd_mls_nc3,dirm+ifile,ncmls,nrmls,nthmls,alonmls,alatmls,thmls,pv2mls,p2mls,$
         u2mls,v2mls,qdf2mls,mark2mls,co2mls,z2mls,sf2mls,h2o2mls,markco2mls,iflag

      if iflag ne 0L then goto,jump
tmp2=0.*p2
for k=0L,nth-1L do tmp2(*,*,k)=th(k)*(p2(*,*,k)/1000.)^0.286
tmp2mls=0.*p2mls
for k=0L,nthmls-1L do tmp2mls(*,*,k)=thmls(k)*(p2mls(*,*,k)/1000.)^0.286

sp2=sqrt(u2^2.+v2^2.)
sp2mls=sqrt(u2mls^2.+v2mls^2.)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename=sdate+'_merra+mls_arctic.ps'
       device,/color
...
; extract MLS vortex
;
        index=where(thmls eq th2(kk))
        if index(0) ne -1L then mark1mls=transpose(mark2mls(*,*,index(0)))
        if index(0) eq -1L then index=where(abs(thmls-th2(kk)) eq min(abs(thmls-th2(kk))))
print,th2(kk),index(0),thmls(index(0))
mark1mls=transpose(mark2mls(*,*,index(0)))
sp1mls=transpose(sp2mls(*,*,index(0)))
        temp1mls=transpose(tmp2mls(*,*,index(0)))
        tempmls=fltarr(nc+1,nr2)
        tempmls(0:nc-1,0:nr2-1)=temp1mls(0:nc-1,nr/2:nr-1)    ; NH
        tempmls(nc,*)=tempmls(0,*)
        sf1mls=transpose(sf2mls(*,*,index(0)))
        sfmls=fltarr(nc+1,nr2)
        sfmls(0:nc-1,0:nr2-1)=sf1mls(0:nc-1,nr/2:nr-1)    ; NH
        sfmls(nc,*)=sfmls(0,*)
...
; draw latitude circles
        ...
        MAP_SET,90,0,0,/ortho,/contin,/grid,/noborder,/noeras,londel=90.,$
...
;
        ...
        loadct,39
        contour,sfmls,xcn,ycn,levels=sflevel,color=100,c_labels=0+0.*sflevel,thick=2,title=sdate,charsize=2,charthick=2
        endif
        ...
        dum(nc,*)=dum(0,*)
        dummls=fltarr(nc+1,nr2)
        dummls(0:nc-1,0:nr2-1)=mark1mls(0:nc-1,nr/2:nr-1)    ; NH
        dummls(nc,*)=dummls(0,*)
        spmls=fltarr(nc+1,nr2)
        spmls(0:nc-1,0:nr2-1)=sp1mls(0:nc-1,nr/2:nr-1)    ; NH
        spmls(nc,*)=spmls(0,*)
print,max(sp),max(spmls),max(spmls)-max(sp)
print,max(temp),max(tempmls),max(tempmls)-max(temp)
;
...
;          contour,temp,xcn,ycn,levels=[180.],color=10,$
;                  thick=3,max_value=1.e15
print,'MLS y0 ',min(y2d(lindex))
           contour,dummls,xcn,ycn,levels=[0.1],color=150,$
                   c_labels=0,thick=5,max_value=1.e15
           contour,spmls,xcn,ycn,levels=[80],color=250,$
                   c_labels=0,thick=5,max_value=1.e15
           contour,spmls,xcn,ycn,levels=[60],color=210,$
                   c_labels=0,thick=5,max_value=1.e15
        endif
;
; anticyclones
...
;    !p.charthick=1.
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim '+sdate+'_merra+mls_arctic.ps -rotate -90 '+sdate+'_merra+mls_arctic.jpg'
       spawn,'rm -f '+sdate+'_merra+mls_arctic.ps'
    endif

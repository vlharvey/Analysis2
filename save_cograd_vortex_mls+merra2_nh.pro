...
;
for kk=12,27L do begin	; 10 hPa to 0.01 hPa (28=0.00464159)
...
;
        if setplot eq 'ps' then begin
           ...
           device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                  /bold,/color,bits_per_pixel=8,/helvetica,filename='Polar_plots/polar_daily_coelat+cograd_press_merra2pv_'+sdate+'_2d_nh.ps'
           !p.charsize=1.25
           !p.thick=2
           !p.charthick=5
           !y.thick=2
...
if max(level) eq 0. then goto,jumpday
       contour,delatlevs2d,yeq,pmls,/ylog,levels=level,c_color=col1,/cell_fill,/noeras,charsize=1,charthick=2,ytitle='Pressure (hPa)',xtitle='Equivalent Latitude',xrange=[20,90],$
...
;if index(0) ne -1L then print,'Vortex Flag ',reform(novortex_flag_2d(n,index))
;
...
; Close PostScript file and return control to X-windows
       ...
       if setplot eq 'ps' then begin
          device,/close
          spawn,'convert -trim Polar_plots/polar_daily_coelat+cograd_press_merra2pv_'+sdate+'_2d_nh.ps -rotate -90 '+$
                              'Polar_plots/polar_daily_coelat+cograd_press_merra2pv_'+sdate+'_2d_nh.jpg'
          spawn,'rm -f Polar_plots/polar_daily_coelat+cograd_press_merra2pv_'+sdate+'_2d_nh.ps'
       endif
jumpday:
;endfor
icount=icount+1L
...
; save monthly file
;
save,filename='Save_files/daily_mls_coelatedge+merra2_sfelatedge_'+syear(iyear)+smon(imon)+'_2d_nh.sav',$

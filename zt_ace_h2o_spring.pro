;
; plot time-altitude section of vortex water
;
@fillit
@smoothit
...
;
    ...
    restore,dira+'h2o_ace_v2.2.'+syear(iyear)
...
if syear(iyear) eq '2004' then oco_fill(0:35,*)=0.      ; do not extrapolate data voids 
oco=oco_fill
;
; smooth
;
...
;oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_h2o_spring_'+syear(iyear)+'.ps'
       device,/color
...
;kday=long(max(fdoy))	; uncomment to "zoom" in on partial year
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,c_color=col1,/cell_fill,levels=level,$
         yrange=[30.,90.],xrange=[1.,kday],xticks=3,$
         xtickname=['                            Jan',$
                    '                            Feb',$
                    '                            Mar','  '],$
         charsize=1.75,ytitle='Alitude (km)',color=0,$
         title=syear(iyear)+' ACE Arctic Water Vapor',min_value=0.
    ...
    plot,fdoy,latitude,psym=1,color=0,yrange=[30.,90.],xrange=[0.,kday],xticks=11,$
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_h2o_spring_'+syear(iyear)+'.ps -rotate -90 zt_ace_h2o_spring_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

...
index=where(yyz lt 4000)
    imin=min(sfyz(index))-abs(0.1*min(sfyz(index)))
    imax=max(sfyz(index))+0.1*max(sfyz(index))
    ...
    contour,sfyz,alat,th,levels=level,/cell_fill,c_color=col1,color=0,title='SF+H2O',$
            xtitle=slon1+'       '+slon2,ytitle='Theta (K)',c_charsize=2,$
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,yrange=[500.,5000.]
    contour,sfyz,alat,th,levels=level,/overplot,/follow,color=0
    contour,spyz,alat,th,levels=[100.],/overplot,/follow,color=0,thick=4,c_labels=['']
    contour,spyz,alat,th,levels=[75.],/overplot,/follow,color=0.9*mcolor,thick=4,c_labels=['']
    contour,spyz,alat,th,levels=[50.],/overplot,/follow,color=0.8*mcolor,thick=4,c_labels=['']
loadct,0
    contour,markyz,alat,th,levels=[0.1],color=100,thick=5,/overplot
loadct,39
...
; water
;
    contour,mco_swath,findgen(n_elements(mlon_swath)),th,levels=[0.1,0.5,1.,2,5,10],/follow,c_color=mcolor,/overplot,thick=5
    imin=min(level)
    imax=max(level)

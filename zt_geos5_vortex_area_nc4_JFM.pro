...
smon=['Jan','Feb','Mar','A','M','J','J','A','S','O','N','D']
...
;
mmdd=strmid(sfile,0,5)
index0=where(mmdd eq '01_01')
index1=where(mmdd eq '04_01')
AREA_ZT_NC4=area_zt_nc4(index0:index1,*)
AREA_ZT_NC5=area_zt_nc5(index0:index1,*)
HAREA_ZT=harea_zt(index0:index1,*)
SFILE=sfile(index0:index1)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_geos5_vortex_area_'+y2+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
!type=2^2+2^3+2^7
plot,[1,kday,kday,1,1],[400.,400.,3000.,3000.,400.],min_value=0.,$
      xrange=[1,kday],yrange=[400.,3000.],/nodata,charsize=1.5,color=0,$
...
for i=0,nxtick-1 do begin
    ...
    xyouts,kindex(i)+1,200.,xlab,/data,alignment=0.5,charsize=1.5,color=0
endfor
nlvls=18
...
contour,area_zt,1.+findgen(kday),th,levels=level,c_color=0,/follow,/overplot,c_labels=0*level
;contour,harea_zt,1.+findgen(kday),th,levels=level2,c_color=mcolor,/follow,/overplot,thick=4
xyouts,5.,500.,y2,/data,charsize=3,color=mcolor,charthick=5
imin=min(level)
imax=max(level)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='GEOS-5 Vortex Area (% NH)',charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_geos5_vortex_area_'+y2+'.ps -rotate -90 zt_geos5_vortex_area_'+y2+'.jpg'
endif
end

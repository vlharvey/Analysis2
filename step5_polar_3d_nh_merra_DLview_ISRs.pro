...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Arctic_3D/'+ifile+'_3D_ISRs.ps'
       device,/color
...
yy=ysites(ii) & xx=xsites(ii)
               ...
               for m=0,10 do $
                   oplot,[xn,xn],[yn,yn],zvalue=nz0(kk)+float(m)*dist/11.,/T3D,psym=8,color=col2(ii)
               endif
endfor
loadct,0
...
loadct,39
    for ii=0L,nsites-1L do begin
        xyouts,.7,.7-0.04*ii,ssites(ii),/normal,color=col2(ii),charsize=2,charthick=2
    endfor
loadct,0
...
;    endfor
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Arctic_3D/'+ifile+'_3D_ISRs.ps -rotate -90 Arctic_3D/'+ifile+'_3D_ISRs.jpg'
       spawn,'rm -f Arctic_3D/'+ifile+'_3D_ISRs.ps'
    endif

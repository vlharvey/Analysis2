;
; monthly mean timeseries of the latitude of the Antarctic vortex at a specified altitude
;
@stddat
@kgmt
@ckday
...
;
    ...
    if ii eq 0L then begin
       nr=n_elements(alat)
       nth=n_elements(th)
       gmarklat=fltarr(12,nr)
       w3marklat=fltarr(12,nr)
       w4marklat=fltarr(12,nr)
       zangle=fltarr(12,nr)
       rth=1600.
       print,th
       read,'Enter desired altitude ',rth
       index=where(rth eq th)
       ith=index(0)
       sth=strcompress(long(rth),/remove_all)+'K'
    endif
    marklev=reform(markzm(*,ith))
    for j=0,nr-1L do gmarklat(ii,j)=marklev(j)
    restore,'yz_wa3_ubarth+mark_'+month(ii)+'.sav'        ;,alat,th,tzm,uzm,markzm
    index=where(rth eq th)
    ithw=index(0)
    marklev=reform(markzm(*,ithw))
    for j=0,nr-1L do w3marklat(ii,j)=marklev(j)
    restore,'yz_wa4_ubarth+mark_'+month(ii)+'.sav'        ;,alat,th,tzm,uzm,markzm
    marklev=reform(markzm(*,ithw))
    for j=0,nr-1L do w4marklat(ii,j)=marklev(j)
;
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_mark_multiyear_monthly_'+sth+'_sh.ps'
   !p.charsize=1.5
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
contour,gmarklat,1+findgen(12),alat,/nodata,/noeras,xrange=[1.,12.],yrange=[-90.,-20.],charsize=2,color=0,/cell_fill,c_color=col1,$
      ytitle='Latitude',xticks=n_elements(xlab)-1,xtickname=xlab,title='Antarctic Vortex at '+sth,levels=level
contour,gmarklat,1+findgen(12),alat,/overplot,/follow,levels=[0.5],color=0,thick=20,c_labels=[0]
contour,w3marklat,1+findgen(12),alat,/overplot,/follow,levels=[0.5],color=250,thick=20,c_labels=[0]
contour,w4marklat,1+findgen(12),alat,/overplot,/follow,levels=[0.5],color=100,thick=20,c_labels=[0]
loadct,0
contour,zangle,1+findgen(12),alat,/overplot,/follow,levels=[90],color=150,thick=20,c_linestyle=5,c_labels=[0]
xyouts,1.2,-40,'SZA=90',/data,color=150,charsize=2
loadct,39
xyouts,1.2,-35.,'GEOS-5',/data,color=0,charsize=2
xyouts,1.2,-30.,'WACCM-3',/data,color=250,charsize=2
xyouts,1.2,-25.,'WACCM-4',/data,color=100,charsize=2

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim timeseries_mark_multiyear_monthly_'+sth+'_sh.ps -rotate -90 timeseries_mark_multiyear_monthly_'+sth+'_sh.jpg'
endif
end

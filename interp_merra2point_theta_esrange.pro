...
lstmn=1L & lstdy=1L & lstyr=1997L
ledmn=2L & leddy=28L & ledyr=2015
lstday=0L & ledday=0L
;
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename=pthout+'merra2esrange_'+sdate+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
contour,temperature_profiles,findgen(kday),th,levels=level,c_color=col1,/cell_fill,/noeras,title='T at Esrange '+sloc,xrange=[0.,kday-1],color=0,yrange=[min(th),max(th)]
...
nlvls=nlvls+1
col1=1+indgen(nlvls)*icolmax/float(nlvls)
contour,zonal_wind_profiles,findgen(kday),th,levels=level,c_color=col1,/cell_fill,/noeras,title='U at Esrange '+sloc,xrange=[0.,kday-1],color=0,yrange=[min(th),max(th)]
...
save,file=pthout+'merra_at_esrange_'+sdate+'.sav',dates,time,longitude,latitude,$
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim '+pthout+'merra2esrange_'+sdate+'.ps -rotate -90 '+pthout+'merra2esrange_'+sdate+'.jpg'
   spawn,'/usr/bin/rm '+pthout+'merra2esrange_'+sdate+'.ps'
endif
end

...
rtd=double(180./!pi)
dtr=1./rtd
lstmn=1L & lstdy=1L & lstyr=2009L 
ledmn=12L & leddy=31L & ledyr=2009L
...
dirm='/atmos/aura6/data/MLS_data/Datfiles_Grid/MLS_grid5_'
...
;
      ...
      wlat=lat
;
; restore MLS gridded data on this day
...
;
      dum=findfile(dirm+'ALL_v3.3_'+sdate+'.sav')
      if dum(0) eq '' then goto,jump
      restore,dum
      restore,dirm+'U_V_v3.3_'+sdate+'.sav'
      zbarm=mean(GP_GRID,dim=1)
      ubarm=mean(u,dim=1)
;
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/times,filename='yz_sdwaccm_ubar+mls_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !y.thick=2
...
contour,ubar,wlat,zbar/1000.,color=0,xtitle='Latitude',thick=6,yrange=[10,100],/noeras,ytitle='Approximate Altitude (km)',/fill,$
     title=sdate,levels=level,c_color=col1,charthick=2,charsize=1.5,xticks=6
index=where(abs(lat) le 10)
ubarm(index,*)=0./0.
contour,ubarm,lat,zbarm/1000.,levels=10+10*findgen(20),c_color=0,/follow,/overplot,c_labels=1+0*level,thick=3
contour,ubarm,lat,zbarm/1000.,levels=-200+10*findgen(20),c_color=mcolor,/follow,/overplot,c_labels=1+0*level,thick=3
imin=min(level)
imax=max(level)
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,$
      xtitle='SD-WACCM Zonal Mean Zonal Wind (m/s)',charthick=2,charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_sdwaccm_ubar+mls_'+sdate+'.ps -rotate -90 yz_sdwaccm_ubar+mls_'+sdate+'.jpg'
endif

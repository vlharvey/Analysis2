...
sdate1=sdate_all(n_elements(sdate_all)-1)
    ...
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yt_geos5_tuvqpv+edge_'+sdate0+'-'+sdate1+'_pv.ps'
       device,/color
...
level=imin+((imax-imin)/float(nlvls))*findgen(nlvls)
    contour,pvbar,findgen(nfile),alat,charsize=1.5,/noeras,yrange=[-90.,0.],$
    ...
    contour,pvbar,findgen(nfile),alat,charsize=1.5,/noeras,/overplot,levels=[0.],color=0
    contour,ubar,findgen(nfile),alat,charsize=1.5,/noeras,/overplot,levels=[80,100,120],color=0
...
loadct,0
;  contour,markbar,findgen(nfile),alat,levels=[1],color=mcolor*.5,/follow,/noeras,/overplot,min_value=0.,c_labels=[0]
loadct,39
    result=moment(zbar)
...
;   xyouts,xmx-0.1,ymx+0.01,'270 K',/normal,color=mcolor*.9,charthick=2,charsize=2
    ...
    for ii=0L,nyear-1L do xyouts,index(ii),-100.,syr(index(ii)),/data,color=0,charsize=2
    xyouts,0.,-100.,syr(0),/data,color=0,charsize=2
;   imin=min(level)
;   imax=max(level)
...
;   !type=2^2+2^3+2^6
;   plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='(PV)'
;   ybox=[0,10,10,0,0]
;   x1=imin
...
;   endfor
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert yt_geos5_tuvqpv+edge_'+sdate0+'-'+sdate1+'_pv.ps -rotate -90 yt_geos5_tuvqpv+edge_'+sdate0+'-'+sdate1+'_pv.jpg'
    endif
end

...
; --- Test for end condition and close windows.
      ...
      if ndays gt ledday then begin
         save,file='WA3_LOP_ZT_sh.sav',date_zt,lop_zt,ho3mean_zt,ao3mean_zt,ho3sigma_zt,ao3sigma_zt
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/lop_o3_wa3_shDL_'+ifile+'.ps'
       device,/color
...
;
        ...
        itheta=index(0)
;
; look in SH for anticyclones
;
        index=where(wtheta2 eq rtheta2 and wlat2 lt -20.,npt)
...
;
        if mmin lt -1L then begin
           ...
           for imin=mmin,-1L do begin
               ...
               if min(ydata(index)) lt ymax then ymax=min(ydata(index))
               endif
           endfor
...
;
        ...
        y0=resulty(0)
        if y0 ge -20. then goto,jumplev                ; ignore tropical anticyclones
if rtheta2 eq th(ilev) then begin
...
;
; calculate distance from anticyclone center of mass to all SH data points
;
; GM logic
;
...
;
        agood=where(o3mark eq 0. and dist le dist0 and ydata le -20.)
        hgood=where(o3mark eq mmin and ydata le -30.)
;
...
;
      save,file='/aura7/harvey/WACCM_data/Datfiles/wa3_shDLlop_'+syr+smn+sdy+'.sav',xlop_save,ylop_save,$
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/lop_o3_wa3_shDL_'+ifile+'.ps -rotate -90 Figures/lop_o3_wa3_shDL_'+ifile+'.jpg'
    endif
    icount=icount+1L
goto, jump

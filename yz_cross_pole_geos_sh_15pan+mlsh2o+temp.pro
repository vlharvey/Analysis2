...
jump: iday = iday + 1
      ...
      ckday,iday,iyr
;
; mar through oct only
;
      if iday lt 90 or iday gt 274 then goto,jumpday
...
;
    index=where(muttime gt 0. and mlat lt -20.,mprof)
...
;
    index=where(mlat le -62. and mlat ge -66.)
    mlon60=mlon(index)
...
;
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='yz_cross_pole_geos_sh_'+sdate+'_'+slon1+'_15pan+mlsh2o+temp.ps'
...
int=5.
    level=imin+int*findgen(nlvls)
    contour,tyz,alat,th,levels=level,/cell_fill,c_color=col1,color=0,title='GEOS-5 Temp',$
            xtitle=slon1+'       '+slon2,ytitle='Theta',$
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,yrange=[500.,4600.]
...
;   level=imin+int*findgen(nlvls)
    contour,dthdzyz,alat,th,levels=level,/cell_fill,c_color=col1,color=0,title='MLS Temp',$
            ...
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,yrange=[500.,4600.],/nodata
    axis,xrange=[0.,n_elements(mlon_swath)-1L],yrange=[500.,4000.],/save,/data
    mtemp_swath=smooth(mtemp_swath,9)
...
int=10.
    level=imin+int*findgen(nlvls)
    contour,zetayz,alat,th,levels=level,/cell_fill,c_color=col1,color=0,title='GEOS-5 dth/dz',$
            xtitle=slon1+'       '+slon2,ytickname=[' ',' ',' ',' ',' ',' ',' ',' '],$
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,yrange=[500.,4600.]
...
;   level=imin+int*findgen(nlvls)
    contour,pvyz,alat,th,levels=level,/cell_fill,c_color=col1,color=0,title='MLS dth/dz',$
            xtitle=slon1+'       '+slon2,ytickname=[' ',' ',' ',' ',' ',' ',' ',' '],$
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,yrange=[500.,4600.],/nodata
    axis,xrange=[0.,n_elements(mlon_swath)-1L],yrange=[500.,4600.],/save,/data
    mdthdz_swath=smooth(mdthdz_swath,9)
    contour,mdthdz_swath,findgen(n_elements(mlon_swath)),th,levels=level,/fill,c_color=col1,/overplot
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim yz_cross_pole_geos_sh_'+sdate+'_'+slon1+'_15pan+mlsh2o+temp.ps -rotate -90 '+$
                            'yz_cross_pole_geos_sh_'+sdate+'_'+slon1+'_15pan+mlsh2o+temp.jpg'
        spawn,'/usr/bin/rm yz_cross_pole_geos_sh_'+sdate+'_'+slon1+'_15pan+mlsh2o+temp.ps'
     endif
     jumpday:
goto,jump
end

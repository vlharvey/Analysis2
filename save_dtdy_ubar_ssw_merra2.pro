...
; MERRA-2 version.  
;
; zonal mean temperature and zonal wind from pressure data
; store 2-D arrays (day vs. pressure) of dT/dy and Ubar 
...
ledmn=5L & leddy=31L & ledyr=2017L
;
...
; read data
;
      restore,dir+sdate+'12.sav'			; use 12Z data
      alat=LATITUDE_WACCM
...
save,file='Save_files/MERRA2_dTdy_Ubar_SSW_Climo.sav',yyyymmdd,syyyymmdd,pressure,$
     nh_dtdy,sh_dtdy,nh_ubar,sh_ubar,comment,icount
;
; plot
;
plotit:
restore,'Save_files/MERRA2_dTdy_Ubar_SSW_Climo.sav'
;
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='merra2_dtdy_ubar_ssw_'+sdate0+'-'+sdate1+'.ps'
   device,/color
...
contour,nh_dtdy,findgen(icount),pressure,yrange=[max(pressure),min(pressure)],/ylog,/noeras,levels=level,$
        c_color=col1,/cell_fill,color=0,ytitle='Theta (K)',title='MERRA2 Tbar 90N-60N',$
...
contour,nh_ubar,findgen(icount),pressure,yrange=[max(pressure),min(pressure)],/ylog,/noeras,levels=level,$
        c_color=col1,/cell_fill,color=0,ytitle='Theta (K)',title='MERRA2 Ubar at 60N',$
        xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
;index=where(level lt 0.)
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim merra2_dtdy_ubar_ssw_'+sdate0+'-'+sdate1+'.ps -rotate -90 '+$
         'merra2_dtdy_ubar_ssw_'+sdate0+'-'+sdate1+'.jpg'
endif

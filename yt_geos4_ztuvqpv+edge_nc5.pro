...
sdate1=sdate_all(n_elements(sdate_all)-1)
    ...
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yt_geos4_tuvqpv+edge_'+sdate0+'-'+sdate1+'_nc5.ps'
       device,/color
...
print,min(tbar),max(tbar)
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim yt_geos4_tuvqpv+edge_'+sdate0+'-'+sdate1+'_nc5.ps -rotate -90 '+$
                           'yt_geos4_tuvqpv+edge_'+sdate0+'-'+sdate1+'_nc5.jpg'
    endif
end

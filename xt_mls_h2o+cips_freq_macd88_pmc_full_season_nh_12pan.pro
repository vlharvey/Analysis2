;
; Hovmoller of MLS water vapor NH
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_mls_h2o+cips_freq_12pan_'+slat+'.ps'
   !p.charsize=1.5
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
mlspolarh2o_xt=reform(mlspolarh2o_xt(*,index))
...
print,min(mlspolarh2o_xt),max(mlspolartp_xt)
contour,mlspolarh2o_xt,longrid,findgen(kday),levels=tlevel(0:-1:2),color=0,/follow,/overplot,c_labels=fltarr(nlvls)
;contour,mlspolartp_xt,longrid,findgen(kday),levels=[1,2,3],color=mcolor,/follow,/overplot,c_labels=[1],thick=4
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS H2O (ppmv) '+slat+'N/'+salt+'km',charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_mls_h2o+cips_freq_12pan_'+slat+'.ps -rotate -90 xt_mls_h2o+cips_freq_12pan_'+slat+'.png'
    endif

;
; plot time-altitude section of vortex SF6
;
@fillit
@smoothit
...
;
    ...
    restore,dira+'sf6_ace_v2.2.'+syear(iyear)
...
;
    ...
    oco(index)=1.e12*oco(index)/float(num(index))
;
; fill
;
oco_fill=oco
...
;oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_sf6_'+syear(iyear)+'.ps'
       device,/color
...
;kday=long(max(fdoy))	; uncomment to "zoom" in on partial year
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,c_color=col1,/cell_fill,levels=level,$
         ...
         title=syear(iyear)+' ACE Arctic SF6',min_value=0.
    ...
    plot,[0,0],[min(level),max(level)],xrange=[0,10],color=0,$
         ...
         yrange=[min(level),max(level)],charsize=1.5,title='pptv'
    xbox=[0,10,10,0,0]
    y1=min(level)
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_sf6_'+syear(iyear)+'.ps -rotate -90 zt_ace_sf6_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

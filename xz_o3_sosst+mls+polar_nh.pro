...
ledmn=1 & leddy=31 & ledyr=2006 & ledday=0
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,bits=8,$
                 filename='XZ_Ozone/xz_o3_sosst+mls+polar_nh_'+lfile+'_'+stheta+'K.ps'
         device,/color
...
if index(0) eq -1L then goto,jumpmls
...
if min(o3mark(index)) lt 0. then begin
...
omax=10.
    nlvls=21
    level=omin+((omax-omin)/(nlvls-1L))*findgen(nlvls)
    col1=1+indgen(nlvls)*mcolor/nlvls
...
;set_viewport,.55,.95,.15,.55
;plot,findgen(10),th,/noeras,xrange=[2.,8.],yrange=[400.,2000.],/nodata,color=0,charsize=1.5,$
;     xtitle='Ozone (ppmv)',charthick=2
;loadct,0
;o3mavg=fltarr(mlev)
;thmavg=fltarr(mlev)
;if nmcoin gt 0L then begin
...
;oplot,o3mavg,thmavg,color=0,thick=8
;endif
;loadct,38
;if nacoin gt 0L then begin
;for ii=0L,nacoin-1L do oplot,o3acoin(ii,*),thacoin(ii,*),color=icolmax*.9,thick=5
;endif
;xyouts,6.5,1850.,'MLS',charthick=2,charsize=2,color=0,/data
;xyouts,6.5,1750.,'ACE',charthick=2,charsize=2,color=icolmax*.9,/data
...
jumpmls:
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert XZ_Ozone/xz_o3_sosst+mls+polar_nh_'+lfile+'_'+stheta+'K.ps -rotate -90 '+$
               ' XZ_Ozone/xz_o3_sosst+mls+polar_nh_'+lfile+'_'+stheta+'K.jpg'
         spawn,'/usr/bin/rm XZ_Ozone/xz_o3_sosst+mls+polar_nh_'+lfile+'_'+stheta+'K.ps'
      endif
      icount=icount+1L
      goto,jump
end

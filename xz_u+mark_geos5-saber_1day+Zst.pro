...
;        !p.font=0
         ...
         device,/landscape,bits=8,$
                 filename='xz_u+mark_geos5-saber_'+slat+'_'+sdate+'.ps'
         device,/color
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle=slat+' N Zonal Wind (m/s)'
ybox=[0,10,10,0,0]
x1=imin
...
contour,tdataz,lonbin,altitude,levels=level,c_color=col1,thick=1,/cell_fill,/noeras,yrange=[20,60],$
        xtitle='Longitude',ytickname=[' ',' ',' ',' ',' ',' '],title='SABER'
contour,tdataz,lonbin,altitude,/overplot,levels=level,/follow,color=0,c_labels=0*level
contour,tdataz,lonbin,altitude,/overplot,levels=[0.],/follow,color=0,thick=3
contour,mark2zbin,lonbin,altitude,/overplot,levels=[0.1],/follow,color=0,c_labels=0*level,thick=5
contour,mark2zbin,lonbin,altitude,/overplot,levels=[-0.1],/follow,color=mcolor,c_labels=0*level,thick=5
loadct,0
index=where(finite(sheight) eq 1)
if index(0) ne -1L then oplot,saberlon(index),sheight(index),color=150,psym=8,symsize=1.5
loadct,39
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
...
contour,pdiff,lonbin,altitude,levels=level,/cell_fill,c_color=col2,min_value=-99.,yrange=[20,60],$
        xtitle='Longitude',ytickname=[' ',' ',' ',' ',' ',' '],title='GEOS5 - SABER'
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,$
      xtitle=slat+' N Differences (m/s)',xticks=n_elements(level)/2
ybox=[0,10,10,0,0]
x2=imin
...
;
         ...
         if setplot eq 'ps' then begin
            device, /close
            spawn,'convert -trim xz_u+mark_geos5-saber_'+slat+'_'+sdate+'.ps '+$
                  ' -rotate -90  xz_u+mark_geos5-saber_'+slat+'_'+sdate+'.jpg'
         endif

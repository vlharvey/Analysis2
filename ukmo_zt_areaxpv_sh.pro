...
rtd=double(180./!pi)
dtr=1./rtd
lstmn=4L & lstdy=1L & lstyr=0L 
ledmn=9L & leddy=30L & ledyr=0L
lstday=0L & ledday=0L
...
;
; sum area of gridpoints in Antarctic vortex
;
      for thlev=0,nth-1 do begin
          pv=transpose(pv2(*,*,thlev))
          mark=transpose(marksf2(*,*,thlev))
          index=where(lat lt 0. and mark gt 0.0,npts)
...
goto,jump
;
; plot altitude-time series of Antarctic vortex area
;
plotit:
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_areaxpv_'+y1+'_sh.ps'
   device,/color
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_areaxpv_'+y1+'_sh.ps -rotate -90 ukmo_areaxpv_'+y1+'_sh.jpg'
endif

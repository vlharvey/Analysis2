...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_mls_temp_'+yearlab+'_'+slat+'_pmc_midseason.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
imin=min(smooth(mlspolartp_xt,1,/nan,/edge_truncate))
imax=max(smooth(mlspolartp_xt,1,/nan,/edge_truncate))
plot,doy_all(*,0),smooth(mlspolartp_xt(*,0),1,/nan,/edge_truncate),/noeras,xrange=[min(doy_all(*,0)),max(doy_all(*,0))],yrange=[imin,imax],ytitle='MLS Temperature (K)',$
...
for nn=0,nyear-1L do begin
    oplot,doy_all(*,nn),mlspolartp_xt(*,nn),color=mcolor*(nn/float(nyear)),thick=20	;psym=nn
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert timeseries_mls_temp_'+yearlab+'_'+slat+'_pmc_midseason.ps -rotate -90 timeseries_mls_temp_'+yearlab+'_'+slat+'_pmc_midseason.png'
endif

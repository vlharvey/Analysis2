...
;
      index=where(yorder ge -90. and yorder lt 0.,npan)
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,bits=8,$
                 filename='xz_eddyo3_soundings+mark_sh_'+ifile+'.ps'
         device,/color
...
;
          ...
          for i=0,nl-1 do begin
              ...
              index=where(o3data(*,i) ne -999.,nx)
              if index(0) ne -1 then begin
              o3bar(i)=total(o3data(index,i))/float(nx)
              o3eddy(index,i)=100.*(o3data(index,i)-o3bar(i))/o3data(index,i)
              endif
          endfor
          xlabels=strcompress(string(ydata(sindex)),/remove_all)
          !type=2^2+2^3
          print,min(o3eddy),max(o3eddy)
          level=-50.+5.*findgen(nlvls)
          contour,o3eddy,xdata,thdata,levels=level,/cell_fill,$
                  title=instorder(ipan)+'      '+xlabels(0),c_color=col1,$
                  min_value=-999.,xticks=4,xrange=[0.,360.],yrange=[500.,1500.]
          contour,o3eddy,xdata,thdata,levels=level,/follow,$
                  /overplot,color=0
;
...
;
      ...
      if setplot eq 'x' then begin
         ...
         write_gif,'xz_eddyo3_soundings+mark_sh_'+ifile+'.gif',img
      endif

;
; daily zonal mean MLS temperature and 1-day temperature tendency
;
@stddat
@kgmt
@ckday
...
if icount eq 0 then tbarold=0.*tbar
ttbar=tbar-tbarold
;
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_mls_temp+tt_'+sdate+'.ps'
   device,/color
...
level=120.+5.*findgen(nlvls)
index=where(tbar eq 0.)
if index(0) ne -1L then tbar(index)=-9999.
contour,tbar,alat,altitude,/noeras,xrange=[-90.,90.],yrange=[20.,100.],charsize=1.5,color=0,$
      xticks=6,/fill,c_color=col1,levels=level,min_value=-9999.,ytitle='Altitude (km)',xtitle='Latitude',title=sdate,charthick=2
...
!type=2^2+2^3
pdiff=-99.+0.*tbar
index=where(tbar ne -9999. and tbarold ne -9999.)
if index(0) eq -1L then goto,jump
pdiff(index)=tbar(index)-tbarold(index)
level=-25.+5.*findgen(11)
index=where(pdiff eq -99.)
if index(0) ne -1L then pdiff(index)=-9999.
contour,pdiff,alat,altitude,/noeras,xrange=[-90.,90.],yrange=[20.,100.],charsize=1.5,color=0,$
      xticks=6,/fill,c_color=col2,levels=level,min_value=-9999.,ytitle='Altitude (km)',xtitle='Latitude',title=sdate,charthick=2
index=where(level gt 0.)
contour,pdiff,alat,altitude,levels=level(index),color=0,/follow,/overplot,min_value=-9999.,c_labels=0*level(index)
index=where(level lt 0.)
contour,pdiff,alat,altitude,levels=level(index),color=mcolor,c_linestyle=5,/follow,/overplot,min_value=-9999.,c_labels=0*level(index)
imin=min(level)
imax=max(level)
...
set_viewport,xmn,xmx,ymnb,ymxb
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS Temp Tend (K)',charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
dx=(imax-imin)/float(nlvls)
for j=0,nlvls-1 do begin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_mls_temp+tt_'+sdate+'.ps -rotate -90 yz_mls_temp+tt_'+sdate+'.jpg'
   spawn,'/usr/bin/rm -f yz_mls_temp+tt_'+sdate+'.ps'
endif
goto,jump
end

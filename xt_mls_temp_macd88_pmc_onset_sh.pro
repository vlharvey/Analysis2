...
; save temp
;
save,file='xt_mls_temp_'+yearlab+'_'+slat+'_pmc_onset_'+salt+'.sav',mlspolartp_xt,kday,longrid,sdate_all
quick:
restore,'xt_mls_temp_'+yearlab+'_'+slat+'_pmc_onset_'+salt+'.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_mls_temp_novdec_'+yearlab+'_'+slat+'_pmc_onset_'+salt+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
;tlevel=130.+2.5*findgen(27)
tlevel=270.+findgen(31)	; summer stratopause
tlevel=200.+2.*findgen(31)	; summer stratosphere
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_mls_temp_novdec_'+yearlab+'_'+slat+'_pmc_onset_'+salt+'.ps -rotate -90 xt_mls_temp_novdec_'+yearlab+'_'+slat+'_pmc_onset_'+salt+'.png'
    endif

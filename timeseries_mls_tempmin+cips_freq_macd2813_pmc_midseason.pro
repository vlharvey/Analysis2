;
; minimum temperature around each latitude circle
;
...
; save temp
;
save,file='timeseries_mls_temp_'+yearlab+'_'+slat+'_pmc_midseason.sav',mlspolartp_xt,kday,doy_all,sdate_all,mlspolartp_xt_min,mlspolartp_xt_max,mlspolartp_strat
quick:
yearlab='2007-2018'
slat=slt
salt='83'
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_mls_tempmin_'+yearlab+'_'+slat+'_pmc_midseason.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
index=where(mlspolartp_xt eq 0.)	;eq 0.)
if index(0) ne -1L then mlspolartp_xt(index)=0./0.
index=where(mlspolartp_xt_min eq 0.)      ;eq 0.)
if index(0) ne -1L then mlspolartp_xt_min(index)=0./0.
index=where(mlspolartp_xt_max eq 0.)      ;eq 0.)
if index(0) ne -1L then mlspolartp_xt_max(index)=0./0.
imin=min(smooth(mlspolartp_xt_min,1,/nan,/edge_truncate))
imax=max(smooth(mlspolartp_xt_min,1,/nan,/edge_truncate))
plot,doy_all(*,0),smooth(mlspolartp_xt_min(*,0),3,/nan,/edge_truncate),/noeras,xrange=[min(doy_all(*,0)),max(doy_all(*,0))],yrange=[imin,imax],ytitle='MLS Minimum Temperature (K)',$
      charsize=1.5,color=0,xtitle='DOY',charthick=2,xticklen=-0.075,/nodata	;,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-99.,/nodata

for nn=0,nyear-1L do begin
    oplot,doy_all(*,nn),mlspolartp_xt_min(*,nn),color=mcolor*(nn/float(nyear)),thick=20	;psym=nn
;   oplot,[start_doy(nn),start_doy(nn)],[140,180],color=mcolor*(nn/float(nyear)),thick=5,linestyle=5
endfor
xyouts,xmn+0.01,ymx-0.04,slat+'N/'+salt+'km',/normal,charsize=2,color=0,charthick=2
loadct,39
...
;set_viewport,xmn,xmx,ymn,ymx
;!type=2^2+2^3+2^7       ; ticks outward
;index=where(mlspolartp_strat eq 0.)      ;eq 0.)
;if index(0) ne -1L then mlspolartp_strat(index)=0./0.
;imin=min(smooth(mlspolartp_strat,1,/nan,/edge_truncate))
;imax=max(smooth(mlspolartp_strat,1,/nan,/edge_truncate))
;plot,doy_all(*,0),smooth(mlspolartp_strat(*,0),1,/nan,/edge_truncate),/noeras,xrange=[min(doy_all(*,0)),max(doy_all(*,0))],yrange=[imin,imax],ytitle='MLS Temperature (K)',$
;      charsize=1.5,color=0,xtitle='DOY',charthick=2,/nodata,xticklen=-0.075
;for nn=0,nyear-1L do begin
;    oplot,doy_all(*,nn),mlspolartp_strat(*,nn),color=mcolor*(nn/float(nyear)),thick=20  ;psym=nn
;    oplot,[start_doy(nn),start_doy(nn)],[225,240],color=mcolor*(nn/float(nyear)),thick=5,linestyle=5
;endfor
;xyouts,xmn+0.01,ymn+0.01,slat+'N/'+salt2+'km',/normal,charsize=2,color=0,charthick=2
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert timeseries_mls_tempmin_'+yearlab+'_'+slat+'_pmc_midseason.ps -rotate -90 timeseries_mls_tempmin_'+yearlab+'_'+slat+'_pmc_midseason.png'
endif

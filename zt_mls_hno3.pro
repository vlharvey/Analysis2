;
; altitude-time series of MLS nitric acid
;
@stddat
@kgmt
@ckday
...
;
      ...
      restore,mdir+'hno3_mls_v4.2_'+sdate+'.sav'
...
mlspolarco_zt=mlspolarco_zt*1.e9
;save,file='zt_mls_hno3.sav',mlspolarco_zt,kday,altitude,sdate_all
;quick:
;restore,'zt_mls_hno3.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_mls_hno3_'+syear(0)+'.ps'
   device,/color
...
!type=2^2+2^3
level=0.5*findgen(25)
;level=[1.e-10,1.e-9,1.e-8,1.e-7,1.e-6,1.e-5,1.e-4,1.e-3,1.e-2,1.e-1,1.e0,1.e1]
...
contour,mlspolarco_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,40.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='MLS HNO!l3!n Avg > 50 N',/cell_fill,c_color=col1,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_hno3_'+syear(0)+'.ps -rotate -90 zt_mls_hno3_'+syear(0)+'.jpg'
    endif
end

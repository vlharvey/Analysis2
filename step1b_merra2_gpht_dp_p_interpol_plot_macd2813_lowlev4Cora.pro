;
; save 850 hPa winds for Randall et al 2017
;
;loadct,39
;nlvls=30
;mcolor=byte(!p.color)
...
g0 = 9.81
Cp = 1004.
Pressure = [.015,0.02,.03,.04,.05,.06,.07,.08,.1,.15,.2,.3,.4,.5,.6,.7,.8,1.,1.5,2.,3.,4.,5.,6.,7.,8.,10.,15.,20.,30.,40.,50.,60.,70.,80.,100.,150.,200.,300.,400.,500.,850.]
np=n_elements(pressure)

print,'starting'
spawn,'date'
dirw='/atmos/harvey/MERRA2_data/Datfiles/MERRA2-on-WACCM_'
dum1=file_search(dirw+'20160716??.sav',count=count1)		; works for 2010-2019
...
;
    dum=findfile(dirw+'press2_'+sdate+'.sav')
...
;
    ...
    levels_assim=levels_waccm	; 1d Pressure array - carried over from MERRA - this doesn't exist for MERRA2 and levels_assim is model levels 1-72
...
;lnPsum[*,*,nz- 1L] = alog(1000.) - lnP[nz - 1L]
	...
	for jj = 0L, nr - 1L do begin
		...
		GPHT_on_P[ii,jj,*] = interpol(GPHT1[ii,jj,*],alog(P1[ii,jj,*]),alog(Pressure))
                ...
                O3GRD[ii,jj,*] = interpol(o3new[ii,jj,*],alog(P[ii,jj,*]),alog(Pressure))
...
save,filename=dirw+'press2_'+sdate+'.sav',LATITUDE_WACCM,LONGITUDE_WACCM,PRESSURE,PSGRD,O3GRD,QVGRD,TGRD,UGRD,VGRD,ZGRD
jumpfile:

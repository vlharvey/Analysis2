...
;oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_ch4_'+syear(iyear)+'.ps'
       device,/color
...
;   contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,color=mcolor,/follow,levels=[.2],/overplot,$
;           c_labels=[0],min_value=0.,thick=2
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_ch4_'+syear(iyear)+'.ps -rotate -90 zt_ace_ch4_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

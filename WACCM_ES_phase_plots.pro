...
for iES = 0L, n_elements(dayzeros) - 1L do begin
	...
	title = 'ES event ' + strtrim(strcompress(string(format='(I2.2)',ies + 1L)),2), color = iday/29.*255.,xstyle = 1, ystyle = 1
	
	if iday gt 0L then oplot,phase60[inday,*], pressure_grid, psym = 2,symsize = 1.2, color = iday/29.*255.

	oplot,  lon,stratopause, linestyle = 1, thick = 2,  color = iday/29.*255.

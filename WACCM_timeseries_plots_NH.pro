...
restore, '/Volumes/MacD68-1/france/ES_paper/Post_process/WACCM4_dTdy_Ubar_SSW_Climo.sav'
;ICOUNT          LONG      =        14610
;NH_DTDY         FLOAT     = Array[14610, 46]
;NH_UBAR         FLOAT     = Array[14610, 46]
;SH_DTDY         FLOAT     = Array[14610, 46]
;SH_UBAR         FLOAT     = Array[14610, 46]
;SYYYYMMDD       STRING    = Array[14610]
;TH              FLOAT     = Array[46]
;YYYYMMDD        LONG      = Array[14610]

xth = where(th eq 900.,nxth)
if nxth eq 0. then stop

SSW = where(reform(nh_ubar[*,xth]) lt 0L and reform(nh_ubar[*,xth]) gt -999. and reform(nh_dtdy[*,xth]) gt 0L and reform(nh_dtdy[*,xth]) lt 900.,nSSW)
minorSSW = where(reform(nh_dtdy[*,xth]) gt 0L and reform(nh_dtdy[*,xth]) lt 900.,nminorSSW)

SSWs = fltarr(icount)
minorSSWs = fltarr(icount)

SSWs[0] = ssw[0]
ssws[ssw] = 1L
minorSSWs[0] = minorssw[0]
minorssws[minorssw] = 1L


;for ns = 1L, nssw - 1L do begin;
;	if ssw[ns] - ssw[ns-1L] lt 14L then continue   ;  If an SSW day has been defined in the previous two weeks then skip
;	sswstart[ssw[ns]] = 1L
;endfor	

SSWdates = syyyymmdd[where(ssws eq 1L)]
minorSSWdates = syyyymmdd[where(minorssws eq 1L)]


restore, '/Volumes/MacD68-1/france/WACCM_paper/Post_process/area_weighted_strat_daily_mean.sav'
;
spawn,'ls /Volumes/MacD68-1/france/WACCM_paper/Post_process/timeseries_NH_20*.sav',ifiles
...
if nx gt 0L then fake[x] = min(level) +.1
  ...
  contour, fake,days, z ,  levels=level, /cell_fill, c_color = col1,$
           xrange = [1,181],yrange=[20,120],ytitle='Altitude (km)', ystyle = 1,title = 'NH 20' + string(FORMAT='(I2.2)',iyear2)+ '-20' + string(FORMAT='(I2.2)',iyear3),$
           min_value = 0.,max_value = 500., xticks = 6, position = [.1,.51,.9,.93],$
...
loadct, 39
  ...
  if setplot eq 'ps' then begin
	...
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/timeseries/WACCM_timeseries_NH_20' + string(FORMAT='(I2.2)',iyear2) +'-20'+ string(FORMAT='(I2.2)',iyear3) + '.png'

...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_corr_'+smm0+'_mark+msfpr.ps'
   device,/color
...
contour,yzcorr,ymids,th2,/noeras,levels=level(index),/follow,color=mcolor,c_labels=0*level(index),/overplot
index=where(level gt 0.)
contour,yzcorr,ymids,th2,/noeras,levels=level(index),/follow,color=0,c_labels=0*level(index),/overplot
imin=min(level)
imax=max(level)
ymnb=0.2-cbaryoff
ymxb=ymnb+cbarydel
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_corr_'+smm0+'_mark+msfpr.ps -rotate -90 yz_corr_'+smm0+'_mark+msfpr.jpg'
   spawn,'/usr/bin/rm yz_corr_'+smm0+'_mark+msfpr.ps'
endif

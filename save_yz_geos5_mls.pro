;
; daily zonal mean GEOS-5 T, U, V and MLS T, CO, H2O
;
@stddat
@kgmt
@ckday
...
;
    ...
    for j=0L,nr-1L do begin
        ...
        if index(0) ne -1L then begin
           ...
           vbarg(j,k)=total(vpts(index))/float(nx)
...
; TEMPERATURE_MASK
;
    dum=findfile(mdir+'cat_mls_'+sver+'_'+sdate+'.sav')
    if dum(0) eq '' then goto,jump
    restore,mdir+'cat_mls_'+sver+'_'+sdate+'.sav'             ; altitude
    restore,mdir+'tpd_mls_'+sver+'_'+sdate+'.sav'             ; temperature, pressure
    restore,mdir+'co_mls_'+sver+'_'+sdate+'.sav'              ; mix
;
; apply mask
;
    ...
    mlsco=mix
    restore,mdir+'h2o_mls_'+sver+'_'+sdate+'.sav'              ; mix
    index=where(mask eq -99.)
    if index(0) ne -1L then mix(index)=-99.
    mlsh2o=mix
...
;
    nbar=lonarr(nr,nlv)
    for ii=0L,n_elements(id)-1L do begin
        good=where(reform(mlsco(ii,*)) ne -99.,ngood)
        if good(0) ne -1L then begin
           ymean=latitude(ii)
           for j=0L,nr-1L do begin
               if ymean ge alat(j)-dy/2. and ymean lt alat(j)+dy/2. then begin
                  cobar(j,good)=cobar(j,good)+mlsco(ii,good)
                  nbar(j,good)=nbar(j,good)+1L
               endif
           endfor
        endif
...
good=where(nbar gt 0L)
if good(0) ne -1L then cobar(good)=cobar(good)/float(nbar(good))
;
; H2O
;
    nbar=lonarr(nr,nlv)
    for ii=0L,n_elements(id)-1L do begin
        good=where(reform(mlsh2o(ii,*)) ne -99.,ngood)
        if good(0) ne -1L then begin
           ymean=latitude(ii)
           for j=0L,nr-1L do begin
               if ymean ge alat(j)-dy/2. and ymean lt alat(j)+dy/2. then begin
                  h2obar(j,good)=h2obar(j,good)+mlsh2o(ii,good)
                  nbar(j,good)=nbar(j,good)+1L
               endif
           endfor
        endif
...
good=where(nbar gt 0L)
if good(0) ne -1L then h2obar(good)=h2obar(good)/float(nbar(good))
;
    ...
    for j=0L,nr-1L do begin
        ...
        for k=1L,nth-1L do begin
            ...
            if zup ge zz and zlw le zz then begin
               ...
               vbarz(j,kk)=vbarg(j,k-1)+zscale*(vbarg(j,k)-vbarg(j,k-1))
...
;
save,file=mdir+'yz_geos5_mls_'+sdate+'.sav',alat,altitude,tbar,cobar,h2obar,tbarz,ubarz,vbarz
plotit:
;restore,mdir+'yz_geos5_mls_'+sdate+'.sav'

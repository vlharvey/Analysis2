;
; restore daily avg, min, max Temperature poleward of 45 degrees on each day and altitude
...
; restore save file
;
; contents: 'MetO_polarT.sav',fdoy,yyyymmdd,th,nh_polart,sh_polart
restore,'/aura2/harvey/Analysis/Save_files/MetO_polarT.sav'
;
...
; --- Test for end condition and close windows.
      ...
      if icount eq 0 then begin
         ...
         ysindex=where(alat eq min(alat))	; pole
         kday=ledday-lstday
         nfile=kday
...
; save file
;
save,file='/aura2/harvey/Analysis/Save_files/MetO_polarT.sav',fdoy,yyyymmdd,th,nh_polart,sh_polart
plotit:
restore,'/aura2/harvey/Analysis/Save_files/MetO_polarT.sav'
;
; remove leap days and data voids
...
for idoy=0L,ndoy-1L do begin
    index=where(fdoy eq float(idoy+1L) and nh_polart_lev gt 180. and syear ne '2009',nn)
...
loadct,39
;
; superimpoase 2007-2008 in blue
;
index=where(syear eq '2008')
oplot,fdoy(index)+181,NH_POLART_LEV(index),color=90,thick=5
index=where(syear eq '2007' and fdoy ge 181)
oplot,fdoy(index)-181,NH_POLART_LEV(index),color=90,thick=5
xyouts,10.,195.,'2007-2008',color=90,/data,charsize=1.25,charthick=5
;
; superimpose 2009 in red
;
index=where(syear eq '2009')
oplot,fdoy(index)+181,NH_POLART_LEV(index),color=254,thick=5
index=where(syear eq '2008' and fdoy ge 181)
oplot,fdoy(index)-181,NH_POLART_LEV(index),color=254,thick=5
xyouts,10.,185.,'2008-2009',color=254,/data,charsize=1.25,charthick=5

...
;read,'Enter desired theta ',rth
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='timeseries_area_sdwaccm_'+sth+'.ps'
             device,/color
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[0,50],color=0,charsize=2,charthick=2,ytitle='SD-WACCM Vortex Area (%)',xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
...
if ssw(iyear) ne 2 then begin
   oplot,fdoy,arr1,thick=3,color=1.+mcolor*(iyear/float(nyear-1))
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert timeseries_area_sdwaccm_'+sth+'.ps -rotate -90 timeseries_area_sdwaccm_'+sth+'.jpg'
endif

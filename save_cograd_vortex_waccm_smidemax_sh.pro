...
;   if imon le 2 or imon gt 10 then goto,skipmon		; omit Jan, Feb, Dec
    if imon ne 1 then goto,skipmon				; Feb only
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
           /bold,/color,bits_per_pixel=8,/helvetica,filename='Polar_plots/polar_daily_coelat+cograd_waccm-smidemax_'+sdate+'_2d_sh.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
contour,delatlevs2d,yeq,zmean,levels=level,c_color=col1,/fill,/noeras,charsize=1,charthick=2,ytitle='Approx. Alt (km)',xtitle='Equivalent Latitude',xrange=[-90,-20],$
...
imax=max(spbin2d)
level=imin+((imax-imin)/float(nlvls-1))*findgen(nlvls)
if max(level) gt 0. then begin
contour,spbin2d,yeq,zmean,levels=level,c_color=col1,/fill,/noeras,charsize=1,charthick=2,ytitle='Approx. Alt (km)',xtitle='Equivalent Latitude',xrange=[-90,-20],$
        yrange=[min(zmean),100.],thick=5,color=0
dum=reform(sfelatedge_2d(n,*))
index=where(dum lt -10.)
if index(0) ne -1 then oplot,dum(index),zmean(index),psym=8,color=0
if index(0) ne -1 then oplot,dum(index),zmean(index),color=0,thick=3

loadct,0
dum=reform(nashelatedge_2d(n,*))
index=where(dum lt -10.)
if index(0) ne -1 then oplot,dum(index),zmean(index),psym=8,color=150
if index(0) ne -1 then oplot,dum(index),zmean(index),color=150,thick=3
loadct,39
ymnb=ymn -cbaryoff-0.05
ymxb=ymnb+cbarydel
set_viewport,xmn+0.01,xmx-0.01,ymnb,ymxb
...
; Close PostScript file and return control to X-windows
       ...
       if setplot eq 'ps' then begin
          device, /close
          spawn,'convert -trim Polar_plots/polar_daily_coelat+cograd_waccm-smidemax_'+sdate+'_2d_sh.ps -rotate -90 '+$
                              'Polar_plots/polar_daily_coelat+cograd_waccm-smidemax_'+sdate+'_2d_sh.jpg'
          spawn,'rm -f Polar_plots/polar_daily_coelat+cograd_waccm-smidemax_'+sdate+'_2d_sh.ps'
       endif
jumpday:
;endfor
icount=icount+1L
...
; save monthly file
;
save,filename='/atmos/harvey/WACCM_data/Datfiles/Datfiles_Ethan_600yr/CO_Vortex_data/daily_waccm-smidemax_coelatedge+sfelatedge_'+syear(iyear)+smon(imon)+'_2d_sh.sav',$

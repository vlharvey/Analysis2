;
; altitude-time series of MLS temperature + CO + Arctic vortex area
;
@stddat
@kgmt
@ckday
...
;
      ...
      if dum1(0) eq '' then begin
         ...
         if dum1(0) eq '' then goto,skipmls
      endif
      ...
      ncdf_close,ncid
      dum1=findfile(dir2+sdate+'_AVG.V01.nc4')
      if dum1(0) ne '' then ncid=ncdf_open(dum1(0))
      if dum1(0) eq '' then goto,skipmls
...
;
      for thlev=0,nth-1 do begin
          ...
          if index(0) ne -1 then height_zt_nc4(icount,thlev)=mean(z1)
      endfor
...
;
; interpolate area to MLS altitude grid
;
area_zt_nc4_alt=fltarr(kday,nlv)
for i=0,kday-1L do begin
for k=15L,nlv-1L do begin
    alt=altitude(k)
    for kk=0L,nth-2L do begin
        z1=height_zt_nc4(i,kk) & z0=height_zt_nc4(i,kk+1)
        if z0 le alt and z1 gt alt then begin
           zscale=(z1-alt)/(z1-z0)
           area_zt_nc4_alt(i,k)=area_zt_nc4(i,kk+1)+zscale*(area_zt_nc4(i,kk)-area_zt_nc4(i,kk+1))
;print,z0,alt,z1,zscale
;print,area_zt_nc4(i,kk),area_zt_nc4_alt(i,k),area_zt_nc4(i,kk+1)
        endif
    endfor
endfor
endfor
...
; save temp, co, etc
;
save,file='zt_mls_temp+co_'+yearlab+'.sav',mlspolarco_zt,mlspolartp_zt,kday,altitude,sdate_all,area_zt_nc4_alt
quick:
restore,'zt_mls_temp+co_'+yearlab+'.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co+area_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
contour,mlspolartp_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[15.,80.],$
...
;
loadct,0
area_zt_nc4_alt=smooth(area_zt_nc4_alt,3)
;contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[2.],/follow,color=0,thick=10,/overplot
;contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[5.],/follow,color=mcolor*.1,thick=10,/overplot
;contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[10.],/follow,color=mcolor*.1,thick=10,/overplot
contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[15.],/follow,color=mcolor*.1,thick=10,/overplot
contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[20.],/follow,color=mcolor*.4,thick=10,/overplot
contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[25.],/follow,color=mcolor*.8,thick=10,/overplot
contour,area_zt_nc4_alt,1.+findgen(kday),altitude,levels=[30.],/follow,color=mcolor,thick=10,/overplot
x2d=0.*area_zt_nc4_alt
y2d=0.*area_zt_nc4_alt
nlv=n_elements(altitude)
for i=0L,kday-1L do y2d(i,*)=altitude
for k=0L,nlv-1L do x2d(*,k)=1.+findgen(kday)
small=where(y2d le 70. and y2d ge 20. and x2d lt 140. and area_zt_nc4_alt gt 0. and area_zt_nc4_alt lt 2.)
oplot,x2d(small),y2d(small),psym=1,color=10         
loadct,39
;
; print end date
;
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS Average Temperature > 70!uo!n N (K)'
ybox=[0,10,10,0,0]
x1=imin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co+area_'+yearlab+'.ps -rotate -90 zt_mls_temp+co+area_'+yearlab+'.jpg'
    endif
end

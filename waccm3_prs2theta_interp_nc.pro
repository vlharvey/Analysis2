...
; Input data:  IDL> restore,'wa3_tnv3_19990506.sav
...
dirw='/aura7/harvey/WACCM_data/Datfiles/TNV3_files/'
spawn,'ls '+dirw+'wa3_tnv3_pv+elat_*.sav',ifiles
...
FOR n=0l,nfile-1l DO BEGIN
    ...
    result2=strsplit(result(5),'_',/extract)
    result3=strsplit(result2(3),'.',/extract)
    sdate=result3(0)
    restore,dirw+'wa3_tnv3_'+sdate+'.sav'
    restore,dirw+'wa3_tnv3_pv+elat_'+sdate+'.sav'
    nc=n_elements(alon)
    nr=n_elements(alat)
    nl=n_elements(lev)
    print,sdate
...
no2sfc=reform(no2(*,*,nl-1L))
h2osfc=reform(h2o(*,*,nl-1L))
qdfsfc=0.*no2sfc
pvsfc=0.*no2sfc
;
...
; LINEARLY INTERPOLATE IN THETA TO ISENTROPIC LEVEL
      ...
      NO2GRD(LAT,I,M)=NO2(I,LAT,K)+SCALE*(NO2(I,LAT,LM1)-NO2(I,LAT,K))
...
; isentropic quantities are equal to the sfc values (at psfc)
      ...
      NO2GRD(LAT,I,M)=no2sfc(i,lat)
      H2OGRD(LAT,I,M)=h2osfc(i,lat)
      endif
...
NO2GRD2=fltarr(nru,ncu,nth)
H2OGRD2=fltarr(nru,ncu,nth)
for m=0,nth-1 do begin
...
;
                   ...
                   no2j=NO2GRD(jj,ii,m)+XSCALE*(NO2GRD(jj,ip1,m)-NO2GRD(jj,ii,m))
                   h2oj=H2OGRD(jj,ii,m)+XSCALE*(H2OGRD(jj,ip1,m)-H2OGRD(jj,ii,m))
                   ...
                   no2jp1=NO2GRD(jp1,ii,m)+XSCALE*(NO2GRD(jp1,ip1,m)-NO2GRD(jp1,ii,m))
                   h2ojp1=H2OGRD(jp1,ii,m)+XSCALE*(H2OGRD(jp1,ip1,m)-H2OGRD(jp1,ii,m))
;
...
;
                   ...
                   NO2GRD2(j,i,m)=no2j+yscale*(no2jp1-no2j)
                   H2OGRD2(j,i,m)=h2oj+yscale*(h2ojp1-h2oj)
...
;
ofile='/aura7/harvey/WACCM_data/Datfiles/TNV3_files/wa3_tnv3_'+sdate+'.nc'
print,'writing ',ofile
...
no2id  = ncdf_vardef(nocid, 'NO2'       , [latdimid,londimid,levdimid])
h2oid  = ncdf_vardef(nocid, 'H2O'       , [latdimid,londimid,levdimid])
...
ncdf_varput, nocid, no2id , no2grd2, COUNT=[nru,ncu,nth]
ncdf_varput, nocid, h2oid , h2ogrd2, COUNT=[nru,ncu,nth]
ncdf_close,nocid

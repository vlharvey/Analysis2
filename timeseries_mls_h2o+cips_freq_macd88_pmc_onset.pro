...
start_date=[-27, -21, -24, -24, -26, -27, -34, -28,-33]
end_date=[66, 65, 61, 61, 64, 61, 64, 80,80]
...
for ilat=75,75 do begin		;80,5 do begin
...
;
      h2ovals=reform(mlsh2omix(*,ilev))
      index=where(abs(latitude-rlat) le 2.0 and h2ovals ne -99.)
      if index(0) ne -1L then mlspolartp_xt(icount,nn-start_year(0))=mean(h2ovals(index))
skipmls:
      icount=icount+1L
goto,jump
...
; save
;
save,file='timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.sav',mlspolartp_xt,kday,sdate_all
quick:
;yearlab='2007-2014'
;slat='50'
;salt='83'
restore,'timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.ps'
   !p.charsize=1.55
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
index=where(mlspolartp_xt le 0.)	;eq 0.)
if index(0) ne -1L then mlspolartp_xt(index)=0./0.

plot,130.+findgen(kday)-172.,smooth(mlspolartp_xt(*,0),3,/nan,/edge_truncate),/noeras,xrange=[-40.,20],yrange=[1.,7.],ytitle='MLS Water Vapor (ppmv)',$
...
xyouts,xmn+0.4,ymn+0.03,slat+'N/'+salt+'km',/normal,charsize=2,color=0,charthick=2
loadct,39
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.ps -rotate -90 timeseries_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.png'
endif
end

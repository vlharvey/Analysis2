...
waccmmean = fltarr(n_elements(alon), n_elements(altitude))
...
for ii = 0L, n_elements(alon) - 1L do begin
	for jj = 0L, n_elements(altitude) - 1L do begin
		...
		waccmmean[ii,jj] = mean(q[*,ii,jj],/nan)
	endfor
endfor
...
loadct,39
         ...
         !type=2^2+2^3
      level1 = findgen(13)*2. - 20.
      level1a = findgen(21)*1. - 15.
            level3 = findgen(1)+.2
	nlvls = n_elements(level1)
      nlvls  = n_elements(level1)
      col1 = (1 + indgen(nlvls)) * icolmax / nlvls	; define colors

          	 contour,waccmmean,lon,altitude,levels=level1,/cell_fill,c_color=col1,color=0,charsize = .8,$
                 	yticks=7,xticks=6,xtickname = ['0','60','120','180','240','300','360'],yrange=[20,100.], $
			position = [.15,.5,.5,.9], ytitle = 'Altitude (km)', xtitle = 'Longitude', title = '75!E!4'+string("157B)+ '!X!N N!CLatitude ES Composite'
  		contour,waccmmean,lon,altitude, levels=level1,color=0,charsize=1.5,$
...
for i = 0L, n_elements(lon) - 1L do begin
	...
	z = where(altitude gt 25. and altitude lt 90.)
		strat[i] = altitude[where(reform(waccmmean[i,*]) eq max(waccmmean[i,z],/nan))]
endfor
...
loadct, 39
    ; -----------------plot the color bar -----------------------
     ...
     position = [.25,.4,.8,.42],xstyle=1,xtickname=slab,$
        xtitle = '75!E!4'+string("157B)+ '!X!N N Mean!Cd' + greek('theta')+'/dt (K/day)', charsize = 1.2
...
for i=0L,n_elements(slab)-2L do begin
   ...
   if setplot eq 'ps' then begin
	...
	spawn,$
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/WACCM_q_75N_Z_lon.png'

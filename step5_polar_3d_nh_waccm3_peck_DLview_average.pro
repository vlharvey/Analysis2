;
; 3-D Arctic vortex and Anticyclones in black
;
@stddat
@kgmt
@ckday
...
!noeras=1
dir='/Volumes/earth/aura3/data/WACCM_data/Datfiles/noaurfcoaverageYear_'
;dir='/Volumes/earth/aura3/data/WACCM_data/Datfiles/noaurfplaverageYear_'
...
; --- Test for end condition and close windows.
      ...
      date=strcompress(string(FORMAT='(A3,A1,I2)',$
                              month(imn-1),' ',idy))

      ifile=string(FORMAT='(i2.2,i2.2)',imn,idy)+'.nc3'
...
sdate=string(FORMAT='(i2.2,i2.2)',imn,idy)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/wa3_peck_avg_'+sdate+'.ps'
       device,/color
...
; pressure of theta surface
	...
	avgz=result(0)
        if avgz lt 0.1 then savgz=strcompress(string(FORMAT='(F4.2)',avgz))
        if avgz ge 0.1 and avgz lt 1. then savgz=strcompress(string(FORMAT='(F3.1)',avgz))
        if avgz ge 1. and avgz lt 10. then savgz=strcompress(string(FORMAT='(i1)',long(avgz)))
        if avgz ge 10. and avgz lt 100. then savgz=strcompress(string(FORMAT='(i2)',long(avgz)))
        if avgz ge 100. then savgz=strcompress(string(FORMAT='(i3)',long(avgz)))
print,th(lev),avgz,savgz
...
; anticyclones
;
        lindex=where(dum lt -9999.,nl)	; 0.0,nl)
        if lindex(0) ne -1 then begin
...
dx=x2d(1,0)-x2d(0,0)
           for ihigh=0,nhigh-1 do begin
               ...
               index=where(tmp ne -9999. and y2d gt 3.75 and dum eq -1.0*(ihigh+1))
               if index(0) ne -1L then oplot,xcn(index),ycn(index),psym=8,color=0,/T3D,zvalue=nz,symsize=2
...
jumplev:
        if kk mod 2 eq 0 then xyouts,.05,nz4,savgz,color=0,/normal,charsize=3,charthick=2
;       xyouts,.08,nz4,thlevs(kk),color=0,/normal,charsize=2,charthick=2
    endfor	; loop over stacked polar plots
    !psym=0
    xyouts,0.4,0.88,date,/normal,charsize=5,color=0,charthick=2
;   xyouts,.08,.8,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    xyouts,.02,.8,'Pressure (hPa)',charsize=2.5,/normal,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/wa3_peck_avg_'+sdate+'.ps -rotate -90 Figures/wa3_peck_avg_'+sdate+'.jpg'
       spawn,'rm -f Figures/wa3_peck_avg_'+sdate+'.ps'
    endif

...
;
      ...
      for j=0L,nlat-1L do begin
          utz(icount,k,j)=total(t3d(*,j,k))/float(nlg)
      endfor
      endfor
...
saveit:
save,file='zt_geos5_tbarp.sav',utz,alat,nlat,kday,sdates,altitude
plotit:
restore,'zt_geos5_tbarp.sav'
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_geos5_tbarp_'+slat+'.ps'
   device,/color
...
contour,utzlat,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,max(altitude)],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='GEOS-5 Tbar at '+slat+' Latitude',/fill,c_color=col1,$
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_geos5_tbarp_'+slat+'.ps -rotate -90 zt_geos5_tbarp_'+slat+'.jpg'
endif

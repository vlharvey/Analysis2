...
ifiles=file_search(dirw+'press_2018??????.sav')			; works for 2018
...
sdate=result2(0)
print,sdate
dum=findfile(dirw+'theta_'+sdate+'.nc3')	; skip if final nc3 file exists
...
; 550.00000       600.00000       650.00000       700.00000       725.00000       750.00000       775.00000       800.00000       825.00000       850.00000       875.00000       900.00000       925.00000       950.00000       975.00000       1000.0000
;IDL> print,pressure
...
; save memory
;
tgrd=0 & ugrd=0 & vgrd=0 & pgrd=0 & zgrd=0 & qvgrd=0 & o3grd=0
;
...
; LINEARLY INTERPOLATE IN THETA TO ISENTROPIC LEVEL
      ...
      O3GRD(LAT,I,M)=O3L(I,LAT,K)+SCALE*(O3L(I,LAT,LM1)-O3L(I,LAT,K))
...
;if index(0) ne -1L then contour,pp,alon,alat,levels=level(index),/follow,c_color=mcolor,c_linestyle=5,/noeras,/overplot,thick=3
;index=where(level gt 0.)
;if index(0) ne -1L then contour,pp,alon,alat,levels=level(index),/follow,c_color=0,/noeras,/overplot,thick=3
;endfor
;
; write theta file
;
...
print,'writing ',ofile
save,file=ofile,nr,nc,nth,alon,alat,thlev,ipvgrd,pgrd,ugrd,vgrd,zgrd,qdfgrd,qvgrd,qgrd,o3grd

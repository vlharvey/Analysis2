...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Antarctic_3D/'+ifile+'_3D+McMurdo.ps'
       device,/color
...
yy=slat & xx=slon
               ...
               yn = -1.0 * FACTOR * SIN(THETA0)
               a=findgen(8)*(2*!pi/8.)
               usersym,2*cos(a),2*sin(a),/fill
               oplot,[xn,xn],[yn,yn],zvalue=nz0(kk),/T3D,psym=8,color=250
               dist=nz0(kp1)-nz0(kk)
...
;   xyouts,.08,.85,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Antarctic_3D/'+ifile+'_3D+McMurdo.ps -rotate -90 Antarctic_3D/'+ifile+'_3D+McMurdo.jpg'
       spawn,'rm -f Antarctic_3D/'+ifile+'_3D+McMurdo.ps'
    endif

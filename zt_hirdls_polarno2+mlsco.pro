...
save,file='zt_hirdls_polarno2+mlsco_2007.sav',polart_zt,polarno2_zt,mlspolarco_zt,kday,altitude,sdate_all
quick:
restore,'zt_hirdls_polarno2+mlsco_2007.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_hirdls_polarno2+mlsco_'+syear(0)+'.ps'
   device,/color
...
contour,polarno2_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[25.,57.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='HIRDLS Avg NO!l2!n and MLS CO > 60 N',/fill,c_color=col1,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_hirdls_polarno2+mlsco_'+syear(0)+'.ps -rotate -90 zt_hirdls_polarno2+mlsco_'+syear(0)+'.jpg'
    endif
end

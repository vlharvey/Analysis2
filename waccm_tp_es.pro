...
print,'reading ',strmid(sdate,2,2),smn+sdy
        ncfile0=dir+strmid(sdate,2,2)+'_'+smn+sdy+'_TP.sav'
        ncfile1=dir+strmid(sdate,2,2)+'_'+smn+sdy+'_CO.sav'
        restore,ncfile0
        restore,ncfile1
      ...
      if icount eq 0 then begin
         ...
         nr=n_elements(lat)
         sfile=strarr(kday)
         tp_zt_nc4=fltarr(kday,nr,nth)
         co_zt_nc4=fltarr(kday,nr,nth)
         kcount=1L
      endif
      sfile(icount)=sdate
;
...
; save file
;
save,file=diro+'vortex_tp_ES_'+esdate0+'_waccm.sav',tp_zt_nc4,lat,lev,sfile,y1,y2,koff
index=where(tp_zt_nc4 le 0.)
if index(0) ne -1L then tp_zt_nc4(index)=0./0.

plotit:
restore,diro+'vortex_tp_ES_'+esdate0+'_waccm.sav'
...
tp_zt_nc4_save=tp_zt_nc4
tp_zt_nc4_plot=reform(tp_zt_nc4(*,ilat,*))
tp_zt_nc4=tp_zt_nc4_plot
co_zt_nc4=reform(co_zt_nc4(*,ilat,*))
;
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/times,filename='../Figures/waccm_vortex_tp_ES_event_'+sevent+'_'+slat+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
!type=2^2+2^3
level=180+5.*findgen(20)
nlvls=n_elements(level)
col1=1+indgen(nlvls)*icolmax/nlvls
index=where(tp_zt_nc4 eq 0.)
if index(0) ne -1L then tp_zt_nc4(index)=0./0.
index=where(co_zt_nc4 eq 0.)
if index(0) ne -1L then co_zt_nc4(index)=0./0.
tp_zt_nc4=smooth(tp_zt_nc4,5,/nan)
contour,tp_zt_nc4,-1.*koff+findgen(kday),lev,color=0,xtitle='Days From ES Onset',thick=6,yrange=[10.,0.0001],/ylog,/noeras,ytitle='Pressure (hPa)',/cell_fill,$
     title='WACCM ES event '+sevent+' ('+esdate0+') at '+slat,levels=level,c_color=col1
contour,tp_zt_nc4,-1.*koff+findgen(kday),lev,levels=level,c_color=mcolor,/follow,/overplot,c_labels=1+0*level,min_value=0
contour,co_zt_nc4*1.e6,-1.*koff+findgen(kday),lev,levels=[0.0001,0.001,0.01,0.1,1.,10.],c_color=0,/follow,/overplot,c_labels=1+0*level,min_value=0,thick=2
;plots,0,500
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,$
      xtitle='Temperature (K)',charthick=2,charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ../Figures/waccm_vortex_tp_ES_event_'+sevent+'_'+slat+'.ps -rotate -90 ../Figures/waccm_vortex_tp_ES_event_'+sevent+'_'+slat+'.jpg'
endif

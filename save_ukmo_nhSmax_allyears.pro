;
; save Smax poleward of 30N for all levels and years
;
@stddat
@kgmt
@ckday
...
;
    if icount eq 0L then begin
       nh_smax_all=fltarr(nyear,kday,nth)
...
;
    ...
    for thlev=0,nth-1 do begin
        speed=transpose(s2(*,*,thlev))
        index=where(lat gt 20.)
        nh_smax_all(myr,iday,thlev)=max(speed(index))
    endfor
goto,jump
saveit:
save,file='ukmo_nhSmax_allyears.sav',nh_smax_all,th,yyyymmdd,doy
end

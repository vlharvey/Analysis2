...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_geos4_ubarth+mark_'+sdate+'.ps'
   device,/color
...
contour,tzm,alat,th,/noeras,xrange=[-90.,90.],yrange=[min(th),max(th)],charsize=1.5,color=0,$
      ytitle='Theta (K)',title='GEOS-4 Zonal Mean Temperature '+sdate,xticks=6,/fill,c_color=col1,$
      levels=level
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_geos4_ubarth+mark_'+sdate+'.ps -rotate -90 yz_geos4_ubarth+mark_'+sdate+'.jpg'
endif

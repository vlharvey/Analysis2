...
;
lstmn=5L & lstdy=10L & lstyr=6L
ledmn=6L & leddy=1L & ledyr=6L
lstday=0L & ledday=0L 
...
;
      if result1(0) eq 0L or result2(0) eq 0L or result4(0) eq 0L or $
...
;
      bro=readl2gp(brofiles(0),swathName='BrO',variableName=variableName,precisionName=precisionName)
...
;
      ...
      hclmask=0.*hclmls
      clomls=0.*transpose(clo.L2GPVALUE)
      cloprecision=0.*transpose(clo.L2GPPRECISION)
      clostatus=0.*clo.STATUS
      cloquality=0.*clo.QUALITY
      clomask=0.*clomls
...
;
      ...
      if tpbad(0) ne -1L then tpmask(tpbad,*)=-99.
;
; impose quality limits
;
      brobad=where(broquality lt 1.0)			; do not use if quality < 1
      if brobad(0) ne -1L then bromask(brobad,*)=-99.
...
; (zprof begins at the ground, "altitude" array is top-down)
;
          ...
          for k=0L,nz-1L do begin           
              ...
              for kk=0L,mlev-2L do begin
                  ...
                  if z0 le zp and z1 ge zp then begin
                     ...
                     if bromaskprof(kk+1L) ne -99. and bromaskprof(kk+1L) ne -99. then $
                     ...
                     if o3maskprof(kk+1L) ne -99. and o3maskprof(kk+1L) ne -99. then $
                     ...
                     if comaskprof(kk+1L) ne -99. and comaskprof(kk+1L) ne -99. then $
                     ...
                     if hclmaskprof(kk+1L) ne -99. and hclmaskprof(kk+1L) ne -99. then $
                     ...
                     if clomaskprof(kk+1L) ne -99. and clomaskprof(kk+1L) ne -99. then $
                     ...
                     if h2omaskprof(kk+1L) ne -99. and h2omaskprof(kk+1L) ne -99. then $
                     ...
                     if hno3maskprof(kk+1L) ne -99. and hno3maskprof(kk+1L) ne -99. then $
                     ...
                     if n2omaskprof(kk+1L) ne -99. and n2omaskprof(kk+1L) ne -99. then $
                     ...
                     if tpmaskprof(kk+1L) ne -99. and tpmaskprof(kk+1L) ne -99. then $
...
; check
;
;plot,tpprof,zprof,yrange=[0.,130.],xrange=[180.,400.],thick=2
;oplot,tpgrid(i,*),altitude,psym=2
;plot,pprof,zprof,/xlog,yrange=[0.,130.],xrange=[0.01,1000.],thick=2
;oplot,pgrid(i,*),altitude,psym=2
...
;
      ...
      id=syyyymmdd+'.'+sprof
      catfile=odir+'cat_mls_'+sdate+'.sav'
...
;
      rawfile=odir+'raw_mls_'+sdate+'.sav'
      save,file=rawfile,pmls,mtime,bromls,broprecision,brostatus,broquality,o3mls,o3precision,o3status,$
           o3quality,gpmls,gpprecision,gpstatus,gpquality,comls,coprecision,costatus,coquality,$
           hclmls,hclprecision,hclstatus,hclquality,clomls,cloprecision,clostatus,cloquality,$
           h2omls,h2oprecision,h2ostatus,h2oquality,hno3mls,hno3precision,hno3status,hno3quality,$
           n2omls,n2oprecision,n2ostatus,n2oquality,tpmls,tpprecision,tpstatus,tpquality
; 
...
;
      ...
      mask=bromaskgrid
      save,file=odir+'bro_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; Ozone
;
      mix=o3grid
      ...
      mask=o3maskgrid
      save,file=odir+'o3_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; Carbon Monoxide
;
      mix=cogrid
      ...
      mask=comaskgrid
      save,file=odir+'co_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; HCl
;
      mix=hclgrid
      ...
      mask=hclmaskgrid
      save,file=odir+'hcl_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; ClO
;
      mix=clogrid
      ...
      mask=clomaskgrid
      save,file=odir+'clo_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; Water vapor
;
      mix=h2ogrid
      ...
      mask=h2omaskgrid
      save,file=odir+'h2o_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; Nitric Acid
;
      mix=hno3grid
      ...
      mask=hno3maskgrid
      save,file=odir+'hno3_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
; Nitrous Oxide
;
      mix=n2ogrid
      ...
      mask=n2omaskgrid
      save,file=odir+'n2o_mls_'+sdate+'.sav',altitude,id,mix,err,mask,comment
;
...
;     dentot= (100.*pressure*av)/(gasconst*temperature*1.e6)
      ...
      tpdfile=odir+'tpd_mls_'+sdate+'.sav'

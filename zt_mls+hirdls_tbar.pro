...
if index(0) ne -1L then tbar_zt_mls(index)=0./0.
...
altitude_hirdls=altitude
TBAR_ZT_hirdls=TBAR_ZT
SDATE_ALL_hirdls=SDATE_ALL
index=where(tbar_zt_hirdls eq 0.)
if index(0) ne -1L then tbar_zt_hirdls(index)=0./0.
tbar_zt_hirdls=smooth(tbar_zt_hirdls,3,/NaN)
index=where(finite(tbar_zt_hirdls) ne 1L)
if index(0) ne -1L then tbar_zt_hirdls(index)=-999.
;
; loop over latitude
;
...
for j=29L,34L do begin
    ...
    if setplot eq 'ps' then begin
       ...
       device,font_size=9
       device,/landscape,bits=8,filename='zt_mls+hirdls_tbar_'+slat+'.ps'
       device,/color
...
tzlat=transpose(reform(tbar_zt_mls(j,*,*)))
print,slat,' ',min(tbar_zt_mls),max(tbar_zt_mls)
;
; hardwire xticks
;
...
set_viewport,xmn,xmx,ymn,ymx
!type=2^2+2^3+2^7
tzlatg=transpose(reform(tbar_zt_hirdls(j,*,*)))
syear=strmid(sdate_all_hirdls,0,4)
smon=strmid(sdate_all_hirdls,4,2)
sday=strmid(sdate_all_hirdls,6,2)
...
contour,tzlatg,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[20.,60.],$
      charsize=1.5,color=0,title='HIRDLS',/fill,c_color=col1,$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=-99.
index=where(level mod 10 eq 0.)
...
pdiff(index)=tzlat(index)-tzlatg(index)
...
contour,pdiff,1.+findgen(kday),altitude,xrange=[1.,kday],yrange=[20.,60.],$
        ...
        title='MLS - HIRDLS',c_color=col2,color=0,min_value=-99.,$
...
contour,tzlat,1.+findgen(kday),altitude,/overplot,levels=[230.,250,260],color=0,thick=6
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
ymxb=ymnb+cbarydel
...
jump:
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls+hirdls_tbar_'+slat+'.ps -rotate -90 zt_mls+hirdls_tbar_'+slat+'.jpg'
    endif

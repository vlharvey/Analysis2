...
;
      good=where(CLOUD_INDEX_ALL20 eq 1. and ALB_ALL20 gt ALBLIM and RAD_ALL20 ge 20. and SZA_ALL20 GE SZALIM_LO and SZA_ALL20 le SZALIM_HI,ngood)
      freq20(icount)=100.*float(ngood)/float(n_elements(CLOUD_INDEX_ALL20))
      good=where(CLOUD_INDEX_ALL51 eq 1. and ALB_ALL51 gt ALBLIM and RAD_ALL51 ge 20. and SZA_ALL51 GE SZALIM_LO and SZA_ALL51 le SZALIM_HI,ngood)
...
;
      good=WHERE(FINITE(ALB_ALL20) EQ 1 and ALB_ALL20 gt ALBLIM and RAD_ALL20 ge 20. and SZA_ALL20 ge SZALIM_LO and SZA_ALL20 le SZALIM_HI and CLOUD_INDEX_ALL20 EQ 1,ngood)
...
;     cutdate20=ut_date_all20(good)
      ...
      good=WHERE(FINITE(ALB_ALL51) EQ 1 and ALB_ALL51 gt ALBLIM and RAD_ALL51 ge 20. and SZA_ALL51 ge SZALIM_LO and SZA_ALL51 le SZALIM_HI and CLOUD_INDEX_ALL51 eq 1,ngood) 
      if good(0) eq -1L then goto,skipcips
...
amax=130.
nlev=64L        ;100L
abin=2. ;(amax-alblim)/float(nlev)
alevs=alblim+abin*findgen(nlev)
y20=histogram(a20,min=alblim,max=amax,binsize=abin)/float(n_elements(a20))
y51=histogram(a51,min=alblim,max=amax,binsize=abin)/float(n_elements(a51))
plot,alevs,y20,thick=10,color=0,/ylog,yrange=[1.e-6,max([y20,y51])],title=sdate		; normalized
;plot,alevs,y20,thick=10,color=0,/ylog,yrange=[1,max([y20,y51])],title=sdate
...
plotyear:
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/helvetica,filename='compare_cips_l4_v4.20_v5.10_rawfiles_Jul_'+syear+'.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
!type=2^2+2^3
set_viewport,.1,.9,.1,.9
;amax=max([alb20,alb51])
amax=130.
nlev=64L	;100L
abin=2.	;(amax-alblim)/float(nlev)
alevs=alblim+abin*findgen(nlev)
y20=histogram(alb20,min=alblim,max=amax,binsize=abin)	;/float(n_elements(alb20))
y51=histogram(alb51,min=alblim,max=amax,binsize=abin)	;/float(n_elements(alb51))
;plot,alevs,y20,thick=10,color=0,/ylog,yrange=[1.e-6,max([y20,y51])],title=sdate
plot,alevs,y20,thick=10,color=0,/ylog,yrange=[1,max([y20,y51])],title=sdate
oplot,alevs,y51,thick=8,color=250
stop
    if setplot ne 'ps' then stop
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim compare_cips_l4_v4.20_v5.10_rawfiles_Jul_'+syear+'.ps -rotate -90 compare_cips_l4_v4.20_v5.10_rawfiles_Jul_'+syear+'.jpg'
;        spawn,'rm -f compare_cips_l4_v4.20_v5.10_rawfiles_Jul_'+syear+'.ps'
      endif

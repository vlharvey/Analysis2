...
print,'MLS ',ith,th(ith)
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='timeseries_y0_3pan_'+sth+'.ps'
             device,/color
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=smooth(reform(y0_allyears(*,iyear)),3,/nan)
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[40,90],color=0,charsize=2,charthick=2,xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=smooth(reform(y0_allyearsmerra(*,iyear)),3,/nan)
    arr1=reform(y0_allyearsmerra(*,iyear))
index=where(finite(arr1) eq 1)
if index(0) ne -1L then begin
   kindex=where(arr1 eq max(arr1(index)))
endif
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmeanmerra,yrange=[40,90],color=0,charsize=2,charthick=2,ytitle=sth+' Vortex Centroid Latitude',xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
;          if fdoy(i) ge startday and fdoy(i) lt max(fdoy) then begin
...
for iyear=0L,nyear-1L do begin
    ...
    arr1=smooth(reform(y0_allyearsmls(*,iyear)),3,/nan)
    arr1=reform(y0_allyearsmls(*,iyear))
index=where(finite(arr1) eq 1)
if index(0) ne -1L then begin
   kindex=where(arr1 eq max(arr1(index)))
endif
...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmeanmls,yrange=[40,90],color=0,charsize=2,charthick=2,xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=15,/noeras,/nodata
;      for i=0L,nn-1L do begin
;          if fdoy(i) ge startday and fdoy(i) lt max(fdoy) then begin
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert timeseries_y0_3pan_'+sth+'.ps -rotate -90 timeseries_y0_3pan_'+sth+'.jpg'
endif

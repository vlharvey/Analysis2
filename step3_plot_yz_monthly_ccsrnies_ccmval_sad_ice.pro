;
; read multi-year monthly means of Surface Area Density of Ice and plot zonal mean
;
a=findgen(8)*(2*!pi/8.)
...
dir='/aura7/harvey/CCMval_data/Datfiles/CCMVal2_REF-B1_CCSRNIES_1_T3I_sad_ice_'
...
for imon=0L,nmon-1L do begin
    ...
    print,'read CCMVal2_REF-B1_CCSRNIES_1_T3I_sad_ice_'+smon(imon)+'.sav'
;
; compute zonal mean
;
    nr=n_elements(alat)
...
;
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='step3_plot_yz_monthly_ccsrnies_ccmval_sad_ice_'+smon(imon)+'.ps'
...
wsadzm=wsadzm*1.e8
    contour,wsadzm,alat,lev*1000.,/ylog,c_color=col1,/cell_fill,title='CCSRNIES SAD ICE '+smon(imon),$
            xrange=[-90.,90.],yrange=[500.,5.],charsize=1.5,ytitle='Sigma',xtitle='Latitude',color=0,levels=level
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim step3_plot_yz_monthly_ccsrnies_ccmval_sad_ice_'+smon(imon)+'.ps -rotate -90 '+$
                            'step3_plot_yz_monthly_ccsrnies_ccmval_sad_ice_'+smon(imon)+'.jpg'
        spawn,'/usr/bin/rm step3_plot_yz_monthly_ccsrnies_ccmval_sad_ice_'+smon(imon)+'.ps'
     endif

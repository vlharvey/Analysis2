...
dir5='/aura7/harvey/GEOS5_data/Datfiles/DAS.ops.asm.tavg3d_dyn_v.GEOS510.MetO.'
...
; read new vortex
;
            ncid=ncdf_open(dir5+sdate+'_1200.V01.nc5')
            marknew2=fltarr(nr,nc,nth)
            ncdf_varget,ncid,3,marknew2
            ncdf_close,ncid
         endif
...
;
    ...
    for k=0,nth-1 do begin
        ...
        spyz(0:nr/2-1,k)=reverse(sp2(0:nr/2-1,ilon1,k))
        spyz(nr/2:nr-1,k)=sp2(0:nr/2-1,ilon2,k)
        ...
        lyz(0:nr/2-1,k)=reverse(mark2(0:nr/2-1,ilon1,k))
        lyz(nr/2:nr-1,k)=mark2(0:nr/2-1,ilon2,k)
        lnewyz(0:nr/2-1,k)=reverse(marknew2(0:nr/2-1,ilon1,k))
        lnewyz(nr/2:nr-1,k)=marknew2(0:nr/2-1,ilon2,k)
        hyz(0:nr/2-1,k)=reverse(mark2(0:nr/2-1,ilon1,k))
        hyz(nr/2:nr-1,k)=mark2(0:nr/2-1,ilon2,k)
        smarkyz(0:nr/2-1,k)=reverse(smark2(0:nr/2-1,ilon1,k))
        smarkyz(nr/2:nr-1,k)=smark2(0:nr/2-1,ilon2,k)
    endfor
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='Figures/yz_cross_pole_geos_sh_'+sdate+'_'+sth+'_'+slon1+'.ps'
...
;    contour,sp,x,alat,levels=level(index),/overplot,/follow,color=icolmax,c_linestyle=5
    ...
    pmin=-0.3
    pmax=0.
    pvlevel=pmin+((pmax-pmin)/nlvls)*findgen(nlvls)
...
;drawvectors,nc+1,nr,x,alat,u,v,3,0
    ...
    contour,lnew,x,alat,levels=[0.1],color=icolmax*.1,thick=8,/overplot
;   contour,h,x,alat,levels=[-0.1],color=icolmax*.9,thick=8,/overplot
    loadct,38
;   contour,smark,x,alat,levels=[0.1],color=icolmax*.9,thick=8,/overplot
...
;if index(0) ne -1L then oplot,x2d(index),y2d(index),color=.9*icolmax,psym=8
    ...
    contour,tyz,alat,th,levels=tlevel,/fill,/cell_fill,c_color=col1,$
            ...
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,title='Temperature+Vortex'
...
;    if setplot ne 'ps' then stop
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim Figures/yz_cross_pole_geos_sh_'+sdate+'_'+sth+'_'+slon1+'.ps -rotate -90 '+$
                            'Figures/yz_cross_pole_geos_sh_'+sdate+'_'+sth+'_'+slon1+'.jpg'
        spawn,'/usr/bin/rm Figures/yz_cross_pole_geos_sh_'+sdate+'_'+sth+'_'+slon1+'.ps'
     endif

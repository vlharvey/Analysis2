...
; Input data:  IDL> restore,'../Datfiles/h0.lynn+dust.ddd_00Z.sav
...
;		QGRD            FLOAT     = Array[72, 46, 66]
;		DUSTADGRD       FLOAT     = Array[72, 46, 66]
;		DUSTMDGRD       FLOAT     = Array[72, 46, 66]
;		DUSTMMRGRD      FLOAT     = Array[72, 46, 66]
;		DUSTNDGRD       FLOAT     = Array[72, 46, 66]
;		DUSTREGRD       FLOAT     = Array[72, 46, 66]
;		
...
dirw='/aura3/data/WACCM_data/Datfiles/'
spawn,'ls '+dirw+'h0.lynn+dust.*.sav',ifiles
...
FOR n=0l,nfile-1l DO BEGIN
    ...
    result3=strsplit(result2(0),'.',/extract)
    sdate=result3(2)
    restore,dirw+'h0.lynn+dust.'+sdate+'_00Z.sav'
    nc=n_elements(alon)
    nr=n_elements(alat)
    result=size(tgrd)
    nl=result(3)
...
dustadsfc=reform(dustadl(*,*,nl-1L))
dustmdsfc=reform(dustmdl(*,*,nl-1L))
dustmmrsfc=reform(dustmmrl(*,*,nl-1L))
dustndsfc=reform(dustndl(*,*,nl-1L))
dustresfc=reform(dustrel(*,*,nl-1L))
qdfsfc=0.*tsfc
pvsfc=0.*tsfc
;
...
dustadgrd=fltarr(nr,nc,nth)
dustmdgrd=fltarr(nr,nc,nth)
dustmmrgrd=fltarr(nr,nc,nth)
dustndgrd=fltarr(nr,nc,nth)
dustregrd=fltarr(nr,nc,nth)
;
; LOOP OVER LATITUDES
;
pv=0.*tl
...
; LINEARLY INTERPOLATE IN THETA TO ISENTROPIC LEVEL
      ...
      DUSTADGRD(LAT,I,M)=DUSTADL(I,LAT,K)+SCALE*(DUSTADL(I,LAT,LM1)-DUSTADL(I,LAT,K))
      DUSTMDGRD(LAT,I,M)=DUSTMDL(I,LAT,K)+SCALE*(DUSTMDL(I,LAT,LM1)-DUSTMDL(I,LAT,K))
      DUSTMMRGRD(LAT,I,M)=DUSTMMRL(I,LAT,K)+SCALE*(DUSTMMRL(I,LAT,LM1)-DUSTMMRL(I,LAT,K))
      DUSTNDGRD(LAT,I,M)=DUSTNDL(I,LAT,K)+SCALE*(DUSTNDL(I,LAT,LM1)-DUSTNDL(I,LAT,K))
      DUSTREGRD(LAT,I,M)=DUSTREL(I,LAT,K)+SCALE*(DUSTREL(I,LAT,LM1)-DUSTREL(I,LAT,K))
      ENDIF
...
; isentropic quantities are equal to the sfc values (at psfc)
      ...
      DUSTADGRD(LAT,I,M)=dustadsfc(i,lat)
      DUSTMDGRD(LAT,I,M)=dustmdsfc(i,lat)
      DUSTMMRGRD(LAT,I,M)=dustmmrsfc(i,lat)
      DUSTNDGRD(LAT,I,M)=dustndsfc(i,lat)
      DUSTREGRD(LAT,I,M)=dustresfc(i,lat)
      endif
...
DUSTADGRD2=fltarr(nru,ncu,nth)
DUSTMDGRD2=fltarr(nru,ncu,nth)
DUSTMMRGRD2=fltarr(nru,ncu,nth)
DUSTNDGRD2=fltarr(nru,ncu,nth)
DUSTREGRD2=fltarr(nru,ncu,nth)
for m=0,nth-1 do begin
...
;
                   ...
                   qj=QGRD(jj,ii,m)+XSCALE*(QGRD(jj,ip1,m)-QGRD(jj,ii,m))
                   d1j=DUSTADGRD(jj,ii,m)+XSCALE*(DUSTADGRD(jj,ip1,m)-DUSTADGRD(jj,ii,m))
                   d2j=DUSTMDGRD(jj,ii,m)+XSCALE*(DUSTMDGRD(jj,ip1,m)-DUSTMDGRD(jj,ii,m))
                   d3j=DUSTMMRGRD(jj,ii,m)+XSCALE*(DUSTMMRGRD(jj,ip1,m)-DUSTMMRGRD(jj,ii,m))
                   d4j=DUSTNDGRD(jj,ii,m)+XSCALE*(DUSTNDGRD(jj,ip1,m)-DUSTNDGRD(jj,ii,m))
                   d5j=DUSTREGRD(jj,ii,m)+XSCALE*(DUSTREGRD(jj,ip1,m)-DUSTREGRD(jj,ii,m))
                   ...
                   qjp1=QGRD(jp1,ii,m)+XSCALE*(QGRD(jp1,ip1,m)-QGRD(jp1,ii,m))
                   d1jp1=DUSTADGRD(jp1,ii,m)+XSCALE*(DUSTADGRD(jp1,ip1,m)-DUSTADGRD(jp1,ii,m))
                   d2jp1=DUSTMDGRD(jp1,ii,m)+XSCALE*(DUSTMDGRD(jp1,ip1,m)-DUSTMDGRD(jp1,ii,m))
                   d3jp1=DUSTMMRGRD(jp1,ii,m)+XSCALE*(DUSTMMRGRD(jp1,ip1,m)-DUSTMMRGRD(jp1,ii,m))
                   d4jp1=DUSTNDGRD(jp1,ii,m)+XSCALE*(DUSTNDGRD(jp1,ip1,m)-DUSTNDGRD(jp1,ii,m))
                   d5jp1=DUSTREGRD(jp1,ii,m)+XSCALE*(DUSTREGRD(jp1,ip1,m)-DUSTREGRD(jp1,ii,m))
;
...
;
                   ...
                   QGRD2(j,i,m)=qj+yscale*(qjp1-qj)
                   DUSTADGRD2(j,i,m)=d1j+yscale*(d1jp1-d1j)
                   DUSTMDGRD2(j,i,m)=d2j+yscale*(d2jp1-d2j)
                   DUSTMMRGRD2(j,i,m)=d3j+yscale*(d3jp1-d3j)
                   DUSTNDGRD2(j,i,m)=d4j+yscale*(d4jp1-d4j)
                   DUSTREGRD2(j,i,m)=d5j+yscale*(d5jp1-d5j)
...
;
ofile='/aura3/data/WACCM_data/Datfiles/h0.lynn+dust.'+sdate+'_00Z.nc'
print,'writing ',ofile
...
qid    = ncdf_vardef(nocid, 'spec_hum'  , [latdimid,londimid,levdimid])
d1id   = ncdf_vardef(nocid, 'dust_ad'   , [latdimid,londimid,levdimid])
d2id   = ncdf_vardef(nocid, 'dust_md'   , [latdimid,londimid,levdimid])
d3id   = ncdf_vardef(nocid, 'dust_mmr'  , [latdimid,londimid,levdimid])
d4id   = ncdf_vardef(nocid, 'dust_nd'   , [latdimid,londimid,levdimid])
d5id   = ncdf_vardef(nocid, 'dust_re'   , [latdimid,londimid,levdimid])
...
ncdf_varput, nocid, qid   , qgrd2  , COUNT=[nru,ncu,nth]
ncdf_varput, nocid, d1id  , dustadgrd2  , COUNT=[nru,ncu,nth]
ncdf_varput, nocid, d2id  , dustmdgrd2  , COUNT=[nru,ncu,nth]
ncdf_varput, nocid, d3id  , dustmmrgrd2 , COUNT=[nru,ncu,nth]
ncdf_varput, nocid, d4id  , dustndgrd2  , COUNT=[nru,ncu,nth]
ncdf_varput, nocid, d5id  , dustregrd2  , COUNT=[nru,ncu,nth]
ncdf_close,nocid

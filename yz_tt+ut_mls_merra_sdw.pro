;
; plot zonal mean daily temperature and Ubar tendency from MLS, MERRA, and SD-WACCM
;
@stddat
@kgmt
@ckday
...
for n=0L+7L,nday-1L-7L do begin
    ...
    print,sdate
;
; zonal mean temperature and winds the week prior to today
;
    ...
    mubar0=fltarr(n_elements(mlat),n_elements(pmls2))
    meubar0=fltarr(n_elements(latitude_waccm),n_elements(pressure))
    wubar0=fltarr(n_elements(lat),n_elements(lev))
    ...
    for nn=n-7L,n-1L do begin
        ...
        mubar0=mubar0+reform(mlsut(nn,*,*))
        meubar0=meubar0+reform(merraut(nn,*,*))
        wubar0=wubar0+reform(sdwut(nn,*,*))
...
;
    ...
    mubar1=fltarr(n_elements(mlat),n_elements(pmls2))
    meubar1=fltarr(n_elements(latitude_waccm),n_elements(pressure))
    wubar1=fltarr(n_elements(lat),n_elements(lev))
    ...
    for nn=n,n+7L-1L do begin
        ...
        mubar1=mubar1+reform(mlsut(nn,*,*))
        meubar1=meubar1+reform(merraut(nn,*,*))
        wubar1=wubar1+reform(sdwut(nn,*,*))
...
mlsubar0=reform(mlsut(n,*,*))
sdwubar0=reform(sdwut(n,*,*))
...
;
        if setplot eq 'ps' then begin
           ...
           device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                  /bold,/color,bits_per_pixel=8,/helvetica,filename='yz_tt+ut_mls_merra_sdw_'+sdate+'.ps'
           !p.charsize=1.25
           !p.thick=2
           !p.charthick=5
           !y.thick=2
...
print,min(mlstbar),max(mlstbar)
       ...
       contour,mlsubar,mlat,pmls2,levels=level(index),/overplot,/noeras,c_color=0,/follow,thick=2
       index=where(level lt 0.)
       contour,mlstbar,mlat,pmls2,levels=level(index),/overplot,/noeras,color=mcolor,/follow,c_linestyle=5
       contour,mlsubar,mlat,pmls2,levels=level(index),/overplot,/noeras,color=mcolor,/follow,c_linestyle=5,thick=2
...
print,min(sdwtbar),max(sdwtbar)
       ...
       contour,sdwubar,lat,lev,levels=level(index),/overplot,/noeras,c_color=0,/follow,thick=2
       index=where(level lt 0.)
       contour,sdwtbar,lat,lev,levels=level(index),/overplot,/noeras,color=mcolor,/follow,c_linestyle=5
       contour,sdwubar,lat,lev,levels=level(index),/overplot,/noeras,color=mcolor,/follow,c_linestyle=5,thick=2
...
; Close PostScript file and return control to X-windows
       ...
       if setplot eq 'ps' then begin
          device, /close
          spawn,'convert -trim yz_tt+ut_mls_merra_sdw_'+sdate+'.ps -rotate -90 '+$
                              'yz_tt+ut_mls_merra_sdw_'+sdate+'.jpg'
          spawn,'rm -f yz_tt+ut_mls_merra_sdw_'+sdate+'.ps'
       endif

;
; Hovmoller of MLS water vapor 1 Nov to 1 Jan
...
; save 
;
save,file='xt_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.sav',mlspolarh2o_xt,kday,longrid,sdate_all
quick:
yearlab=string(format='(i4)',iyear)+'-'+strcompress(iyear+1,/remove_all)
;restore,'xt_mls_h2o_'+yearlab+'_'+slat+'_pmc_onset.sav'
if iyear eq 2013 then restore,'xt_mls_h2o_2013_'+slat+'_pmc_onset.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_mls_h2o+cips_freq_'+yearlab+'_'+slat+'_pmc_onset.ps'
   !p.charsize=1.55
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
xyouts,xmn+0.01,ymx-0.05,yearlab+'/'+slat+'S/'+salt+'km',/normal,charsize=2.5,color=mcolor,charthick=2
loadct,39
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS Water Vapor (ppmv)',charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_mls_h2o+cips_freq_'+yearlab+'_'+slat+'_pmc_onset.ps -rotate -90 xt_mls_h2o+cips_freq_'+yearlab+'_'+slat+'_pmc_onset.png'
    endif

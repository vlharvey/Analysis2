...
; --- Test for end condition and close windows.
      ...
      if ndays gt ledday then begin
         save,file='zt_height_waccm_Traject_Season_70-90N.sav',/all
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_waccm_Traject_Season_6pan_'+sdate+'.ps'
       device,/color
       device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
    endif

    erase
    xyouts,.4,.95,'WACCM-4 '+sdate,/normal,charsize=2,color=0
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
    ymx=yorig(0)+ylen
    ...
    contour,yz_no,alat,zlev,xrange=[0.,ksteps-1],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='NO',/ylog
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,yz_co,alat,zlev,xrange=[-90.,90.],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='CO',/ylog
    index=where(level gt 0)
...
imax=20.
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
;   contour,yz_height,alat,zlev,xrange=[-90.,90.],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='dZ',/ylog
    contour,yz_gph,alat,zlev,xrange=[-90.,90.],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Pressure (hPa)',xtitle='Days',title='dZ',/ylog
    index=where(level gt 0)
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,yz_t,alat,zlev,xrange=[-90.,90.],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,xtitle='Days',title='Temperature',/ylog
    index=where(level gt 0)
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,yz_u,alat,zlev,xrange=[-90.,90.],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,xtitle='Days',title='Zonal Wind',/ylog
    index=where(level gt 0)
...
;   contour,yz_u,alat,zlev,/overplot,levels=[0],/follow,c_labels=[0],/noeras,color=0,thick=5
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,yz_v,alat,zlev,xrange=[-90.,90.],yrange=[max(zlev),min(zlev)],levels=level,c_color=col1,/cell_fill,/noeras,color=0,xtitle='Days',title='Meridional Wind',/ylog
    index=where(level gt 0)
    contour,yz_v,alat,zlev,/overplot,levels=[5.,10.,20.,30.],/follow,c_labels=[0,0,0,0],/noeras,color=0
    contour,yz_v,alat,zlev,/overplot,levels=[-40.,-30.,-20.,-10.,-5.],/follow,c_labels=[0,0,0,0,0],/noeras,color=mcolor,c_linestyle=5
;   contour,yz_v,alat,zlev,/overplot,levels=[0],/follow,c_labels=[0],/noeras,color=0,thick=5
    !type=2^2+2^3+2^5
    xmnb=xmx+0.075
    xmxb=xmnb+cbarydel
    set_viewport,xmnb,xmxb,ymn,ymx
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_waccm_Traject_Season_6pan_'+sdate+'.ps -rotate -90 yz_waccm_Traject_Season_6pan_'+sdate+'.jpg'
       spawn,'rm -f yz_waccm_Traject_Season_6pan_'+sdate+'.ps'
    endif

...
; loop over years
;
for iyear=2007,2013 do begin		; 2015/2016 is the last SH v4.2 season
...
plotyear:
save,filename='cips_l2_medians_vs_sza_south_'+syear+'.sav',SZALIM_HI,SZALIM_LO,kday,sdate_all,dfs_all,freq20,freq51,szabin,medalb20_all,medalb51_all,sigalb20_all,sigalb51_all,$
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
for j=0,nlvls-1 do begin
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.ps -rotate -90 time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.jpg'
;      spawn,'rm -f time-sza-6pan_cips_l4_v4.20_v5.10_rawfiles_south_'+syear+'.ps'
    endif

;contour_summary_r_hi.pro -- Make the radius contour plots, but omit the smallest
;  particles. The summary files already omit radius < 20, but try even higher
;  thresholds. Will this give us plots with larger radii at the highest latitudes?

;NO: THE PLOTS WERE SIMILAR (I COMPARED FOR NORTH_09).
   ...
   for i=0,ndays-1 do begin
      ...
      if nx gt 0 then begin
         for j=0,nlat-1 do begin
            good=where(rad(x,j) ne fill AND RAD(X,J) GT MINRAD,ngood)
            if ngood gt 2 then begin
               array(i,j)=mean(rad(x(good),j))
            endif
         endfor
      endif
   endfor
...
DEVICE,/BOLD
DEVICE, FILENAME=PTHo+'R_V4.20_'+season+'_'+G+'G_HI.ps'
...
FLEV = [0,20,30,33,36,39,42,45,48,51,54]
...
title='Asc Minus Desc Radius',SUBTITLE=SUBTI
COLOR_PANEL_INTEGER,POS,FLEV,COL,-0.00,1.4
XYOUTS,0.975,0.855,'nm',/NORMAL,CHARSIZE=1.5,ALIGNMENT=0.5

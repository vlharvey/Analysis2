...
;       pvyz(nr/2:nr-1,k)=reverse(pv2(nr/2:nr-1,ilon2,k))
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='yz_cross_pole_geos_nh_'+sdate+'_'+slon1+'_15pan.ps'
...
index=where(yyz lt 3000)
    ...
    int=(imax-imin)/float(nlvls)
    level=imin+int*findgen(nlvls)
    contour,sfyz,alat,th,levels=level,/cell_fill,c_color=col1,color=0,title='Stream Function',$
            xtitle=slon1+'       '+slon2,ytickname=[' ',' ',' ',' ',' ',' ',' ',' '],$
            xtickname=['Eq','30','60','NP','60','30','Eq'],xticks=6,yrange=[500.,3000.]
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim yz_cross_pole_geos_nh_'+sdate+'_'+slon1+'_15pan.ps -rotate -90 '+$
                            'yz_cross_pole_geos_nh_'+sdate+'_'+slon1+'_15pan.jpg'
        spawn,'/usr/bin/rm yz_cross_pole_geos_nh_'+sdate+'_'+slon1+'_15pan.ps'
     endif
     jumpday:
goto,jump
end

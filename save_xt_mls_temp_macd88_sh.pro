;
; save Hovmollers of MLS temperature
; SH
;
@stddat
@kgmt
...
; loop over years
;
for iyear=lstyr,ledyr-1 do begin
...
; save 
;
save,file='xt_mls_tp_'+yearlab+'_'+slat+'.sav',mlspolartp_xt,kday,longrid,sdate_all
quick:
restore,'xt_mls_tp_'+yearlab+'_'+slat+'.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='xt_mls_tp_'+yearlab+'_'+slat+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
index=where(mlspolartp_xt le 0.)
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim xt_mls_tp_'+yearlab+'_'+slat+'.ps -rotate -90 xt_mls_tp_'+yearlab+'_'+slat+'.png'
    endif

...
; contour NOx and the vortex edge as a function of latitude and time
...
for ievent=0,nevent-1L do begin
    ...
    restore,idir+'yt_waccm_nox+mark_'+yearlab+'.sav'
;
...
;
    if ievent eq 0L then begin
       ...
       nytmark_all=0.*ytnox
       ytspeed_all=0.*ytnox
       nytspeed_all=0.*ytnox
    endif

    index=where(ytnox ne -9999.)
    if index(0) ne -1L then ytnox_all(index)=ytnox_all(index)+ytnox(index)
    if index(0) ne -1L then nytnox_all(index)=nytnox_all(index)+1.0
...
index=where(nytnox_all gt 0.)
if index(0) ne -1L then ytnox_all(index)=ytnox_all(index)/float(nytnox_all(index))
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='yt_waccm_nox+mark_'+slev+'_composite.ps'
         device,/color
...
plotarray=reform(ytnox(*,*,kk))*1.e9
stitle=' '
;stitle='Composite NOx, Arctic Vortex, and Wind Speed'
cbartitle=slev+' WACCM NOx (ppbv)'
;if rlev le 2000. then begin
;   plotarray=reform(yth2o(*,*,kk))
;   stitle='Composite H2O, Arctic Vortex, and Wind Speed'
;   cbartitle=slev+' WACCM H2O (ppbv)'
;endif
...
contour,smooth(ytmark(*,*,kk),5,/edge_truncate),-30.+findgen(kday),alat,/overplot,levels=[0.1,0.3,0.5,0.7,0.9],color=0,thick=15,/noeras,/follow,min_value=-9999.
loadct,0
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim yt_waccm_nox+mark_'+slev+'_composite.ps -rotate -90 '+$
         'yt_waccm_nox+mark_'+slev+'_composite.jpg'
endif

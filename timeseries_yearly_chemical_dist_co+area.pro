;
; timeseries of chemical distinction d and vortex area
;
@stddat
@kgmt
@ckday
...
;read,'Enter desired theta ',rth
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='timeseries_d+area_'+sth+'.ps'
             device,/color
...
if iyear eq 0L then begin
   if ssw(iyear) eq 0 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[0.,0.8],color=0,charsize=2,charthick=2,ytitle='Chemical Distinction',xrange=[startday-1,max(fdoy)-30],xticks=n_elements(smonth)-1,xtickname=smonth,thick=5
      for i=0L,nn-1L do begin
...
if iyear eq 0L then begin
   if ssw(iyear) eq 0 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[0,40],color=0,charsize=2,charthick=2,ytitle='Vortex Area (%)',xrange=[startday-1,max(fdoy)-30],xticks=n_elements(smonth)-1,xtickname=smonth,thick=5,/noeras
      for i=0L,nn-1L do begin
          if fdoy(i) ge startday and fdoy(i) lt max(fdoy)-30 then begin
          plots,fdoy(i),arrmean(i)-arrsig(i)
          plots,fdoy(i),arrmean(i)+arrsig(i),/continue,color=150,thick=8
   ...
   if ssw(iyear) eq 1 then plot,fdoy,arr1,yrange=[0,30],color=0,charsize=2,charthick=2,ytitle='Vortex Area (%)',xrange=[startday,max(fdoy)-30],xticks=n_elements(smonth)-1,xtickname=smonth,title=sth,thick=10,/nodata
day0=startday
for i=0L,n_elements(smonth)-1 do begin
    plots,day0,500
    plots,day0,300,/continue,color=0
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim timeseries_d+area_'+sth+'.ps -rotate -90 timeseries_d+area_'+sth+'.jpg'
endif

...
;
index=where(finite(area_allyearsmls) eq 1L and area_allyearsmls ne 0.)
arrmeanmls=median(area_allyearsmls(index))
arrsigmls=stdev(area_allyearsmls(index))
;
; SD-WACCM
;
; loop over years
...
; calculate means and sigmas for non ssw years
;
index=where(finite(area_allyearssdwaccm) eq 1L and area_allyearssdwaccm ne 0.)
arrmeansdwaccm=median(area_allyearssdwaccm(index))
arrsigsdwaccm=stdev(area_allyearssdwaccm(index))
;
; end SD-WACCM
;
; loop over years
...
;read,'Enter desired theta ',rth
          ...
          if setplot eq 'ps' then begin
             ...
             !p.font=0
             device,font_size=9
             device,/landscape,bits=8,filename='barplot_area_merra+waccm+mls_'+sth+'_'+smon(0)+'.ps'
             device,/color
...
; calculate means and sigmas for non ssw years
;
index=where(finite(area_allyears) eq 1L and area_allyears ne 0.)
arrmean=median(area_allyears(index))
arrsig=stdev(area_allyears(index))
...
plot,lyear,area_allyears,yrange=[0,50],color=0,charsize=2,charthick=2,ytitle=sth+' Vortex Area (%)',xrange=[min(lyear),max(lyear)],psym=8,/noeras,symsize=3
oplot,lyear,area_allyears+areasig_allyears,psym=1,color=0,symsize=2
oplot,lyear,area_allyears-areasig_allyears,psym=1,color=0,symsize=2
for n=0,nyear-1 do begin
    plots,lyear(n),area_allyears(n)+areasig_allyears(n)
    plots,lyear(n),area_allyears(n)-areasig_allyears(n),/continue,color=0,thick=5
;   if ssw(n) eq 1 then oplot,[lyear(n),lyear(n)],[area_allyears(n),area_allyears(n)],color=250,psym=8
endfor
oplot,lyear,area_allyearssdwaccm,color=250,psym=8,symsize=2
oplot,lyear,area_allyearssdwaccm+areasig_allyearssdwaccm,psym=1,color=250
oplot,lyear,area_allyearssdwaccm-areasig_allyearssdwaccm,psym=1,color=250
for n=0,nyear-1 do begin
    plots,lyear(n),area_allyearssdwaccm(n)+areasig_allyearssdwaccm(n)
    plots,lyear(n),area_allyearssdwaccm(n)-areasig_allyearssdwaccm(n),/continue,color=250,thick=3
;   if ssw(n) eq 1 then oplot,[lyear(n),lyear(n)],[area_allyearssdwaccm(n),area_allyearssdwaccm(n)],color=250,psym=8
endfor
index=where(area_allyearsmls eq 0.)
if index(0) ne -1L then area_allyearsmls(index)=0./0.
if index(0) ne -1L then areasig_allyearsmls(index)=0./0.
oplot,lyear,area_allyearsmls,color=150,psym=8,symsize=1.25
oplot,lyear,area_allyearsmls+areasig_allyearsmls,psym=1,color=150
oplot,lyear,area_allyearsmls-areasig_allyearsmls,psym=1,color=150
for n=0,nyear-1 do begin
    plots,lyear(n),area_allyearsmls(n)+areasig_allyearsmls(n)
    plots,lyear(n),area_allyearsmls(n)-areasig_allyearsmls(n),/continue,color=150,thick=2
;   if ssw(n) eq 1 then oplot,[lyear(n),lyear(n)],[area_allyearsmls(n),area_allyearsmls(n)],color=150,psym=8
endfor
...
xyouts,xmn+0.01,ymx-0.03,smon(0),/normal,charsize=2,charthick=2,color=0
xyouts,xmn+0.21,ymx-0.09,'MLS',/normal,charsize=1.5,charthick=2,color=150
xyouts,xmn+0.21,ymx-0.06,'MERRA',/normal,charsize=1.5,charthick=2,color=0
xyouts,xmn+0.21,ymx-0.03,'SD-WACCM',/normal,charsize=1.5,charthick=2,color=250
;
; solar cycle?
;
;axis,yaxis=1,/save,yrange=[50,100],color=250,ytitle='Solar 10.7 cm Radio Flux',charsize=2,charthick=2
...
set_viewport,xmn,xmx,ymn,ymx
!type=2^2+2^3
diff=100.*(area_allyears-area_allyearssdwaccm)/area_allyearssdwaccm
plot,lyear,diff,yrange=[-50,100],color=0,charsize=2,charthick=2,ytitle='100*(MERRA-WACCM)/WACCM',xrange=[min(lyear),max(lyear)],psym=8,/noeras,symsize=2
oplot,lyear,diff,thick=3,color=0
oplot,lyear,0*diff,color=0
;
; WACCM-MLS
;
diff=100.*(area_allyearsmls-area_allyearssdwaccm)/area_allyearssdwaccm
axis,yaxis=1,/save,yrange=[-100,100],color=150,charsize=2,charthick=2,ytitle='100*(MLS-WACCM)/WACCM',xrange=[min(lyear),max(lyear)],/noeras
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim barplot_area_merra+waccm+mls_'+sth+'_'+smon(0)+'.ps -rotate -90 barplot_area_merra+waccm+mls_'+sth+'_'+smon(0)+'.jpg'
endif

...
; superimpose vortex area from GEOS-5
; VLH February 5 2010
;
@stddat
@kgmt
...
;
      ...
      for j=0L,nr-1L do begin
          ...
          if index(0) ne -1L then begin
              ...
              markbarg(j,k)=total(markpts(index))/float(nx)
          endif
      endfor
      endfor
;
...
;contour,markbarg,alat,th,/noeras,/fill,c_color=col1,color=0,levels=level,title=sdate
;index=where(level lt 0.)
;contour,markbarg,alat,th,/noeras,/follow,color=mcolor,levels=level(index),/overplot
;index=where(level gt 0.)
;contour,markbarg,alat,th,/noeras,/follow,color=0,levels=level(index),/overplot
;contour,markbarg,alat,th,/noeras,/follow,color=0,levels=[0],/overplot,thick=3
;contour,zbarg,alat,th,/noeras,/follow,color=mcolor,levels=10.+10*findgen(20),/overplot,thick=5
;stop
;
...
;
      for thlev=0,nth-1 do begin
          ...
          z1=reform(z2(*,*,thlev))
          index=where(y2d gt 0. and mark1 gt 0.0,nn)
          if index(0) ne -1 then begin
             varea_nh_zt(icount,thlev)=total(area(index))		; km^2
             vz_nh_zt(icount,thlev)=total(z1(index))/float(nn)
          endif
          index=where(y2d gt 0. and mark1 lt 0.0,nn)
          if index(0) ne -1 then begin
             aarea_nh_zt(icount,thlev)=total(area(index))
...
; save temp, co, etc
;
save,file='zt_mls_temp+co+anticylone_area_'+yearlab+'.sav',mlspolarco_zt,mlspolartp_zt,kday,$
     varea_nh_zt_alt,aarea_nh_zt_alt,altitude,sdate_all
quick:
restore,'zt_mls_temp+co+anticylone_area_'+yearlab+'.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co+vortex_area_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
index=where(varea_nh_zt_alt eq 0.)
if index(0) ne -1L then varea_nh_zt_alt(index)=0./0.
varea_nh_zt_alt=smooth(varea_nh_zt_alt,7,/nan)
index=where(finite(varea_nh_zt_alt) eq 1)
if index(0) ne -1L then varea_nh_zt_alt(index)=varea_nh_zt_alt(index)/1.e6	; millions of km^2
loadct,0
contour,varea_nh_zt_alt,1.+findgen(kday),altitude,levels=[30],color=mcolor*.8,/follow,/overplot,thick=15
contour,varea_nh_zt_alt,1.+findgen(kday),altitude,levels=[50],color=mcolor*.6,/follow,/overplot,thick=15
contour,varea_nh_zt_alt,1.+findgen(kday),altitude,levels=[75],color=mcolor*.4,/follow,/overplot,thick=15
contour,varea_nh_zt_alt,1.+findgen(kday),altitude,levels=[100],color=mcolor*.2,/follow,/overplot,thick=15
loadct,39
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS Average Temperature > 60!uo!n N (K)'
ybox=[0,10,10,0,0]
x1=imin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co+vortex_area_'+yearlab+'.ps -rotate -90 zt_mls_temp+co+vortex_area_'+yearlab+'.jpg'
    endif
end

...
ilat=where(abs(alat-rlat) eq min(abs(alat-rlat)))
endif
      zxz=fltarr(nc,nth)
      txz=fltarr(nc,nth)
      tzm=fltarr(nr,nth)
      ...
      for k=0L,nth-1L do begin
          ...
          for i=0L,nc-1L do txz(i,k)=temp2(ilat(0),i,k)
          for i=0L,nc-1L do zxz(i,k)=gph2(ilat(0),i,k)/1000.
      endfor
...
contour,txz,alon,zxz,color=0,xtitle='Longitude',thick=6,yrange=[0,120],/noeras,ytitle='Theta (K)',/cell_fill,$
     title=sdate,levels=level,c_color=col1
contour,txz,alon,zxz,levels=level,c_color=mcolor,/follow,/overplot,c_labels=1+0*level,min_value=0
imin=min(level)
imax=max(level)

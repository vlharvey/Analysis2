...
if iyear eq 0L then begin
   if ssw(iyear) ne 2 then begin
      loadct,0
      plot,fdoy,arrmean,yrange=[0,50],color=0,charsize=2,charthick=2,ytitle='MERRA Vortex Area (%)',xrange=[startday-1,max(fdoy)],xticks=n_elements(smonth)-1,xtickname=smonth,thick=5,/noeras,/nodata
;      for i=0L,nn-1L do begin
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert timeseries_area_merra_'+sth+'.ps -rotate -90 timeseries_area_merra_'+sth+'.jpg'
endif

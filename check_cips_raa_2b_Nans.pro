...
ifiles=file_search(dir+'cips_raa_2b*2016-*_alb.nc')
...
;help,longitude,latitude,NLAYERS,RAYLEIGH_ALBEDO,RAYLEIGH_ALBEDO_ANOMALY,RAYLEIGH_ALBEDO_ANOMALY_UNC,UT_DATE,UT_TIME,ZENITH_ANGLE
result=size(longitude)
npoints=result(-1)

good1=where(finite(longitude) eq 1)
good2=where(finite(latitude) eq 1)
good3=where(finite(NLAYERS) eq 1)
good4=where(finite(RAYLEIGH_ALBEDO) eq 1)
good5=where(finite(RAYLEIGH_ALBEDO_ANOMALY) eq 1)
good6=where(finite(RAYLEIGH_ALBEDO_ANOMALY_UNC) eq 1)
good7=where(finite(UT_DATE) eq 1)
good8=where(finite(UT_TIME) eq 1)
good9=where(finite(ZENITH_ANGLE) eq 1)

print,ncfile0,100.*n_elements(good9)/float(npoints),' good data'
;print,100.*n_elements(good1)/float(npoints),' good lons'

fracgood_all(ifile)=100.*n_elements(good9)/float(npoints)
...
;   n_elements(good3) ne n_elements(good8) or $		 ; nlayers has zeros where others are Nan
;   n_elements(good3) ne n_elements(good9) or $
if n_elements(good4) ne n_elements(good5) or $
...
;  n_elements(good6) ne n_elements(good7) or $		; ut_date has zeros where others are Nan
...
;  n_elements(good7) ne n_elements(good9) or $
   n_elements(good8) ne n_elements(good9) then stop,'different number of Nans in 2 or more arrays'
endfor

erase
loadct,39
plot,fyr_all,fracgood_all,psym=8,color=0,title=ncfile0,charsize=2,charthick=2,ytitle='% of RAA L2B Orbit that is not Nan'
save,file='raa_l2b_fracgood_NH16SH16.sav',fyr_all,fracgood_all,ncfile0,orbitnum_all

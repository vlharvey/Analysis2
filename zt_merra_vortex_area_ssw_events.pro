...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/times,filename='../Figures/merra_vortex_area_ssw_event_'+esdate0+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
xmax=30.
contour,area_composite,-1.*koff+findgen(kday),th,color=0,xtitle='Days From SSW Onset ('+esdate0+')',thick=6,yrange=[500.,max(th)],/noeras,ytitle='Theta (K)',/cell_fill,$
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,$
      xtitle='MERRA SSW Vortex Area (% NH)',charthick=2,charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ../Figures/merra_vortex_area_ssw_event_'+esdate0+'.ps -rotate -90 ../Figures/merra_vortex_area_ssw_event_'+esdate0+'.jpg'
endif

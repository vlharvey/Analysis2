;
; water
; Plot time-altitude Arctic HIRDLS
...
;
      ...
      for ii=0L,n_elements(id)-1L do begin
          if latitude(ii) ge 60. then begin
             ...
             good=where(no2_prof ne -99. and no2_prof lt 1.e-5,ngood)
             if good(0) ne -1L then begin
...
plot,polart,altitude,color=0,/noerase,title='Temperature',xrange=[170.,300.]
set_viewport,.15,.85,.1,.45
plot,polarno2,altitude,color=0,/noerase,title='H!l2!nO',xrange=[1.e-6,1.e-5],/xlog
;
...
mlspolarn2o_zt=mlspolarn2o_zt*1.e6
save,file='zt_hirdls_polarh2o+mlsh2o_2006.sav',polart_zt,polarno2_zt,polarn2o_zt,$
     mlspolarn2o_zt,kday,altitude,sdate_all
quick:
restore,'zt_hirdls_polarh2o+mlsh2o_2006.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_hirdls_polarh2o+mlsh2o_'+syear(0)+'.ps'
   device,/color
...
contour,polarno2_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[15.,57.],$
      charsize=1.5,color=0,ytitle='Altitude (km)',title='HIRDLS Avg H!l2!nO and MLS H!l2!nO > 60 N',/fill,c_color=col1,$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,min_value=0.
contour,polarno2_zt,1.+findgen(kday),altitude,levels=level,color=mcolor,/follow,/overplot,c_labels=1+fltarr(nlvls)
...
contour,mlspolarn2o_zt,1.+findgen(kday),altitude,levels=level,color=0,/follow,/overplot,$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_hirdls_polarh2o+mlsh2o_'+syear(0)+'.ps -rotate -90 zt_hirdls_polarh2o+mlsh2o_'+syear(0)+'.jpg'
    endif
end

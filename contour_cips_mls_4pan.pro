...
nxdim=750
nydim=750
xorig=[0.175,0.6,0.175,0.6]
yorig=[0.525,0.525,0.1,0.1]
xlen=0.275
ylen=0.325
cbaryoff=0.07
cbarydel=0.01
...
;
for ih=0,1 do begin
if ih eq 0 then shem='north'
if ih eq 1 then shem='south'
...
dum=findfile(mdir+shem+'_'+syr+'_'+version+'_LT.sav')
if dum(0) eq '' then goto,skip
restore,mdir+shem+'_'+syr+'_'+version+'_LT.sav'
;
; restore CIPS season
;
...
; YEAR            INT       = Array[589]
;
restore,cdir+shem+'_'+syr+'_'+version+'_'+salb+'_cld.sav'
print,'restored '+cdir+shem+'_'+syr+'_'+version+'_'+salb+'_cld.sav'
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,/helvetica,bits_per_pixel=8,filename='contour_cips_mls_4pan_'+version+'_'+syr+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=2
   !p.charthick=2
...
ymin=30		;min(latlo)		; set to 30 to force v4 to match v5
ymax=139	;max(latlo)		; set to 139 to force v4 to match v5
contour,smooth(alb,3),dfs,latlo,ytitle='ASC    Latitude    DES',color=0,title='CIPS Albedo > '+salb,charsize=1.25,ytickv=latlo(index),$
        yticks=nyticks-1,ytickname=strcompress(long(ival*latlosave(index)),/remove_all),/fill,levels=tlevel,c_color=col1,xrange=[-51,71],yrange=[ymin,ymax]
if shem eq 'north' then contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140,150],color=.9*mcolor,thick=8
if shem eq 'south' then contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140,150],color=.9*mcolor,thick=8
imin=min(tlevel)
imax=max(tlevel)
xmnb=xmx+cbaryoff
xmxb=xmnb+cbarydel
...
!type=2^2+2^3+2^5
plot,[0,0],[imin,imax],xrange=[0,10],yrange=[imin,imax],color=0,title='G',charsize=1.25,charthick=1.5
xbox=[0,10,10,0,0]
y1=imin
...
nlvls=n_elements(tlevel)
col1=(findgen(nlvls)/nlvls)*mcolor
contour,smooth(rad,3),dfs,latlo,color=0,title='CIPS Radius > '+salb,charsize=1.25,ytickv=latlo(index),$
        yticks=nyticks-1,ytickname=strcompress(long(ival*latlosave(index)),/remove_all),/fill,levels=tlevel,c_color=col1,xrange=[-51,71],yrange=[ymin,ymax]
if shem eq 'north' then contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140,150],color=.9*mcolor,thick=8
if shem eq 'south' then contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140,150],color=.9*mcolor,thick=8
imin=min(tlevel)
...
xyouts,.425,.46,version,color=0,/normal,charsize=1.5,charthick=2
xmn=xorig(2)
xmx=xorig(2)+xlen
ymn=yorig(2)
ymx=yorig(2)+ylen
...
nlvls=n_elements(tlevel)
col1=(findgen(nlvls)/nlvls)*mcolor
contour,smooth(mls_tp,3),dfs,latlo,xtitle='DFS 20'+syr,ytitle='ASC    Latitude    DES',color=0,title='MLS Temperature',charsize=1.25,ytickv=latlo(index),$
        yticks=nyticks-1,ytickname=strcompress(long(ival*latlosave(index)),/remove_all),/fill,levels=tlevel,c_color=col1,xrange=[-51,71],yrange=[ymin,ymax]
contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[150],color=mcolor,thick=8
contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140],color=mcolor,thick=8
imin=min(tlevel)
imax=max(tlevel)
xmnb=xmx+cbaryoff
xmxb=xmnb+cbarydel
...
nlvls=n_elements(tlevel)
col1=(findgen(nlvls)/nlvls)*mcolor
contour,smooth(mls_h2o,3),dfs,latlo,xtitle='DFS 20'+syr,color=0,title='MLS Water Vapor',charsize=1.25,ytickv=latlo(index),$
        yticks=nyticks-1,ytickname=strcompress(long(ival*latlosave(index)),/remove_all),/fill,levels=tlevel,c_color=col1,xrange=[-51,71],yrange=[ymin,ymax]
;contour,smooth(mls_h2o,3),dfs,latlo,/overplot,/follow,level=[5],color=mcolor,thick=8
if shem eq 'north' then contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140,150],color=0*mcolor,thick=8
if shem eq 'south' then contour,smooth(mls_tp,3),dfs,latlo,/overplot,/follow,level=[140,150],color=0*mcolor,thick=8
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim contour_cips_mls_4pan_'+version+'_'+syr+'.ps -rotate -90 contour_cips_mls_4pan_'+version+'_'+syr+'.jpg'
;  spawn,'rm -f contour_cips_mls_4pan_'+version+'_'+syr+'.ps'
endif

...
;oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_n2o_'+syear(iyear)+'.ps'
       device,/color
...
;           c_labels=0*level,min_value=0.
    contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,color=mcolor,/follow,levels=level(9:12),/overplot,$
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_n2o_'+syear(iyear)+'.ps -rotate -90 zt_ace_n2o_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

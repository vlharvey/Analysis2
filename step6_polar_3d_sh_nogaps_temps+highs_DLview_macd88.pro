...
dir='/Volumes/earth/harvey/NOGAPS_Alpha/Datfiles/NOGAPSA_'
slabs=['00','06','12','18']
slabs=['12']
ntimes=n_elements(slabs)
lstmn=7L & lstdy=20L & lstyr=2007L
ledmn=7L & leddy=20L & ledyr=2007L
lstday=0L & ledday=0L
;
; get date range
;
...
;
      ...
      ifile=dir+sdate+'_MetO_sabmls_aim9c.nc3'
      rd_nogaps_nc3,ifile,nc,nr,nth,alon,alat,th,pv2,p2,msf2,u2,v2,q2,$
         qdf2,mark2,sf2,vp2,o32,h2o2,iflag
      if iflag eq 1 then goto,jump
      temp2=0.*p2
      for lev=0L,nth-1L do temp2(*,*,lev)=th(lev)*(p2(*,*,lev)/1000.)^.286
      z2=(msf2-1004.*temp2)/9.86/1000.
...
;   if l eq 0 then begin
       zindex=where(th ge 300. and th le 9000.,nth2)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='polar_3d_sh_nogaps_meto_'+sdate+'_DL.ps'
   device,/color
...
;                  /T3D,zvalue=nz,thick=1,max_value=1.e15
           contour,temp,xcs,ycs,levels=[280.,290.,300.,310.],color=0,/T3D,zvalue=nz,thick=2,min_value=-1.e15
...
;oplot,xcs(index),ycs(index),psym=8,symsize=2,color=20,/T3D,zvalue=nz
;contour,temp,xcs,ycs,levels=[280.],/T3D,zvalue=nz,c_linestyle=0,/overplot,color=1,thick=6
;endif
;endif
...
jumplev:
    ...
    !psym=0
    xyouts,0.2,0.9,'Antarctic Vortex in NOGAPS-ALPHA',/normal,charsize=3,color=0,charthick=2
    xyouts,0.42,0.85,sdate,/normal,charsize=3,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_3d_sh_nogaps_meto_'+sdate+'_DL.ps -rotate -90 polar_3d_sh_nogaps_meto_'+sdate+'_DL.jpg'
    endif
    endfor	; loop over daily output times
goto, jump
end

;
; read nc files from Doug Kinnison of WACCM Surface Area Density of Sulf
; /aura7/harvey/CCMval_data/Datfiles
;
; 10 day instantaneous fields for 1960 to 1987?
;
...
;       print,result.name
        if result.name ne 'sad_sulf' then ncdf_varget,ncid,ncdf_varid(ncid,result.name),data
...
;
; read SAD data one time step at a time and save daily file. units = "10^(-6) m ^(-1)"
; particle_surface_area_density_of_sulfate_aerosol
;
        if result.name eq 'sad_sulf' then begin
           count = [nc,nr,nl,1]
           for n=0L,nt-1L do begin
               sdate0=sdate(n)
               ofile=dir+sdate0+'.sav'
               print,ofile,time(n)
;              dum=findfile(ofile)

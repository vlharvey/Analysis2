...
ACE_FTS_period = [20040221,20180630] ;uses yearly ACE-FTS nc files (produced using ACE_raw2netcdf.pro) [last updated 20130226]

ace_obs_path='/atmos/aura3/data/ACE_data/Datfiles/v3.5-6/'		;Z:\Brakebusch\SOSST\ACE original nc\'
...
ncid = ncdf_create(output_path+'sathist_02_ACE-FTS_v3.6_'+start_date+'-'+end_date+'_c'+today()+'.nc',/clobber)
  ncdf_attput, ncid, 'Description', 'Satellite profile positions and times from '+start_date+' through '+end_date, /global
  ncdf_attput, ncid, 'Author', 'V. Lynn Harvey, file was created on '+today()+' using satellite_locations_jmp_ace_c20130529_update.pro', /global
  ncdf_attput, ncid, 'Comment', '(2) ACE-FTS ascii files version v3.6 were used, '+str_ace_fts_period+'; ', /global
...
;produce date variables
    ...
    ;read ACE-FTS
     file=file_search(ace_obs_path+'ACEFTS_L2_v3p6_GLC.nc',count=nfiles)
    ...
    if nfiles ne 0 then begin
      if file ne ace_old_file then begin 
        ncid2 = ncdf_open(file[0])
          ...
          ncdf_varget,ncid2,ncdf_varid(ncid2,'day'), ace_orig_day
          ncdf_varget,ncid2,ncdf_varid(ncid2,'month'), ace_orig_month
          ncdf_varget,ncid2,ncdf_varid(ncid2,'year'), ace_orig_year
          ncdf_varget,ncid2,ncdf_varid(ncid2,'hour'), ace_orig_time
          ncdf_varget,ncid2,ncdf_varid(ncid2,'orbit'), ace_orig_occ_num
        ncdf_close,ncid2
;
; convert occultation number to an integer
;
ace_orig_occ_num=long(ace_orig_occ_num)
;
; occ_type needs to be built
;
ace_orig_occ_type=strarr(n_elements(ace_orig_occ_num))
;
; build date from year, month, day
;
ace_orig_date=long(strcompress(long(ace_orig_year),/remove_all)+string(format='(i2.2)',ace_orig_month)+string(format='(i2.2)',ace_orig_day))

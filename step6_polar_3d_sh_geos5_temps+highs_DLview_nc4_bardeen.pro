...
lstmn=6L & lstdy=14L & lstyr=2007L
ledmn=6L & leddy=14L & ledyr=2007L
lstday=0L & ledday=0L
;
; get date range
;
...
;   if l eq 0 then begin
       ...
       x2d=fltarr(nc+1,nr2)
       y2d=fltarr(nc+1,nr2)
       for i=0,nc do y2d(i,*)=alat(0:nr2-1L)
...
;   endif
    ...
    yy=-65.3 & xx=259.9
;   yy=-65.4 & xx=141
    sxx=strcompress(fix(xx),/remove_all)+'E'
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='polar_3d_sh_geos5_meto_'+date+'_DL+highs_nc4_'+sxx+'.ps'
   device,/color
...
; coordinate transformation
    ...
    dum=fltarr(nc+1,nr2)
    if xx eq 141. then irot=-60.
    if xx eq 259.9 then irot=60.
...
; temperature
        ...
        temp(nc,*)=temp(0,*)
        index=where(y2d gt 0. or temp eq 0.)
...
; draw latitude circles
        ...
        MAP_SET,-90,0,-1.*irot,/stereo,/contin,/grid,/noborder,/noeras,londel=90.,$
            label=1,lonlab=1,charsize=2,latdel=180.,/t3d,zvalue=nz(kk)
;
...
;oplot,lonp(index),latp(index),psym=8,symsize=2,color=250,/t3d,zvalue=nz(kk)
;stop
...
;          contour,temp,xcs,ycs,levels=[190.],color=0.25*mcolor,/T3D,zvalue=nz(kk),thick=2,min_value=-1.e15
;          contour,temp,xcs,ycs,levels=[180.],color=0.1*mcolor,/T3D,zvalue=nz(kk),thick=2,min_value=-1.e15
           contour,temp,xcs,ycs,levels=[280.,290.,300.,310.],color=0,/T3D,zvalue=nz(kk),thick=2,min_value=-1.e15
...
lindex=[-1]
        if lindex(0) ne -1 then begin
;          oplot,xcs(lindex),ycs(lindex),/T3D,zvalue=nz(kk),psym=8,symsize=2,color=0
loadct,0
...
;              index=where(tmp ne -9999. and y2d lt -20. and dum eq -1.0*(ihigh+1))
               ...
               oplot,xcs(index),ycs(index),psym=8,color=0,/T3D,zvalue=nz(kk),symsize=2
               contour,tmp,xcs,ycs,levels=[sedge],color=icolmax*.7,$
                 /T3D,zvalue=nz(kk),c_linestyle=0,/overplot,max_value=9999.,thick=3
               jump1:
...
if kk eq 0L then begin
   ...
   oplot,lonp,latp,/T3D,zvalue=nz(kk),color=0,thick=2
   ANG = (90. + yylab) * RADG * 0.5
   FACTOR = TAN(ANG) * FAC20
   THETA = (xx - 90.) * RADG
   xp = FACTOR * COS(THETA)
   yp = -1.* FACTOR * SIN(THETA)
   xyouts,[xp,xp],[yp,yp],sxx,/data,color=mcolor*.9,charsize=3,/t3d,orientation=50,charthick=2
endif
;
; stratopause
;
...
;index=where(y2d lt -30. and temp ge 280.)
...
;oplot,xcs(index),ycs(index),psym=8,symsize=2,color=20,/T3D,zvalue=nz(kk)
;contour,temp,xcs,ycs,levels=[280.],/T3D,zvalue=nz(kk),c_linestyle=0,/overplot,color=1,thick=6
;endif
;endif
...
jumplev:
    ...
    !psym=0
    xyouts,0.25,0.9,'Antarctic Vortex in GEOS-5',/normal,charsize=3,color=0,charthick=2
    xyouts,0.42,0.85,date,/normal,charsize=3,color=0,charthick=2
    xyouts,.05,.83,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_3d_sh_geos5_meto_'+date+'_DL+highs_nc4_'+sxx+'.ps -rotate -90 '+$
                           'polar_3d_sh_geos5_meto_'+date+'_DL+highs_nc4_'+sxx+'.jpg'
    endif
goto, jump
end

...
if index(0) ne -1L then begin
   sdates=sdates(index)
   dttz=reform(dttz(index,*,*))
   ttz=reform(ttz(index,*,*))
   utz=reform(utz(index,*,*))
endif
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_geos5_dthdz+u_'+slat+'.ps'
   device,/color
...
tlat=smooth(tlat,7)
ulat=smooth(reform(utz(*,*,j)),7)
ulat=smooth(ulat,7)
print,slat,' ',min(dthdzlat),max(dthdzlat)
contour,dthdzlat,1.+findgen(kday),th,/noeras,xrange=[1.,kday],yrange=[300.,4000.],$
      charsize=1.5,color=0,ytitle='Theta (K)',title='GEOS-5 d(theta)/dz at '+slat+' Latitude',/fill,c_color=col1,$
      levels=level,xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
;index=where(level gt 0)
;contour,dthdzlat,1.+findgen(kday),th,levels=level(index),color=0,/follow,/overplot,c_labels=1+0*index
;index=where(level lt 0)
;contour,dthdzlat,1.+findgen(kday),th,levels=level(index),color=mcolor,/follow,/overplot,c_labels=1+0*index,c_linestyle=5
contour,dthdzlat,1.+findgen(kday),th,levels=[0],color=0,/follow,/overplot
;contour,tlat,1.+findgen(kday),th,levels=[270],color=mcolor*.9,/follow,/overplot,c_labels=0,thick=2
;contour,tlat,1.+findgen(kday),th,levels=[260],color=mcolor*.8,/follow,/overplot,c_labels=0,thick=2
...
contour,ulat,1.+findgen(kday),th,levels=[25,50,75,100],color=mcolor,/follow,/overplot,c_labels=0
contour,ulat,1.+findgen(kday),th,levels=[-100,-75,-50,-25],color=0,/follow,/overplot,c_labels=0
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_geos5_dthdz+u_'+slat+'.ps -rotate -90 zt_geos5_dthdz+u_'+slat+'.jpg'
endif

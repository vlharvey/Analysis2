...
; in the Arctic vortex as a function of altitude and time
;
@stddat
@kgmt
@ckday
...
;
; min Q from gridpoints in Arctic vortex
;
...
goto,jump
;
; plot altitude-time series of Arctic vortex Qmin
;
plotit:
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_qmin_'+y1+'-'+y2+'.ps'
   device,/color
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],$
     xtitle='Max Descent in the Arctic Vortex (K/day)',charsize=2
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_qmin_'+y1+'-'+y2+'.ps -rotate -90 ukmo_qmin_'+y1+'-'+y2+'.jpg'
endif

...
;***Read WACCM data
      ...
      if dum(0) ne '' then ifile=dir+sdate+'.nc3'
...
;
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='Figures/yz_cross_pole_waccm_nh_'+sdate+'_'+sth+'_'+slon1+'.ps'
...
; plot
    ...
    mtitle=sth+' K '+tlab+' Temperature+Vortex on '+sdate
    !type=2^2+2^3
...
pv=t
    ...
    pmin=min(pv)
    pmax=max(pv(index))
    level=pmin+((pmax-pmin)/nlvls)*findgen(nlvls)
    contour,pv,x,alat,levels=level,/overplot,/cell_fill,c_color=col1
    index=where(level gt 0.)
    if index(0) ne -1L then contour,pv,x,alat,levels=level(index),/overplot,/follow,color=0
    index=where(level lt 0.)
    if index(0) ne -1L then contour,pv,x,alat,levels=level(index),/overplot,/follow,color=icolmax,c_linestyle=5
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim Figures/yz_cross_pole_waccm_nh_'+sdate+'_'+sth+'_'+slon1+'.ps -rotate -90 '+$
                            'Figures/yz_cross_pole_waccm_nh_'+sdate+'_'+sth+'_'+slon1+'.jpg'
        spawn,'/usr/bin/rm Figures/yz_cross_pole_waccm_nh_'+sdate+'_'+sth+'_'+slon1+'.ps'
     endif

;
; read CIPS level 4 data and store in daily IDL save files binned every 5 degrees latitude
;
...
;
      ...
      if norbit eq 0L then goto,jump
;
; loop over orbits
;
...
;
; count clouds in each latitude bin
;
         FOR ilat=0,NLAT-1 DO BEGIN
             ...
             if x(0) ne -1L then cloud_freq(ilat,iorbit)=cloud_freq(ilat,iorbit)+nx
             if x(0) ne -1L then total_points(ilat,iorbit)=xdim*ydim
         endfor
  
if iorbit eq 0L then begin
erase
!type=2^2+2^3
plot,latbin,cloud_freq(*,iorbit),psym=2,/nodata,title=sdate,yrange=[0.,50.],$
     ytitle='Cloud Frequency (%)',xtitle='Latitude',xrange=[-90.,90.]
endif
oplot,latbin,100.*cloud_freq(*,iorbit)/total_points(*,iorbit),psym=0,color=(float(iorbit)/float(norbit))*mcolor,thick=3
...
;
      ...
      print,ofile
      SAVE,file=ofile,latbin,cloud_freq,total_points,norbit
goto,jump
end

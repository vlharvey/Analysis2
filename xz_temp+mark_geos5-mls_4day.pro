...
; --- Test for end condition and close windows.
      ...
      if ndays gt ledday then begin
         ...
         if setplot eq 'ps' then begin
            device, /close
            spawn,'convert -trim xz_temp+mark_geos5-mls_'+slat+'_4days.ps '+$
                  ' -rotate -90  xz_temp+mark_geos5-mls_'+slat+'_4days.jpg'
            stop
         endif
      endif
...
;
      ...
      sdate=syr+smn+sdy
      if sdate ne '20080125' and sdate ne '20080205' and sdate ne '20080216' and sdate ne '20080222' then goto,jump
      print,sdate
...
;        !p.font=0
         ...
         device,/landscape,bits=8,$
                 filename='xz_temp+mark_geos5-mls_'+slat+'_4days.ps'
         device,/color
...
; day 1
;
if imn eq 1 and idy eq 25 then begin
erase
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
...
contour,pdiff,lonbin,altitude,levels=level,/cell_fill,c_color=col2,min_value=-99.,yrange=[20,60],$
        xtickname=[' ',' ',' ',' ',' ',' '],ytickname=[' ',' ',' ',' ',' ',' '],title='GEOS5 - MLS'
...
; day 2
;
if imn eq 2 and idy eq 5 then begin
xmn=xorig(3)
xmx=xorig(3)+xlen
ymn=yorig(3)
ymx=yorig(3)+ylen
...
; day 3
;
if imn eq 2 and idy eq 16 then begin
xmn=xorig(6)
xmx=xorig(6)+xlen
ymn=yorig(6)
ymx=yorig(6)+ylen
...
col1=1+indgen(nlvls)*icolmax/nlvls
level=180.+5.*findgen(nlvls)
contour,geost1,lonbin,altitude,levels=level,c_color=col1,thick=1,/cell_fill,/noeras,yrange=[20,60],ytitle='Altitude (km)',$
        xtickname=[' ',' ',' ',' ',' ',' ']
contour,geost1,lonbin,altitude,/overplot,levels=level,/follow,color=0,c_labels=0*level
contour,geosmark1,lonbin,altitude,/overplot,levels=[0.1],/follow,color=0,c_labels=0*level,thick=5
loadct,0
...
; day 4
;
if imn eq 2 and idy eq 22 then begin
xmn=xorig(9)
xmx=xorig(9)+xlen
ymn=yorig(9)
ymx=yorig(9)+ylen
...
!type=2^2+2^3
!psym=0
contour,mlst1,lonbin,altitude,levels=level,c_color=col1,thick=1,/cell_fill,/noeras,yrange=[20,60],xtitle='Longitude',$
        ytickname=[' ',' ',' ',' ',' ',' ']
contour,mlst1,lonbin,altitude,/overplot,levels=level,/follow,color=0,c_labels=0*level
contour,geosmark1,lonbin,altitude,/overplot,levels=[0.1],/follow,color=0,c_labels=0*level,thick=5
loadct,0

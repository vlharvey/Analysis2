...
; plot time-altitude section of vortex nitrous oxide
;
@fillit
@smoothit
...
;
    ...
    restore,dira+'n2o_ace_v2.2.'+syear(iyear)
...
;oco=ocosmooth
    ...
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_n2o_'+syear(iyear)+'_spring.ps'
       device,/color
...
kday=91
    ...
    contour,oco,1.+findgen(nday),altitude,/noeras,c_color=col1,/cell_fill,levels=level,$
         yrange=[30.,90.],xrange=[1.,kday],xticks=n_elements(xlab)-1L,$
         xtickname=xlab,charsize=1.75,ytitle='Alitude (km)',color=0,$
         title=syear(iyear)+' ACE Arctic Nitrous Oxide',min_value=0.
;   contour,oco,min(fdoy)+findgen(nday),altitude,/noeras,color=0,/follow,levels=level,/overplot,$
;           c_labels=0*level,min_value=0.
    contour,oco,1.+findgen(nday),altitude,/noeras,color=mcolor,/follow,levels=level(9:12),/overplot,$
            c_labels=[0,0,0,0],min_value=0.
    ...
    plot,fdoy,latitude,psym=1,color=0,yrange=[30.,90.],xrange=[1.,kday],xticks=n_elements(xlab)-1L,$
         xtickname=xlab,charsize=1.75,ytitle='Latitude',yticks=2,ytickv=[30.,60.,90.],symsize=0.5

    if setplot ne 'ps' then stop
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_n2o_'+syear(iyear)+'_spring.ps -rotate -90 zt_ace_n2o_'+syear(iyear)+'_spring.jpg'
    endif
    jumpyear:

;
; zonal mean water vapor deviation from multi-year mean
;
@stddat
@kgmt
@ckday
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='MLS_YZ_Anomalies/yz_mls_h2o_minus_multiyear_mean_'+sdate+'.ps'
   !p.charsize=1.55
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,charsize=1.2,charthick=2,xtitle='MLS H!l2!nO Anomaly (ppmv)'
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert MLS_YZ_Anomalies/yz_mls_h2o_minus_multiyear_mean_'+sdate+'.ps -rotate -90 MLS_YZ_Anomalies/yz_mls_h2o_minus_multiyear_mean_'+sdate+'.png'
   spawn,'rm -f MLS_YZ_Anomalies/yz_mls_h2o_minus_multiyear_mean_'+sdate+'.ps'
endif

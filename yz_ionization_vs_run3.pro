...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='yz_ionization_vs_run3_'+sdate+'.ps'
       device,/color
...
;   contour,exfrczm,lat,zmean,levels=level,/overplot,color=0,/noeras
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim yz_ionization_vs_run3_'+sdate+'.ps -rotate -90 yz_ionization_vs_run3_'+sdate+'.jpg'
       spawn,'rm -f yz_ionization_vs_run3_'+sdate+'.ps'
    endif

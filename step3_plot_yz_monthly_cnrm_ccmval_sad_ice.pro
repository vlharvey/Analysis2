;
; read multi-year monthly means of CNRM Surface Area Density of Ice and plot zonal mean
;
a=findgen(8)*(2*!pi/8.)
...
dir='/aura7/harvey/CCMval_data/Datfiles/CCMVal2_REF-B1_CNRM-ACM_2_T3I_sad_ice_'
...
for imon=0L,nmon-1L do begin
    ...
    print,'read CCMVal2_REF-B1_CNRM-ACM_2_T3I_sad_ice_'+smon(imon)+'.sav'
;
; compute zonal mean
;
    nr=n_elements(alat)
...
;
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='step3_plot_yz_monthly_cnrm_ccmval_sad_ice_'+smon(imon)+'.ps'
...
wsadzm=wsadzm*1.e8
    contour,wsadzm,alat,lev,c_color=col1,/cell_fill,title='CNRM SAD ICE '+smon(imon),xrange=[-90.,90.],yrange=[0.,40.],$
         charsize=1.5,ytitle='Altitude (km)',xtitle='Latitude',color=0,levels=level
    contour,wsadzm,alat,lev,color=0,levels=level,/overplot,/follow,c_charsize=1.5,c_labels=0*level
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim step3_plot_yz_monthly_cnrm_ccmval_sad_ice_'+smon(imon)+'.ps -rotate -90 '+$
                            'step3_plot_yz_monthly_cnrm_ccmval_sad_ice_'+smon(imon)+'.jpg'
        spawn,'/usr/bin/rm step3_plot_yz_monthly_cnrm_ccmval_sad_ice_'+smon(imon)+'.ps'
     endif

...
if syear(iyear) eq '2004' then onox_fill(0:35,*)=0.      ; do not extrapolate data voids 
onox=onox_fill
;
; smooth
;
...
;
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       device,font_size=9
       device,/landscape,bits=8,filename='zt_ace_nox_spring_'+syear(iyear)+'.ps'
       device,/color
...
kday=91
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_ace_nox_spring_'+syear(iyear)+'.ps -rotate -90 zt_ace_nox_spring_'+syear(iyear)+'.jpg'
    endif
    jumpyear:

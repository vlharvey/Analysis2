...
; save temp, co, etc
;
save,file='Save_files/zt_mls_temp+co_'+yearlab+'.sav',mlspolarco_zt,mlspolartp_zt,kday,altitude,sdate_all
quick:
restore,'Save_files/zt_mls_temp+co_'+yearlab+'.sav'
sdate0=sdate_all(0)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_mls_temp+co_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
contour,mlspolartp_zt,1.+findgen(kday),altitude,/noeras,xrange=[1.,kday],yrange=[10.,100.],$
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_mls_temp+co_'+yearlab+'.ps -rotate -90 zt_mls_temp+co_'+yearlab+'.jpg'
    endif
end
